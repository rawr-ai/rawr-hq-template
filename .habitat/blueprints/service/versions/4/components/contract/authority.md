...
predicate service_v4_contract_authority_leaf() {
  $filename <: r"(?:^|.*/)src/service/modules/[^/]+/contract/[^/]+\.ts$",
  ! $filename <: r"/contract/index\.ts$"
}

function service_v4_contract_authority_root_status($operation, $binding) js {
  const operation = $operation.text.replace(/\s+/g, "");
  return operation === $binding.text || operation.startsWith($binding.text + ".") ? "ok" : "wrong-root";
}

predicate service_v4_contract_authority_native($operation) {
  or {
    and {
      $program <: contains `import { $..., oc, $... } from "@orpc/contract"`,
      $status = service_v4_contract_authority_root_status(operation=$operation, binding=`oc`),
      $status <: includes "ok"
    },
    and {
      $program <: contains `import { $..., oc as $binding, $... } from "@orpc/contract"`,
      $status = service_v4_contract_authority_root_status(operation=$operation, binding=$binding),
      $status <: includes "ok"
    }
  }
}

predicate service_v4_contract_authority_bridge($binding) {
  or {
    and {
      $binding <: `standard`,
...
predicate service_v4_contract_authority_typebox($binding) {
  or {
    and {
      $binding <: `Type`,
...
predicate service_v4_contract_authority_named($schema) {
  $schema <: identifier()
}

predicate service_v4_contract_authority_raw_schema($schema) {
  or {
    $schema <: `$typebox.$constructor($...)` where {
      service_v4_contract_authority_typebox(binding=$typebox)
    },
    service_v4_contract_authority_named(schema=$schema)
  }
}

predicate service_v4_contract_authority_direct_schema($value) {
  $value <: `$bridge($schema)` where {
    service_v4_contract_authority_bridge(binding=$bridge),
    service_v4_contract_authority_raw_schema(schema=$schema)
  }
}

predicate service_v4_contract_authority_schema($value) {
  or {
    service_v4_contract_authority_direct_schema(value=$value),
    and {
      $value <: identifier(),
      ...
      service_v4_contract_authority_direct_schema(value=$adapted)
    }
  }
}

predicate service_v4_contract_authority_static_text($text) {
  or {
...
predicate service_v4_contract_authority_described($schema) {
  $schema <: `$typebox.$constructor($...)`,
  service_v4_contract_authority_typebox(binding=$typebox),
  $schema <: call_expression(arguments=$arguments),
  $arguments <: [..., $options],
  $options <: contains `description: $description`,
  service_v4_contract_authority_static_text(text=$description),
  not { $schema <: `$typebox.Optional($_)` }
}

predicate service_v4_contract_authority_property($schema) {
  or {
    service_v4_contract_authority_named(schema=$schema),
    service_v4_contract_authority_described(schema=$schema),
    $schema <: `$typebox.Optional($inner)` where {
      service_v4_contract_authority_typebox(binding=$typebox),
      or {
        service_v4_contract_authority_named(schema=$inner),
        service_v4_contract_authority_described(schema=$inner)
      }
    }
  }
}

predicate service_v4_contract_authority_operation($operation) {
...
or {
  `export const $name = $operation` where {
    service_v4_contract_authority_leaf(),
    not { $operation <: object() },
    service_v4_contract_authority_operation(operation=$operation),
    not { service_v4_contract_authority_native(operation=$operation) }
  },
  ...
  `$builder.$direction($schema)` where {
    ...
    not { service_v4_contract_authority_schema(value=$schema) }
  },
  `$builder.errors({ $properties })` where {
    service_v4_contract_authority_leaf(),
    $properties <: some `$code: { $..., data: $data, $... }`,
    not { service_v4_contract_authority_schema(value=$data) }
  },
  `$typebox.Object({ $properties })` where {
    service_v4_contract_authority_leaf(),
    service_v4_contract_authority_typebox(binding=$typebox),
    $properties <: some `$key: $schema` where {
      not { service_v4_contract_authority_property(schema=$schema) }
    }
  }
}
```
