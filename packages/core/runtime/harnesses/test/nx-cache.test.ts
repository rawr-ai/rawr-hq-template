...
const FIXTURE_PREFIX = "runtime-harnesses-nx-cache-";
...
test("restores an unchanged harness build and invalidates owner and upstream inputs", async () => {
  const fixtureRoot = await createFixture();
  const ownerRoot = path.join(fixtureRoot, harnessPath);
  ...
  try {
    ...
    const upstreamConfig = path.join(fixtureRoot, compilerPath, "tsdown.config.ts");
    await writeFile(
      upstreamConfig,
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-harnesses:build", "--outputStyle=static"],
      { cwd: fixtureRoot, env }
    );
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const timer = setTimeout(() => {
      timeoutError = new Error("runtime-harnesses:build exceeded its cache-proof timeout.");
      child.kill("SIGKILL");
    }, 45_000);
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-harnesses:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-harnesses Nx fixture: ${root}`);
  }
