...
assertCondition(
  loggingCorrelationTestSource.includes(
    "writes correlated rpc service logs into .rawr/hq/runtime.log"
  ) &&
    loggingCorrelationTestSource.includes(
      "writes correlated openapi service logs into .rawr/hq/runtime.log"
    ),
...
assertCondition(
  scripts["phase-c:gate:c2-telemetry-contract"] ===
...
assertCondition(
  scripts["phase-c:c2:full"] ===
    "bun run phase-c:c2:quick && bun run phase-a:gate:observability-contract",
