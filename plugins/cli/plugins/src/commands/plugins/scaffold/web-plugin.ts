...
export default class PluginsScaffoldWebPlugin extends RawrCommand {
  ...
  static flags = {
    ...
    "publish-ready": Flags.boolean({
      description: "Generate a publish-ready package scaffold (otherwise defaults to local-only private)",
      default: false,
    }),
  } as const;

  async run() {
    const { args, flags } = await this.parseRawr(PluginsScaffoldWebPlugin);
    ...
    const publishReady = Boolean((flags as any)["publish-ready"]);

    const mode: FactoryWriteMode = baseFlags.dryRun ? "dry-run" : "write";
    const pluginRoot = path.join(workspaceRoot, "plugins", "web", dirName);
...
RAWR HQ plugin scaffold generated by \`rawr plugins scaffold web-plugin\`.
...
1. Build the plugin:
   - \`bun run build\`
2. Enable in RAWR:
   - \`rawr plugins web enable ${dirName} --risk balanced\`
3. Check status:
...
- Default generation is local-only (\`private: true\`). Pass \`--publish-ready\` to generate a publish candidate scaffold.
