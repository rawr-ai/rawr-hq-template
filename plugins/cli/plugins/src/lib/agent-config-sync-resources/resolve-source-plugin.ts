...
import type { HostSourcePlugin } from "./types";
import {
  findWorkspaceRoot,
...
export async function resolveSourcePlugin(
  pluginRef: string,
  cwd = process.cwd(),
  ...
  if (!workspaceRoot) {
    throw new Error(
      "Unable to locate workspace root (expected a ./plugins/{cli,agents,web} directory)",
    );
  }
  ...
  for (const pluginDir of pluginDirs) {
    ...
    if (dirName === pluginRef) pluginDirMatch = pluginDir;

    const packageJson = await readJsonFile<PackageJson>(path.join(pluginDir, "package.json"));
    if (typeof packageJson?.name === "string" && packageJson.name === pluginRef) {
      return loadSourcePluginFromPath(pluginRef, pluginDir);
    }
  }

  if (pluginDirMatch) {
    return loadSourcePluginFromPath(pluginRef, pluginDirMatch);
  }
