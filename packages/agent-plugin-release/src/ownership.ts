...
export function createDistributionOwnershipIndex(
  ...
  for (const memberId of memberIds) {
    const identity = collect(parseOwnershipIdentity(memberId, `ownership.members.${memberId}`), issues);
  ...
  if (claims.length > MAX_OWNERSHIP_CLAIMS) {
    issues.push(issue("COUNT_LIMIT_EXCEEDED", "ownership.claims", "Ownership claims exceed the protocol limit", {
...
export function parseDistributionOwnershipIndex(
  ...
  if (input.schemaVersion !== OWNERSHIP_INDEX_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", `${path}.schemaVersion`, "Unsupported ownership-index version", {
  ...
  const memberIds = claims.filter((claim) => claim.kind === "plugin").map((claim) => claim.ownerPluginId);
...
export function ownershipClaimsFor(
  ...
  return Object.freeze(index.claims.filter((claim) =>
...
function parseClaims(
  ...
  values.forEach((candidate, index) => {
    ...
    const identity = collect(parseOwnershipIdentity(candidate.identity, `${claimPath}.identity`), issues);
    const ownerPluginId = collect(parsePluginId(candidate.ownerPluginId, `${claimPath}.ownerPluginId`), issues);
...
function validateClaims(
  ...
  for (const claim of claims) {
    if (!memberSet.has(claim.ownerPluginId)) {
      issues.push(issue("MISSING_OWNER", `ownership.claims.${claim.kind}.${claim.identity}`, "Claim owner is not a declared release member", {
  ...
  for (const [identity, group] of routing) {
    ...
    if (owners.length > 1 || new Set(group.map((entry) => entry.kind)).size > 1) {
      issues.push(issue("OWNERSHIP_CONFLICT", `ownership.routing.${identity}`, "Plugin identity and alias namespace is ambiguous", {
