...
import { PAYLOAD_TEMP_PREFIX } from "../src/filesystem";
...
describe("agent-plugin explicit destination export", () => {
  ...
  it("rejects a non-closed or modeless request before every reader and destination access", async () => {
    ...
    const application = createExportAgentPluginsApplication({ artifactReader: artifacts, knownNativeHomesReader: homes, undoWriter: undo });
    ...
    const overlapping = await application.execute({
  ...
  it("blocks malformed native-home evidence and overlap before artifact or destination access", async () => {
    ...
    const invalid = await createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: invalidReader,
      undoWriter: new FakeUndoWriter(),
    }).execute(request(fixture.alpha.ref, "targeted-release", destination));
    ...
    const overlap = await createExportAgentPluginsApplication({
      artifactReader: artifacts,
  ...
  it("fails closed when the native-home reader is absent and rejects caller home overrides", async () => {
    ...
    const undo = new FakeUndoWriter();
    const absent = await createExportAgentPluginsApplication({
      artifactReader: artifacts,
    ...
    const homes = new FakeKnownNativeHomesReader(knownHomes());
    const override = await createExportAgentPluginsApplication({
      artifactReader: artifacts,
    ...
    expect(override.kind).toBe("RejectedBeforeMutation");
    expect(homes.reads).toBe(0);
    expect(artifacts.reads).toBe(0);
  });
  ...
  it("settles a complete deterministic layout and repeats as a byte-and-mtime read-only no-op", async () => {
    ...
    const application = createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: undo,
    });

    const first = await application.execute(request(fixture.complete.ref, "complete-set", destination));
    ...
    const second = await application.execute(request(fixture.complete.ref, "complete-set", destination));
  ...
  it("updates one targeted scope, retires only its old claim, and preserves its peer scope", async () => {
    ...
    expect((await application.execute(request(firstFixture.complete.ref, "complete-set", destination))).kind).toBe("MutatedSettled");
    const betaPath = join(destination, "codex/plugins/beta/agents/beta.md");
    const betaBefore = await lstat(betaPath, { bigint: true });

    const updated = await application.execute(request(secondFixture.alpha.ref, "targeted-release", destination));
  ...
  it("re-establishes full-set truth after a targeted update clears the prior complete-set claim", async () => {
    ...
    expect((await application.execute(request(firstSet.complete.ref, "complete-set", destination))).kind)
      .toBe("MutatedSettled");
    ...
    expect((await application.execute(request(secondSet.alpha.ref, "targeted-release", destination))).kind)
      .toBe("MutatedSettled");
    ...
    expect((await application.execute(request(secondSet.complete.ref, "complete-set", destination))).kind)
      .toBe("MutatedSettled");
    ...
    const repeated = await application.execute(request(secondSet.complete.ref, "complete-set", destination));
  ...
  it("treats a later complete set as closed-world only for ledger claims and preserves unmanaged descendants", async () => {
    ...
    const settled = await application.execute(request(alphaOnly.complete.ref, "complete-set", destination));
  ...
  it("renders the explicit Claude layout without creating a Codex projection", async () => {
    ...
    const result = await createExportAgentPluginsApplication({
      ...
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: undo,
    }).execute({ ...request(fixture.alpha.ref, "targeted-release", destination), layout: "claude-v1" });
  ...
  it("preserves unmanaged collisions by default and performs exact reversible replace-planned adoption", async () => {
    ...
    const blockedResult = await application.execute(request(fixture.alpha.ref, "targeted-release", destination));
    expect(blockedResult.kind).toBe("RejectedBeforeMutation");
    expect(await readFile(target, "utf8")).toBe("unmanaged\n");
    expect(undo.beginCalls).toBe(0);

    const adopted = await application.execute({
      ...request(fixture.alpha.ref, "targeted-release", destination),
  ...
  it("cleans its exact replay temporary when a target race blocks inverse publication before mutation", async () => {
    ...
    const application = (undoWriter: UndoWriter) => createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter,
    });
    expect((await application(new FakeUndoWriter()).execute(
      request(firstFixture.alpha.ref, "targeted-release", destination),
    )).kind).toBe("MutatedSettled");
    const undo = new FakeUndoWriter();
    expect((await application(undo).execute(
  ...
  it("settles an independent destination while another destination rejects before mutation", async () => {
    ...
    const result = await createExportAgentPluginsApplication({
      ...
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: undo,
    }).execute({ ...request(fixture.alpha.ref, "targeted-release", good), destinations: [good, blocked] });
  ...
  it("preflights the complete multi-destination candidate and blocks every mutation when the aggregate is overbound", async () => {
    ...
    const result = await createExportAgentPluginsApplication({
      ...
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: undo,
    }).execute({ ...request(fixture.alpha.ref, "targeted-release", second), destinations: [second, first] });
    ...
    await createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: permutedUndo,
    }).execute({ ...request(fixture.alpha.ref, "targeted-release", first), destinations: [first, second] });
  ...
  it("suspends exactly once when accepted-admission validation throws unexpectedly", async () => {
    ...
    await expect(createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: undo,
    }).execute(request(fixture.alpha.ref, "targeted-release", destination)))
  ...
  it("settles only A's exact applied subset when B rejects and blocks C inside one aggregate candidate", async () => {
    ...
    const result = await createExportAgentPluginsApplication({
      ...
      undoWriter: undo,
      failpoints,
    }).execute({ ...request(fixture.alpha.ref, "targeted-release", first), destinations: [first, second, third] });
  ...
  it("blocks hardlinked ledger authority without replacing either link", async () => {
    ...
    expect((await application.execute(request(fixture.alpha.ref, "targeted-release", destination))).kind).toBe("MutatedSettled");
    ...
    const result = await application.execute(request(fixture.alpha.ref, "targeted-release", destination));
    expect(result.kind).toBe("RejectedBeforeMutation");
  ...
  it.each(["symlink", "hardlink"] as const)(
    ...
    async (substitution) => {
      ...
      const initial = createExportAgentPluginsApplication({
        artifactReader: artifacts,
        knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
        undoWriter: new FakeUndoWriter(),
      });
      expect((await initial.execute(request(firstFixture.alpha.ref, "targeted-release", destination))).kind)
        .toBe("MutatedSettled");
      ...
      const result = await createExportAgentPluginsApplication({
        ...
        undoWriter: new FakeUndoWriter(),
        failpoints,
      }).execute(request(secondFixture.alpha.ref, "targeted-release", destination));
  ...
  it.each(["symlink", "hardlink"] as const)(
    ...
    async (substitution) => {
      ...
      expect((await initial.execute(request(initialFixture.complete.ref, "complete-set", destination))).kind)
        .toBe("MutatedSettled");
      ...
      const result = await createExportAgentPluginsApplication({
        ...
        undoWriter: new FakeUndoWriter(),
        failpoints,
      }).execute(request(nextFixture.complete.ref, "complete-set", destination));
  ...
  it.each(["nonempty", "symlink"] as const)(
    ...
    async (substitution) => {
      ...
      expect((await application.execute(request(initialFixture.complete.ref, "complete-set", destination))).kind)
        .toBe("MutatedSettled");
  ...
  it("rejects a stale ledger generation before applying the staged payload action", async () => {
    ...
    const initialApplication = createExportAgentPluginsApplication({
      artifactReader: artifacts,
      knownNativeHomesReader: new FakeKnownNativeHomesReader(knownHomes()),
      undoWriter: initialUndo,
    });
    expect((await initialApplication.execute(request(firstFixture.alpha.ref, "targeted-release", destination))).kind).toBe("MutatedSettled");
    const ledgerPath = join(destination, EXPORT_LEDGER_FILENAME);
    let raced = false;
    const failpoints: ExportFailpoints = {
      async hit(point) {
