      ...
      Effect.map((kernel): CodexNativeAgentProviderSession => {
          ...
          runCodexAppServerRequests(kernel, "app-server-inspect", [
            Object.freeze({
              ...
              params: Object.freeze({
                cwds: Object.freeze([]),
                marketplaceKinds: Object.freeze(["local"]),
              }),
            }),
            Object.freeze({
              method: "hooks/list",
              ...
              return plugins === undefined || hooks === undefined
                ? Effect.fail(
                    providerFailure(
                      "app-server-inspect",
                      "ProtocolFailed",
                      "Codex app server response set was incomplete"
                    )
                  )
              ...
              results[0] === undefined
                ? Effect.fail(
                    providerFailure(
                      "config-read",
                      "ProtocolFailed",
                      "Codex app server returned no configuration"
                    )
                  )
        ...
        const listMarketplaces: CodexNativeAgentProviderSession["listMarketplaces"] = () =>
          kernel
            ...
            .pipe(
              Effect.flatMap((result) => parseJsonObservation("codex", "marketplace-list", result))
            );

        const listPlugins: CodexNativeAgentProviderSession["listPlugins"] = () =>
          kernel
            .run("plugin-list", ["plugin", "list", "--json"])
        ...
        return Object.freeze({
          ...
          listMarketplaces,
          readMarketplace: (
            request: Parameters<CodexNativeAgentProviderSession["readMarketplace"]>[0]
          ) =>
          ...
          addMarketplace: (
            source: Parameters<CodexNativeAgentProviderSession["addMarketplace"]>[0]
          ) =>
            kernel
          ...
          removeMarketplace: (
            request: Parameters<CodexNativeAgentProviderSession["removeMarketplace"]>[0]
          ) =>
              ...
              Effect.flatMap((selector) =>
                listPlugins().pipe(
                  Effect.flatMap((observation) =>
                    selectInstalledPluginRoot(kernel, observation.json, selector)
                  )
                )
              ),
          ...
          addPlugin: (request: Parameters<CodexNativeAgentProviderSession["addPlugin"]>[0]) =>
            requirePluginSelector("codex", "plugin-install", request.selector).pipe(
              Effect.flatMap((canonicalSelector) =>
                kernel.run("plugin-install", ["plugin", "add", canonicalSelector, "--json"])
              )
            ),
          removePlugin: (request: Parameters<CodexNativeAgentProviderSession["removePlugin"]>[0]) =>
            requirePluginSelector("codex", "plugin-remove", request.selector).pipe(
              Effect.flatMap((canonicalSelector) =>
                kernel.run("plugin-remove", ["plugin", "remove", canonicalSelector, "--json"])
              )
            ),
          inspectAppServer,
          readConfiguration,
          setMarketplaceSource: (
            request: Parameters<CodexNativeAgentProviderSession["setMarketplaceSource"]>[0]
          ) =>
            Effect.gen(function* () {
              const canonicalIdentity = yield* requireMarketplaceIdentity(
                "codex",
                "config-write",
                request.identity
              );
              const canonicalSource = yield* kernel.requireCanonicalDirectory(
                "config-write",
                request.source
              );
              yield* setConfigurationValue(
                `marketplaces.${canonicalIdentity}.source`,
                canonicalSource
              );
            }),
          setPluginEnabled: (
            request: Parameters<CodexNativeAgentProviderSession["setPluginEnabled"]>[0]
          ) =>
            requirePluginSelector("codex", "config-write", request.selector).pipe(
              Effect.flatMap((canonicalSelector) =>
                setConfigurationValue(`plugins.${canonicalSelector}.enabled`, request.enabled)
              )
            ),
        });
      })
...
function selectMarketplaceRoot(
  ...
  for (const entry of json.marketplaces) {
    if (!isJsonRecord(entry) || typeof entry.name !== "string") {
      return Effect.fail(
        providerFailure("marketplace-read", "ProtocolFailed", "Codex marketplace entry is invalid")
      );
    }
    if (entry.name !== identity) continue;
    if (typeof entry.root !== "string") {
      return Effect.fail(
        providerFailure("marketplace-read", "ProtocolFailed", "Codex marketplace root is invalid")
      );
    }
...
function selectInstalledPluginRoot(
  ...
  for (const entry of json.installed) {
    if (!isJsonRecord(entry)) {
      return Effect.fail(
        providerFailure("plugin-read", "ProtocolFailed", "Codex installed plugin entry is invalid")
      );
    }
