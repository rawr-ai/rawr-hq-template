...
import {
  ...
  v8StepsForTier,
} from "../../../../src/service/modules/runs/model/policy/steps";
...
async function packetRequiredArtifacts(input: {
  ...
  const packet = JSON.parse(
    await fs.readFile(path.join(input.vaultRoot, input.job.packetPath), "utf8")
  ) as {
...
describe("hyperresearch-codex V8 runtime", () => {
  it("starts a versioned V8 run with query, scaffold, route, and CLI init audit", async () => {
    ...
    const result = await client.runs.startV8Run(
      {
        canonicalQuery:
          "Map Codex parity for Hyperresearch. Address serve(), step.run, Hooks, and MCP boundaries.",
        tier: "auto",
    ...
    await expect(
      fs.readFile(path.join(fixture.vaultRoot, "research", "scaffold.md"), "utf8")
  ...
  it("runs the light fixture route end-to-end with fresh step loads and final report snapshot", async () => {
    ...
    await expect(
      fs.readFile(path.join(fixture.vaultRoot, finalReportPath(advanced.ledger)), "utf8")
    ...
    expect(decomposition.namedTopics).toEqual(
      expect.arrayContaining(["serve()", "step.run", "Hooks", "MCP"])
    );
    expect(decomposition.atomicItems?.map((item) => item.text)).toEqual(
  ...
  it("blocks validation when the final report is wholesale rewritten after snapshot", async () => {
    ...
    const client = createClient(
      createClientOptions({ repoRoot: fixture.root, cli: new RecordingCli() })
    );
    const started = await client.runs.startV8Run(
      {
    ...
    expect(validation.blockingFindings).toContainEqual(
  ...
  it("runs the full fixture route across all 16 V8 steps", async () => {
    ...
    expect(
      advanced.ledger.agentJobs?.map((job) => job.status).every((status) => status === "complete")
    ).toBe(true);
    await expect(
      fs.readFile(
        path.join(fixture.vaultRoot, "research", "critic-findings-dialectic.json"),
        "utf8"
      )
    ).resolves.toContain("12-critics");
    await expect(
      fs.readFile(path.join(fixture.vaultRoot, "research", "patch-log.json"), "utf8")
  ...
  it("stops in codex mode for agent packets and resumes after outputs exist", async () => {
    ...
    await expect(
      packetRequiredArtifacts({ vaultRoot: fixture.vaultRoot, job: awaiting.pendingAgentJobs[0]! })
    ).resolves.toEqual(["research/temp/search-plan.md", "research/temp/scored-urls.md"]);
    await expect(
      packetRequiredArtifacts({ vaultRoot: fixture.vaultRoot, job: awaiting.pendingAgentJobs[1]! })
    ).resolves.toEqual(["research/temp/source-capture-log.md", "research/temp/claims-width.json"]);

    const requiredArtifacts = requiredArtifactsFor("02-width-sweep", awaiting.ledger.vaultTag);
    await Promise.all(
      awaiting.pendingAgentJobs.map(async (job) =>
        writeAgentOutput({
          vaultRoot: fixture.vaultRoot,
          job,
          artifactPaths: await packetRequiredArtifacts({ vaultRoot: fixture.vaultRoot, job }),
          sourceUrls: ["https://www.python.org/about/", "https://www.python.org/downloads/"],
        })
      )
    );
    ...
    expect(resumed.ledger.steps.find((step) => step.id === "02-width-sweep")?.status).toBe(
      "complete"
    );
    expect(resumed.ledger.steps.find((step) => step.id === "02-width-sweep")?.artifacts).toEqual(
      requiredArtifacts
    );
    expect(resumed.ledger.sourceCaptures).toHaveLength(2);
    expect(
      resumed.ledger.sourceCaptures.find(
        (capture) => capture.url === "https://www.python.org/about/"
      )
    ).toEqual(
      expect.objectContaining({
    ...
    expect(resumed.ledger.resumes).toContainEqual(
      expect.objectContaining({
  ...
  it("keeps packet fan-in atomic when agent outputs arrive over multiple advances", async () => {
    ...
    expect(
      stillAwaiting.ledger.agentJobs
        .filter((job) => job.stepId === "02-width-sweep")
        .map((job) => job.status)
    ...
    await writeAgentOutput({
      vaultRoot: fixture.vaultRoot,
      job: secondJob,
      artifactPaths: await packetRequiredArtifacts({
        vaultRoot: fixture.vaultRoot,
        job: secondJob,
      }),
      sourceUrls: ["https://www.python.org/about/"],
    ...
    expect(completed.ledger.steps.find((step) => step.id === "02-width-sweep")?.status).toBe(
      "complete"
    );
    expect(completed.ledger.steps.find((step) => step.id === "02-width-sweep")?.artifacts).toEqual(
      requiredArtifactsFor("02-width-sweep", completed.ledger.vaultTag, "light")
    );
  ...
  it("completes a logical packet job through a ledgered replacement attempt", async () => {
    ...
    expect(
      accepted?.attempts?.find((attempt) => attempt.attemptId === `${replacementJob.id}-a1`)?.status
    ).toBe("non_clean");
    expect(
      accepted?.attempts?.find((attempt) => attempt.attemptId === `${replacementJob.id}-a2`)?.status
    ).toBe("accepted");
  ...
  it("reports accepted packet output hash conflicts during validation", async () => {
    ...
    await fs.writeFile(
      path.join(fixture.vaultRoot, accepted.acceptedOutputPath),
      '{"conflict":true}\n',
      "utf8"
    );
    ...
    expect(
      validation.blockingFindings.some((finding) => finding.code === "agent-output-conflict")
    ).toBe(true);
  });
  ...
  it("blocks packet-mode completion when agent outputs omit required artifact writes", async () => {
    ...
    await Promise.all(
      awaiting.pendingAgentJobs.map((job) =>
        writeAgentOutput({
          vaultRoot: fixture.vaultRoot,
          job,
          sourceUrls: ["https://www.python.org/about/"],
    ...
    expect(blocked.status).toBe("blocked");
    expect(blocked.ledger.steps.find((step) => step.id === "02-width-sweep")?.failure).toContain(
      "did not declare required artifact"
    );
  });
  ...
  it("snapshots an agent-written final report in packet mode", async () => {
    ...
    await Promise.all(
      awaitingWidth.pendingAgentJobs.map((job, index) =>
        writeAgentOutput({
          vaultRoot: fixture.vaultRoot,
          job,
          artifactPaths:
            index === 0
              ? requiredArtifactsFor("02-width-sweep", awaitingWidth.ledger.vaultTag, "light")
              : [],
          sourceUrls: ["https://www.python.org/about/"],
        })
      )
    ...
    expect(awaitingDraft.pendingAgentJobs.map((job) => job.role)).toEqual([
      "hyperresearch-draft-orchestrator",
    ]);
    await writeAgentOutput({
      vaultRoot: fixture.vaultRoot,
      job: awaitingDraft.pendingAgentJobs[0]!,
      artifactPaths: requiredArtifactsFor(
        "10-triple-draft",
        awaitingDraft.ledger.vaultTag,
        "light"
      ),
      sourceUrls: ["https://www.python.org/about/"],
    });
    ...
    expect(resumed.ledger.steps.find((step) => step.id === "10-triple-draft")?.status).toBe(
      "complete"
    );
    expect(resumed.ledger.reportSnapshots).toHaveLength(1);
    expect(resumed.ledger.patchGuard.snapshotPath).toBe(
      `research/notes/final_report_${resumed.ledger.vaultTag}.md`
    );
  });
  ...
  it("blocks failed agent outputs and does not complete the step on later advance calls", async () => {
    ...
    expect(blocked.status).toBe("blocked");
    expect(blocked.ledger.steps.find((step) => step.id === "02-width-sweep")?.status).toBe(
      "blocked"
    );
    ...
    expect(stillBlocked.ledger.steps.find((step) => step.id === "02-width-sweep")?.status).toBe(
      "blocked"
    );
  });
  ...
  it("rejects mismatched start/resume inputs for an existing V8 ledger", async () => {
    ...
    await expect(
      client.runs.startV8Run(
        ...
        invocation("v8-mismatch-second")
      )
    ).rejects.toThrow("mismatched canonicalQuery");
  });

  it("blocks the active step when a required fixture CLI operation fails", async () => {
    const fixture = await makeV8Fixture();
    const startClient = createClient(
      createClientOptions({ repoRoot: fixture.root, cli: new RecordingCli() })
    );
    ...
    expect(result.ledger.steps.find((step) => step.id === "02-width-sweep")?.status).toBe(
      "blocked"
    );
  ...
  it("blocks packet-mode completion when a required CLI operation fails after agent outputs", async () => {
    ...
    expect(blocked.integrity).toContainEqual(
      expect.objectContaining({
        severity: "blocking",
        code: "failed-cli-call",
      })
