...
export interface ReadOnlyGitBackend {
  readonly inspect: (workspacePath: string, canonicalRef: string) => Promise<unknown>;
...
export function createReadOnlyGitAdapter(backend: ReadOnlyGitBackend): ExactGitReader {
  const reader: ExactGitReader = {
    inspect: async (locator, canonicalRef) => {
      ...
      try {
        return parseInspection(await backend.inspect(locator.workspacePath, canonicalRef), locator, canonicalRef);
      } catch (error) {
