...
assertCondition(
  pkg.name === "@rawr/chatgpt-corpus",
  `expected package name @rawr/chatgpt-corpus, got ${pkg.name}`
);
assertCondition(
  (pkg.nx?.tags ?? []).includes("type:service"),
  "chatgpt-corpus must be tagged as a service"
);
assertCondition(
  (pkg.nx?.tags ?? []).includes("role:servicepackage"),
  "chatgpt-corpus must be tagged as a servicepackage"
);
assertCondition(
  pkg.scripts?.structural ===
    "bun ../../scripts/phase-03/run-structural-suite.mjs --project @rawr/chatgpt-corpus",
  "chatgpt-corpus structural script drifted"
);
...
for (const key of ["workspace", "sourceMaterials", "corpusArtifacts"]) {
  assertCondition(
    contractSource.includes(`  ${key},`) || contractSource.includes(`\n  ${key},`),
    `root contract is missing ${key}`
  );
  assertCondition(
    routerSource.includes(`  ${key},`) || routerSource.includes(`\n  ${key},`),
    `root router is missing ${key}`
  );
}

assertCondition(
  artifactsContract.includes("CORPUS_ARTIFACT_VALIDATION_FAILED"),
  "corpus-artifacts error must be defined at the contract boundary"
);
assertCondition(
  !artifactsRouter.includes("rethrow"),
  "corpus-artifacts router must not wrap and rethrow errors"
);
assertCondition(
  !artifactsRouter.includes("buildArtifactsFromSnapshot"),
  "corpus-artifacts router must keep build flow in procedures"
);
