...
import { resolveSourcePlugin } from "../src/resolve-source-plugin";

const tempDirs: string[] = [];

async function makeWorkspace(): Promise<{ root: string; pluginDir: string }> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "agent-config-sync-resolve-"));
  tempDirs.push(root);
...
describe("resolveSourcePlugin", () => {
  ...
  it("throws when missing", async () => {
    ...
    await expect(resolveSourcePlugin("missing-plugin", root)).rejects.toThrow(
