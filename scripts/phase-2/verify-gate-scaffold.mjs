...
const expectedRootScripts = {
  ...
  "phase-2:gate:u00:server-role-runtime-path":
...
assertCondition(
  (nxConfig.namedInputs?.architectureGlobals ?? []).includes(
    "{workspaceRoot}/scripts/phase-2/**/*.mjs"
  ),
...
for (const requiredSnippet of [
  ...
  assertCondition(
    structuralRunnerSource.includes(requiredSnippet),
    "scripts/phase-03/run-structural-suite.mjs must wire the M2 U00 current-findings suites."
  );
}

await runCommand(
  "bun run phase-2:gate:u00:no-legacy-cutover -- --allow-findings",
  "phase-2 no-legacy-cutover scaffold"
);
await runCommand(
  "bun run phase-2:gate:u00:server-role-runtime-path -- --allow-findings",
  "phase-2 server-role-runtime-path scaffold"
);
await runCommand(
  "bun run phase-2:gate:u00:runtime-public-seams -- --allow-findings",
  "phase-2 runtime-public-seams scaffold"
);
