...
const USAGE = "usage: cli-build-production.ts install [options] | activate <controller-digest> [options]";
...
export async function parseProductionControllerCliOptions(
  ...
  if (operation === "activate" && (controllerDigest === undefined || controllerDigest.startsWith("--"))) {
  ...
  for (let index = operation === "activate" ? 2 : 1; index < argv.length; index += 1) {
    ...
    else if (argument.startsWith("--bun-binary=")) bunBinary = argument.slice("--bun-binary=".length);
    else if (argument === "--global-bin-dir") globalBinDir = valueAfter(argv, index++, argument);
    else if (argument.startsWith("--global-bin-dir=")) globalBinDir = argument.slice("--global-bin-dir=".length);
  ...
  dataRoot ??= process.env.RAWR_DATA_DIR
  ...
  if (operation === "activate" && argv.some((argument) => argument === "--bun-binary" || argument.startsWith("--bun-binary="))) {
  ...
  if (globalBinDir === undefined) {
    ...
    globalBinDir = result.stdout.trim() || (operatorHome ? join(operatorHome, ".bun", "bin") : null);
  }
  ...
  if (controllerDigest === undefined) throw new Error("activate requires an existing controller digest");
...
export function isProductionControllerResultHealthy(
  ...
  if (
    ...
    && (
      result.release.durability === "unconfirmed"
      || result.release.cleanup === "failed"
    )
  ) return false;
  if (result.launcher.kind === "drifted" || result.launcher.durability === "unconfirmed") return false;
...
export function formatProductionControllerResult(
  result: ProductionControllerCliResult,
): string {
  const releaseSettlement = "durability" in result.release
    ...
    : "";
  const launcherPostCommitError = "postCommitError" in result.launcher
    ? result.launcher.postCommitError
    : undefined;
  ...
  const aliasSettlement = result.globalAlias === null
    ? ""
...
async function main(): Promise<void> {
  ...
  const result = options.operation === "install"
