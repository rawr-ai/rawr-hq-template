...
This design implements C1 only. Its governing order is the landed repository-separation amendment, the accepted normative proposal, the workstream's C1/B01-B03/B32 and I01-I02/I08/I17-I18 rows, then this execution record. Current source is migration evidence, not target authority.
...
- Materialize one self-contained Template controller release whose launcher and complete official command-module closure have one manifest and digest identity.
...
The builder uses the explicit member set and Nx graph to stage only the CLI, official modules, their transitive runtime project roots, exact dependency lock data, built command output, and one release launcher. That launcher is a self-contained single-file bundle of the pure `@rawr/controller-release` verifier and minimal entry composition, not a copied source implementation. Before complete envelope/payload verification it may import only bundled-runtime built-ins; every pure-package and bundler helper byte is inlined into the selector-hashed launcher. It must not snapshot the whole repository. After frozen installation, the builder enumerates every runtime file and symlink below the release payload root. Each manifest row binds relative path, entry kind, mode, byte digest for files, and relative target for links. Links must resolve inside the same release payload. No mutable shared dependency store is permitted.

The digest-free canonical `ControllerPayloadManifest` contains:

- schema version;
- audit-only Template source revision;
- platform, architecture, and exact bundled Bun path, byte digest, version, and revision for bootstrap and manager import-sandbox semantics;
- target-specific bootstrap-trampoline identity: protocol, platform, architecture, stable install-relative path, and byte digest;
- launcher relative path and digest;
...
### 3. A target-specific pre-JavaScript trampoline selects one stable controller

The controller data root resolves from `RAWR_DATA_DIR`, otherwise the platform XDG data convention. Releases live at `controller/releases/<controller-digest>/`. Selection is one bounded, canonical binary `ControllerSelectorV1` record at `controller/current.v1`, not a symlink, JSON document, shell fragment, or executable file. The pure selector schema and encoder live in `packages/controller-release`; the target-native reader must pass the same golden byte vectors and reject unknown versions, unknown or trailing fields, noncanonical encodings, unsafe relative paths, and size overflow. The record binds the controller digest; trampoline protocol/platform/architecture; and release-relative path plus byte digest for the bundled Bun runtime and launcher. The activation adapter atomically replaces the record only after the candidate release, installed trampoline, envelope, complete payload, dependency closure, command surface, and clean start pass. Re-selecting the same canonical bytes performs reads only and does not rewrite metadata.

The Bun-global `rawr` path targets one immutable non-shell native Template bootstrap trampoline under a target-specific stable-data path, never a checkout, shell script, JavaScript entrypoint, or selected-release file. Each supported target has a closed `BootstrapTrampolineIdentity { protocol, platform, architecture, installRelativePath, digest }`; the release envelope binds that exact identity, and install/activation verify the target and bytes before changing `current.v1`. The trampoline captures the operator cwd and required non-injection home/config locators as data; reads and parses `current.v1` exactly once; requires its compiled protocol/platform/architecture to match; derives the digest-qualified release root; canonicalizes the declared runtime and launcher paths inside that root; and hashes both before any JavaScript executes. It overwrites incoming bootstrap-capture keys, scrubs `BUN_OPTIONS`, `NODE_OPTIONS`, `NODE_PATH`, shell startup, Bun/Node preload/config, and workspace/module-resolution variables, and disables the transpiler cache.

No selected-release or user-controlled config/home path is a pre-entry input. The trampoline uses the platform null device as the explicit Bun `--config` value and temporary `HOME`/`XDG_CONFIG_HOME`, plus `--no-env-file`, `--no-install`, controller cwd, and the already-hashed digest-qualified launcher. A root `bunfig.toml`, purported bootstrap-home tree, `.env`, or preload inside the selected release is ignored rather than trusted. The trampoline contains no command parsing, source lookup, alternate runtime, repair, or fallback. A target, installed-byte, selector, runtime, launcher, containment, or null-bootstrap-contract mismatch blocks execution rather than choosing a second bootstrap implementation.

The loaded selector-hashed launcher is the complete pre-verification JavaScript trust root. It is a self-contained single-file verifier bundle whose only imports before full release acceptance are bundled-runtime built-ins; an architecture guard and runtime import sentinel reject any filesystem/package module load in that phase. The launcher independently verifies its own canonical path and bytes against the payload envelope and verifies that `process.execPath`, its byte digest, `Bun.version`, and full `Bun.revision` match the envelope's bundled runtime identity; mismatch fails before any other controller or external module loads. Its minimal entry composition derives the typed `{ digest, releaseRoot, envelope, runtime, operatorCwd }` invocation context only from the canonical realpath of its already-loaded `import.meta` module. The trampoline's digest-qualified entry argument is an untrusted claim that must canonicalize to that same module or fail. Only after complete envelope/runtime/path verification does the entry restore captured operator cwd and non-injection home/config values and dynamically import controller code from `releaseRoot`. Application code never rereads `current.v1`. Concurrent activation therefore starts an invocation entirely on A or entirely on B, including a switch between selector-record read and digest-qualified exec; it cannot mix their runtime, launcher, verifier, manifest, dependencies, or commands. Build and activation remain explicit Template scripts outside the supported RAWR command surface, so no supported command rewrites the implementation of its next invocation.

Activation failure leaves the prior selector byte-for-byte intact. Source-local `apps/cli/bin/run.js` execution without a materialized release context fails with `CONTROLLER_RELEASE_REQUIRED`; repository tests and development commands use an explicit temporary controller fixture.

**Rejected:** a Bun shebang or direct Bun-global link to JavaScript, ambient cwd/global bunfig, inherited preload/env configuration, `global-rawr-owner-path`, checkout transfer, post-checkout relinking, cwd fallback, and personal-source delegation. They execute before or preserve a second controller identity.

### 4. Startup never gives raw user registry entries directly to Oclif

The root bootstrap reads and verifies the selected controller before constructing Oclif configuration. It loads core controller members from the release manifest and sets Oclif's automatic `userPlugins` loading off. A Template-owned external-extension adapter reads the native Oclif registry as data, validates each candidate's package and static command manifest without importing its command or hook modules, and injects only accepted entries into the runtime configuration.
...
| target-specific bootstrap trampoline bytes/protocol | fixed Template installer artifact under its target-specific stable-data path | selector read and digest-qualified exec only | controller command, worktree hook, selected release, ambient Bun config |
| bundled runtime bytes and contract | selected controller release plus repository Bun pin | native trampoline, launcher, native-manager child | ambient Bun locator/version, candidate runtime, worktree toolchain |
...
- **Bun can execute cwd/global or selected-release preloads before an entry module** -> never launch JavaScript directly or accept release-controlled bootstrap config; use the target-specific digest-verified trampoline with platform-null config/HOME/XDG, disabled env/auto-install/cache, and hostile-cwd plus hostile-release sentinels.
...
3. Install the fixed pre-JavaScript trampoline, convert launcher/global scripts to materialized-release activation with hostile-config neutralization, and remove checkout-owner/auto-refresh authority.
