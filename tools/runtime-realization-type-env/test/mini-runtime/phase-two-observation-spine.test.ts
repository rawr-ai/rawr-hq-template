...
import { createServerAdapterCallbackPayload } from "../../src/mini-runtime/adapters/server";
...
import type { RuntimeOrpcServerResponse } from "../../src/mini-runtime/adapters/orpc-server";
import type { RuntimeInngestAsyncStepResponse } from "../../src/mini-runtime/adapters/inngest-async";
...
describe("phase two telemetry HyperDX catalog observation", () => {
  test("projects provider server async and catalog records into redacted OTLP and control-plane evidence", async () => {
    ...
    const providerResult = await executeMiniBootgraph({
