...
- Previous workstream next packet:
  `workstream-2026-04-30-phase-one-provider-effect-plan-bootgraph-provisioning-lowering.md`.
...
- Provider acquire/release lowering earned contained `simulation-proof` through
  RAWR-owned Oracle bootgraph/provisioning modules with real Effect execution.
...
- Promote `audit.p2.runtime-profile-config-redaction` from `todo` only to
  contained `simulation-proof` if the oracle/provisioning path validates
...
- Treating simulation proof as Lab-Production Proof.
...
- Keep source selection lab-local: module id config overrides provider id config,
  ...
  precedence.
- Validate config in the Oracle bootgraph module `start(...)` before
...
| Provider diagnostics/config snapshots are redacted. | observation | canonical spec, diagnostic, manifest todo | target `simulation-proof` | raw secret sentinels or live handles in trace/catalog fail tests | focused provider diagnostics/config test, oracle | not telemetry export or catalog persistence |
...
| --- | --- | --- | --- |
| Existing provider config path passed raw fixture config directly into provider build. | `src/oracle/provider-lowering.ts` previous `configFor(...)` use. | Replaced with lab-local source selection plus schema validation before provider build/acquire. | high |
...
- `systems/runtime-spine-evidence-map.md`, `current-lab-state.md`, `ref-runtime-realization-research-program.md`,
...
- Oracle: passed by focused provider-provisioning test and the full
  oracle target.
...
- `phases/phase-one/workstreams/workstream-2026-04-30-phase-one-provider-diagnostics-runtime-profile-config-redaction.md`
- `src/oracle/provider-lowering.ts`
