...
import { listRecentSnippetsFull } from "./index-db";
import type { JournalSearchRow } from "./store";
import type { SqliteDatabase } from "./sqlite";

type SemanticProvider = "openai" | "voyage";
type SemanticConfig = {
...
async function ensureSnippetEmbedding(
  ...
  if (row && row.provider === input.provider && row.model === input.model && row.contentHash === contentHash && row.vector) {
