...
4. The canonical future app/runtime architecture is defined by `docs/projects/rawr-final-architecture-migration/resources/spec/RAWR_Canonical_Architecture_Spec.md` and `RAWR_Effect_Runtime_Realization_System_Canonical_Spec.md`.
...
The five C2-C3 packages were proto-modules, not five service domains. [SERVICE_TOPOLOGY.md](./SERVICE_TOPOLOGY.md) records the manifest delta, domain capability set, logical flow, exact module inventory, context lanes, runtime binding placement, and structural proof for their consolidation into one oRPC service before public activation.

The service follows `services/session-intelligence` and the canonical runtime realization specification. The repository has no admitted effect-oRPC provider for this lane, so this correction uses the existing `@rawr/hq-sdk` oRPC primitive and deliberately adds no Effect/runtime realization work.
...
| Gate | State | Evidence |
| --- | --- | --- |
| OpenSpec behavior and boundary record | in progress | proposal, design, service topology, delta specs, and task ledger in this changeset |
| one oRPC service consolidation | pending | exact module inventory, structural gate, typed client, and migrated C2-C3 behavior suites |
| qualified command implementation | pending | command discovery plus owner-local integration tests |
| mixed aggregate retirement | pending | semantic inventory guard plus repository scan |
| standing reviews | pending | TypeScript/refactor, architecture/authority, behavior/testing, structural quality |
| installed-controller acceptance | pending | absolute installed binary and disposable homes only |
