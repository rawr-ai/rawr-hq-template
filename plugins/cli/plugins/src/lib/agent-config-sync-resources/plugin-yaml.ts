...
const ImportsSchema = Type.Object(
  {
    toolkits: Type.Optional(
      Type.Union([Type.Literal("all"), Type.Array(Type.String({ minLength: 1 }))]),
    ),
  },
...
export async function readPluginYaml(pluginAbsPath: string): Promise<{
  ...
  if (!(await pathExists(filePath))) {
    return { path: null, config: null, warnings: [] };
  }
  ...
  try {
    ...
    return {
      ...
      config: null,
      warnings: [
        `Failed to parse plugin.yaml: ${error instanceof Error ? error.message : String(error)}`,
      ],
    };
  }

  if (!Value.Check(PluginYamlV1Schema, parsed)) {
    const warnings = [...Value.Errors(PluginYamlV1Schema, parsed)]
      .slice(0, 5)
      .map((error) => {
        const detail = error as TypeBoxErrorLike;
        const location = detail.path ?? detail.instancePath ?? detail.schemaPath ?? "(root)";
        return `${location}: ${error.message}`;
      });
    return {
      path: filePath,
      config: null,
      warnings: ["Invalid plugin.yaml (expected version: 1).", ...warnings],
    };
  }
