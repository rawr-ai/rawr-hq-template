...
export type VisibleFingerprint = string & {
  readonly [visibleFingerprintBrand]: "VisibleFingerprint";
};
...
export function verifyTargetReceipt(input: unknown): DeploymentResult<TargetReceipt> {
  ...
  if (!exactRecord(input, ["body", "receiptDigest", "schemaVersion"], "receipt", issues)) {
    return failure(
      firstIssue(issues, issue("INVALID_RECEIPT", "receipt", "Receipt envelope is invalid"))
    );
  }
  ...
  const digest = parseDigest(input.receiptDigest, "receipt.receiptDigest", "tr1_", issues) as
    | TargetReceiptDigest
    | undefined;
...
export function decodeTargetReceipt(bytes: unknown): DeploymentResult<TargetReceipt> {
  if (!(bytes instanceof Uint8Array) || bytes.byteLength > MAX_RECEIPT_BYTES) {
    return failure([
      issue("INVALID_RECEIPT", "receipt", "Receipt bytes must be bounded canonical JSON"),
    ]);
  }
  let input: unknown;
  try {
  ...
  return equalBytes(bytes, canonical)
    ...
    : failure([
        issue(
          "INVALID_RECEIPT",
          "receipt",
          "Receipt bytes are not the unique canonical representation"
        ),
      ]);
}
...
export function visibleFingerprint(members: readonly VerifiedMemberIdentity[]): VisibleFingerprint {
  return canonicalDigest(
    "vf1_",
    [...members].sort(compareMembers).map(memberValue)
  ) as VisibleFingerprint;
}
  ...
  switch (scope.kind) {
    case "targeted-test":
      return {
        kind: scope.kind,
        ...common,
        releases: scope.releases.map(releaseRefValue),
        evaluationProfile: scope.evaluationProfile,
      };
    case "complete-test":
      return {
        kind: scope.kind,
...
function parseBody(
  ...
  if (input.schemaVersion !== 1)
    nested.push(
      issue(
        "INVALID_RECEIPT",
        "receipt.body.schemaVersion",
        "Unsupported receipt body schema",
        "1",
        String(input.schemaVersion)
      )
    );
  const provider =
    input.provider === "codex" || input.provider === "claude" ? input.provider : undefined;
  if (provider === undefined)
    nested.push(
      issue(
        "INVALID_RECEIPT",
        "receipt.body.provider",
        "Receipt provider is unsupported",
        "codex|claude",
        String(input.provider)
      )
    );
  const targetDigest = parseDigest(
    input.targetDigest,
    "receipt.body.targetDigest",
    "pt1_",
    nested
  ) as ProviderTargetDigest | undefined;
  ...
  const managedMembers = parseManagedMembers(
    input.managedMembers,
    "receipt.body.managedMembers",
    nested
  );
  if (
    provider !== undefined &&
...
function parseMarketplaceState(
  ...
  if (
    !exactRecord(
      input,
      ["adapterProtocol", "marketplaceIdentity", "projectionDigest", "provider", "sourceDigest"],
      "receipt.body.marketplace",
      issues
    )
  ) {
  ...
  if (provider === undefined) {
    issues.push(
      issue(
        "INVALID_RECEIPT",
        "receipt.body.marketplace.provider",
        "Marketplace provider is unsupported",
        "codex|claude",
        String(input.provider)
      )
    );
  }
  const adapterProtocol = parseProtocol(
    input.adapterProtocol,
    issues,
    "receipt.body.marketplace.adapterProtocol"
  );
  const identity = parseContentAuthority(
    input.marketplaceIdentity,
    "receipt.body.marketplace.marketplaceIdentity"
  );
  if (!identity.ok)
    issues.push(
      ...identity.issues.map((entry) => issue("INVALID_RECEIPT", entry.path, entry.message))
    );
  ...
  return provider !== undefined &&
    ...
    ? Object.freeze({
        provider,
        adapterProtocol,
        marketplaceIdentity: identity.value,
        projectionDigest,
        sourceDigest,
      })
    : undefined;
...
function parseLineage(
  ...
  if (
    ...
    if (kind === "initial") {
      return exactRecord(input, ["kind"], "receipt.body.lineage", issues)
        ? Object.freeze({ kind })
        : undefined;
    }
    if (kind === "successor") {
      if (!exactRecord(input, ["kind", "priorReceiptDigest"], "receipt.body.lineage", issues))
        return undefined;
      const digest = parseDigest(
        input.priorReceiptDigest,
        "receipt.body.lineage.priorReceiptDigest",
        "tr1_",
        issues
      ) as TargetReceiptDigest | undefined;
      return digest === undefined ? undefined : Object.freeze({ kind, priorReceiptDigest: digest });
    }
  }
  issues.push(
    issue("INVALID_RECEIPT", "receipt.body.lineage", "Receipt lineage must be initial or successor")
  );
  return undefined;
}
...
function parseScope(
  ...
  if (
    ...
    issues.push(
      issue("INVALID_RECEIPT", "receipt.body.scope", "Receipt scope must be a closed object")
    );
    return undefined;
  }
  const kind = (input as { readonly kind?: unknown }).kind;
  ...
  if (
    ...
    !exactRecord(
      input,
      [...commonKeys, ...modeKeys].sort(compareCanonical),
      "receipt.body.scope",
      issues
    )
  ) {
    if (modeKeys.length === 0)
      issues.push(
        issue(
          "INVALID_RECEIPT",
          "receipt.body.scope.kind",
          "Receipt scope kind is unsupported",
          "targeted-test|complete-test",
          String(kind)
        )
      );
    return undefined;
  ...
  if (kind === "targeted-test") {
    ...
    return refs === undefined || profile === undefined
      ? undefined
      : Object.freeze({ kind, ...common, releases: refs, evaluationProfile: profile });
  }
  ...
  if (kind === "complete-test") {
    ...
    return profile === undefined
      ? undefined
      : Object.freeze({ kind, ...common, releaseSet: ref, evaluationProfile: profile });
  }
  return undefined;
}

function parseScopeCommon(
  input: Record<string, unknown>,
  issues: ProviderDeploymentIssue[]
): ReceiptScopeCommon | undefined {
  const requestDigest = parseDigest(
    input.requestDigest,
    "receipt.body.scope.requestDigest",
    "prq1_",
    issues
  ) as ProviderRequestDigest | undefined;
  const projectionDigest = parseDigest(
    input.projectionDigest,
    "receipt.body.scope.projectionDigest",
    "ap1_",
    issues
  ) as ProjectionDigest | undefined;
  const adapterProtocol = parseProtocol(input.adapterProtocol, issues);
  const capability = parseDigest(
    input.capabilityProfileDigest,
    "receipt.body.scope.capabilityProfileDigest",
    "cp1_",
    issues
  ) as CapabilityProfileDigest | undefined;
  const fingerprint = parseDigest(
    input.visibleFingerprint,
    "receipt.body.scope.visibleFingerprint",
    "vf1_",
    issues
  ) as VisibleFingerprint | undefined;
  const members = parseMembers(input.verifiedMembers, "receipt.body.scope.verifiedMembers", issues);
  return requestDigest !== undefined &&
    projectionDigest !== undefined &&
    adapterProtocol !== undefined &&
    ...
    ? {
        ...
        projectionDigest,
        adapterProtocol,
        capabilityProfileDigest: capability,
...
function parseMembers(
  ...
  path: string,
  issues: ProviderDeploymentIssue[]
): readonly VerifiedMemberIdentity[] | undefined {
  return parseMemberList(input, path, issues, false) as
    | readonly VerifiedMemberIdentity[]
    | undefined;
}

function parseManagedMembers(
  input: unknown,
  path: string,
  issues: ProviderDeploymentIssue[]
): readonly ManagedMemberClaim[] | undefined {
...
function parseMemberList(
  ...
  for (const [index, candidate] of raw.entries()) {
    ...
    if (!plugin.ok)
      issues.push(
        ...plugin.issues.map((entry) => issue("INVALID_PLUGIN_ID", entry.path, entry.message))
      );
    const identity = canonicalString(
      candidate.nativeIdentity,
      `${path}[${index}].nativeIdentity`,
      issues,
      { maxBytes: 512, pattern: IDENTITY_PATTERN }
    );
    const authority = parseArtifactAuthority(
      candidate.artifactAuthority,
      `${path}[${index}].artifactAuthority`,
      issues
    );
    const source = parseContentAuthority(
      candidate.providerSourceIdentity,
      `${path}[${index}].providerSourceIdentity`
    );
    if (!source.ok)
      issues.push(
        ...source.issues.map((entry) => issue("INVALID_RECEIPT", entry.path, entry.message))
      );
    ...
    const fingerprint = parseDigest(
      candidate.memberFingerprint,
      `${path}[${index}].memberFingerprint`,
      "pm1_",
      issues
    ) as ProviderMemberFingerprint | undefined;
    const sourceProjectionDigest = managed
      ? (parseDigest(
          candidate.sourceProjectionDigest,
          `${path}[${index}].sourceProjectionDigest`,
          "ap1_",
          issues
        ) as ProjectionDigest | undefined)
      : undefined;
  ...
  for (let index = 1; index < members.length; index += 1) {
    if (
      members[index - 1]?.pluginId === members[index]?.pluginId ||
      members[index - 1]?.nativeIdentity === members[index]?.nativeIdentity
    ) {
      issues.push(
        issue(
          "INVALID_RECEIPT",
          path,
          "Receipt members must have unique plugin and native identities"
        )
      );
    }
  }
...
function parseArtifactAuthority(
  ...
  if (!exactRecord(input, ["contentAuthority", "protocol", "sourceCommit"], path, issues))
    return undefined;
  ...
  if (!authority.ok)
    issues.push(
      ...authority.issues.map((entry) => issue("INVALID_RECEIPT", entry.path, entry.message))
    );
  const commit = parseGitCommitId(input.sourceCommit, `${path}.sourceCommit`);
  if (!commit.ok)
    issues.push(
      ...commit.issues.map((entry) => issue("INVALID_RECEIPT", entry.path, entry.message))
    );
  return input.protocol === PROVIDER_ARTIFACT_AUTHORITY_PROTOCOL && authority.ok && commit.ok
    ? Object.freeze({
        protocol: input.protocol,
        contentAuthority: authority.value,
        sourceCommit: commit.value,
      })
    : undefined;
}

function parseReleaseRefs(
  input: unknown,
  path: string,
  issues: ProviderDeploymentIssue[]
): readonly ReleaseArtifactRef[] | undefined {
  const raw = boundedArray(input, path, MAX_MEMBERS, issues);
  if (raw === undefined) return undefined;
  const refs: ReleaseArtifactRef[] = [];
  for (const candidate of raw) {
    const parsed = parseArtifactRef(candidate);
    if (!parsed.ok || parsed.value.kind !== "release")
      issues.push(issue("INVALID_RECEIPT", path, "Targeted scope requires release refs"));
...
function parseSetRef(
  input: unknown,
  issues: ProviderDeploymentIssue[]
): CompleteSetArtifactRef | undefined {
  const parsed = parseArtifactRef(input);
  if (!parsed.ok || parsed.value.kind !== "complete-set") {
    issues.push(
      issue("INVALID_RECEIPT", "receipt.body.scope.releaseSet", "Scope requires a complete-set ref")
    );
    return undefined;
  }
  return parsed.value;
...
function parseEvaluation(
  ...
  return canonicalString(input, "receipt.body.scope.evaluationProfile", issues, {
    maxBytes: 256,
    pattern: EVALUATION_PATTERN,
    code: "INVALID_EVALUATION_PROFILE",
  }) as EvaluationProfile | undefined;
}
...
function parseProtocol(
  ...
  return canonicalString(input, path, issues, {
    maxBytes: 256,
    pattern: PROTOCOL_PATTERN,
    code: "INVALID_PROTOCOL",
  }) as AdapterProtocol | undefined;
}
...
function freezeBody(body: TargetReceiptBody): TargetReceiptBody {
  return Object.freeze({
    ...
    scope: Object.freeze({
      ...body.scope,
      verifiedMembers: freezeMembers(body.scope.verifiedMembers),
    }),
