...
import type { ArtifactReader, ArtifactReadResult } from "../src/artifact-reader";
import type { AtomicPackageOutput } from "../src/atomic-output";
import { COWORK_PACKAGE_FORMAT } from "../src/contract";
import { COWORK_V1_MAX_PAYLOAD_BYTES } from "../src/cowork-v1";
import { createNodeAtomicPackageOutput } from "../src/node-atomic-output";
import { createPackageAgentPluginApplication } from "../src/package-agent-plugin";
...
describe("package agent plugin application", () => {
  ...
  it("rejects unknown request fields before reading artifacts or touching output", async () => {
    ...
    const application = createPackageAgentPluginApplication({ artifactReader: reader, output });
