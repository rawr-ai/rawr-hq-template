...
assertCondition(
  lessons.includes("service truth -> plugin projection -> app composition -> host materialization"),
  "coordination lessons must preserve the wiring pattern"
);
assertCondition(
  lessons.includes("oRPC"),
  "coordination lessons must preserve the transport boundary lesson"
);
assertCondition(
  lessons.includes("What To Leave Behind"),
  "coordination lessons must record what stays archived"
);

console.log(
  "coordination remains archived, removed from the live tree, and preserved only as archive lessons."
);
