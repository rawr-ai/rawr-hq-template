import type { DiscoveredSessionFile, SessionListItem } from "../dto";
import { looksLikePath } from "./session-path";
...
export function hasMetadataFilters(filters: SessionFilters): boolean {
  return Boolean(
    filters.project ||
      filters.cwdContains ||
      filters.branch ||
...
export function matchesSessionFilters(session: SessionListItem, filters: SessionFilters): boolean {
  if (filters.project) {
