...
| HR-CODEX-028 | warning | open | Fresh synced-skill execution through `codex exec` is blocked by local Codex CLI/model compatibility, not by the Hyperresearch service. | `codex-cli 0.94.0-alpha.16` rejects `o3` for this account and rejects `gpt-5.5` / `gpt-5.3-codex` as requiring a newer CLI. Do not claim actual fresh-session Codex workflow proof until the CLI is upgraded or a supported model is identified. |
...
- `HR-CODEX-028`: open warning. Component, CLI, real-backend, and scoped sync gates are green; actual fresh Codex-session proof remains blocked by local CLI/model compatibility.
