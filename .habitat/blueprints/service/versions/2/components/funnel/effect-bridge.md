...
predicate service_v2_effect_bridge_uses_terminal($statements, $owner, $runner) {
  $statements <: contains `$owner.$runner` where {
    service_v2_effect_bridge_terminal_name(runner=$runner)
  }
}
...
or {
  or {
    ...
    service_v2_effect_bridge_is_production_source(),
    service_v2_effect_bridge_is_extension_source(source=$source),
    not { service_v2_effect_bridge_is_implementation_source() }
  },
  program(statements=$statements) where {
    ...
    or {
      and {
        ...
        $source <: r"^[\"']effect[\"']$",
        service_v2_effect_bridge_uses_terminal(
