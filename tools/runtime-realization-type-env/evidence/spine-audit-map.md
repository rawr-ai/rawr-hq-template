...
| Middle-spine derivation and compiler | Simulation proof | Explicit lab declarations, including explicit async descriptors, derive normalized graph artifacts, descriptor refs/table inputs, service binding plans, surface plans, dispatcher descriptor inventory without defaulting access operations, refs-only portable artifacts, compiled process plan, provider graph, registry input, harness placeholders, diagnostics, and scoped bootgraph input. This does not prove cold SDK extraction, async step membership, dispatcher access declaration, server route derivation, provider lowering, or real harness mounting. |
| Bootgraph/catalog/finalization | Simulation proof | Fake boot modules prove dependency ordering, startup rollback, reverse finalization that continues after finalizer failures, and redacted in-memory lifecycle/catalog records without deciding `ProviderEffectPlan` shape or acquiring real providers. |
