...
At M2 exit, confirm these Phase 3 entry conditions are met before declaring Phase 2 complete:

- [ ] The public SDK shapes in `packages/hq-sdk` are stable: `defineService`, `defineServicePackage`, `defineApp`, `startAppRole`, `defineServerApiPlugin`, `defineAsyncWorkflowPlugin`, `defineAsyncSchedulePlugin`, `bindService`.
- [ ] The plugin binding protocol is stable: plugins declare resources through `bind(...)`, the compiler lowers them, the substrate executes them.
- [ ] The manifest insertion pattern is stable: adding a plugin to `apps/hq/rawr.hq.ts` follows one repeatable shape.
- [ ] The runtime substrate is stable enough that generators can target it without accounting for internal churn.
- [ ] Open design questions from the migration plan (package topology, service-boundary provisioning bridge, config split, Effect dependency scope) are resolved or intentionally deferred with documented rationale.
