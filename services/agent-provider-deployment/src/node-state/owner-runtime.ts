...
export type { NativeMemberRestorationPort } from "../ports/provider";

export type PriorProjectionMemberSource = NativeMemberRestorationSource;

export type NativeMemberRestorationPorts = Readonly<Record<ProviderId, NativeMemberRestorationPort>>;
...
export function createNodeProviderOwnerRuntime(input: Readonly<{
  ...
  const readMember = async (
    ...
    await input.members[target.provider].readMember(target, nativeIdentity);

  const readMarketplace: ProviderOwnerRuntime["readMarketplace"] = async (target) =>
    await input.members[target.provider].readMarketplace(target);
  ...
  const restoreMarketplaceExact: ProviderOwnerRuntime["restoreMarketplaceExact"] = async ({
    ...
    const port = input.members[target.provider];
