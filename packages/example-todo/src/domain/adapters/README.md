...
- “Glue” code that depends on domain ports (from `domain/deps.ts`) but is not itself domain behavior.

## What does *not* belong here

- Domain semantics, invariants, and reusable domain types → `domain/shared/`
- Module-local repositories/services → `domain/modules/<name>/`
- Router composition and middleware chains → `domain/router.ts`
