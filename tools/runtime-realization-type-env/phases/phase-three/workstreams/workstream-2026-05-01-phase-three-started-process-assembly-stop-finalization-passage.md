...
architecture authority, final public API authority, Parent-Repo Migration
...
| Active claim | A selected contained Oracle app/runtime story can be started, invoked, observed, stopped, finalized, and post-stop rejected inside the Runtime Realization Lab. |
| Blocked claims | Parent-Repo Migration authorization, production host lifecycle, durable Inngest semantics, HyperDX product visibility, RuntimeCatalog persistence, native host telemetry/error mapping, final public API/DX law, and final Nx/generator ratchet. |
...
- A contained Oracle runtime passage can derive and compile its runtime spine,
  provision providers through the Oracle bootgraph, assemble one
...
- A focused oracle proof that adds a started/stopped passage oracle.
...
- No product HyperDX, durable Inngest, Elysia production host, RuntimeCatalog
  persistence, or Parent-Repo Migration claim is possible here.
...
3. Write the focused proof slice inside the Runtime Realization Lab.
...
- Stop if the proof requires Parent-Repo Migration.
...
| Local telemetry mining | complete | Existing parent-app telemetry is host/runtime-owned OTel and active-span enrichment evidence. Oracle telemetry is separate: redacted in-memory records, deterministic OTLP-shaped payloads, injected fetch export, run-id correlation, and candidate-only packets. |
| Source/test surface | complete | Existing oracle source supports a focused Phase Three test: wrap `ProcessExecutionRuntime.invoke`, mount current oRPC/Inngest boundaries, stop harnesses, finalize providers/runtime, then assert post-stop boundaries reject before runtime delegation. |
...
- Add a focused Phase Three oracle test rather than broad helper
...
- Treat Inngest post-stop rejection in vendor-native terms. Installed Inngest
  ...
  The oracle Inngest boundary now classifies that StepError response as a
  failure observation even though the HTTP status is `206`.

Implementation output:

- Added
  `tools/runtime-realization-type-env/test/oracle/phase-three-started-process-assembly-stop-finalization-passage.test.ts`.
- Added
...
- Updated `proof-manifest.json`, `runtime-spine-verification-diagnostic.md`,
  `systems/runtime-spine-evidence-map.md`, `systems/effect-integration-map.md`, and
  `vendors/README.md` to record the earned Phase Three proof and the new
  residuals without promoting Lab-Production Proof.

Verification:

| Gate | Result |
| --- | --- |
| Focused test: `bun test tools/runtime-realization-type-env/test/oracle/phase-three-started-process-assembly-stop-finalization-passage.test.ts` | Pass after review repair: 1 test, 57 assertions. |
| `bunx nx run runtime-realization-type-env:oracle` | Pass after review repair inside full gate: 63 tests, 498 assertions. |
...
- The proof crosses the existing Runtime Realization Lab path only: derivation,
...
- It does not prove production host lifecycle, Parent-Repo Migration authorization,
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Native host failure envelope and error/status taxonomy across oRPC, Inngest, and future Elysia host paths. | Open residual | Child 2 proved one contained rejection oracle and patched StepError classification, but not a complete native failure/status law. | Phase Three live-passage semantics | Runtime spec plus vendor docs; `vendors/README.md` as evidence map. | A child explicitly opens native boundary observation/failure semantics. | A proof claim depends on interpreting protocol status, native error body, host log/telemetry status, or public error payload. | Native boundary observation and failure semantics ledger. | Phase Three DRA. |
...
- Start from runtime authority, this child report, the vendor integration
  reference, `vendors/README.md`, `TELEMETRY_DESIGN.md`,
...
- DRA accepts the recommended next child:
  `workstream-2026-05-01-phase-three-native-boundary-observation-and-failure-semantics-ledger.md`.
- No Parent-Repo Migration, product HyperDX, durable async, Elysia production
...
| --- | --- | --- | --- |
| Architecture / proof honesty | Boyle | No material proof-honesty findings; accept as contained `simulation-proof`. | Accept. Proof stays yellow/contained and not Lab-Production Proof. |
...
- No Lab-Production Proof, durable async, product observability, native host
