...
- `cold-resume-explicit-child-resume` has also run and now passes after the Codex runtime status-seeding fix. Treat that as the accepted `HR-CODEX-035` closure path for known child ids after parent resume. Replacement packet attempts are fallback hardening for child attempts that still classify non-clean. Evidence lives under `spec/evidence/20260503T213000Z-app-server-explicit-child-resume/`. See `NATIVE_CODEX_SURFACE_REVIEW.md`.
