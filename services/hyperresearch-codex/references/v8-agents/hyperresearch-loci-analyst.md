...
1. **Load the corpus.** Use `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch search "" --tag <corpus_tag> --json`
...
2. **Read breadth first.** For each note, read the title + summary + first
   ~400 chars (use `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch note show <id> -j` and truncate). Do NOT read
