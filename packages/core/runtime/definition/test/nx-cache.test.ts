...
const FIXTURE_PREFIX = "runtime-definition-nx-cache-";
...
test("restores an unchanged build and invalidates on project and root tool inputs", async () => {
  const fixtureRoot = await createFixture();
  const ownerRoot = path.join(fixtureRoot, definitionPath);
  ...
  const outputPath = path.join(ownerRoot, "dist/index.js");

  try {
    const originalConfig = await readFile(configPath, "utf8");
    const originalBunfig = await readFile(bunfigPath, "utf8");
    ...
    await rm(path.dirname(outputPath), { recursive: true, force: false });
    const packageInvalidated = await runBuild(fixtureRoot);
    expect(
      packageInvalidated,
      packageInvalidated.stderr || packageInvalidated.stdout
    ).toMatchObject({
      exitCode: 0,
    });
    expect(cacheHit(packageInvalidated)).toBe(false);
    expect(await readFile(outputPath)).toEqual(expectedOutput);

    await writeFile(bunfigPath, `${originalBunfig}\n# Nx cache input probe.\n`);
    await rm(path.dirname(outputPath), { recursive: true, force: false });
    const bunfigInvalidated = await runBuild(fixtureRoot);
    expect(bunfigInvalidated, bunfigInvalidated.stderr || bunfigInvalidated.stdout).toMatchObject({
      exitCode: 0,
    });
    expect(cacheHit(bunfigInvalidated)).toBe(false);
    expect(await readFile(outputPath)).toEqual(expectedOutput);
  } finally {
...
async function createFixture(): Promise<string> {
  ...
  try {
    for (const relativePath of [definitionPath, schemaPath]) {
...
async function writeNxConfig(fixtureRoot: string): Promise<void> {
  ...
  const config = {
    ...
    plugins: [],
    targetDefaults: {
      build: required(source.targetDefaults?.build, "targetDefaults.build"),
    },
  };
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-definition:build", "--outputStyle=static"],
      { cwd: fixtureRoot, env }
    );
    const stdout: string[] = [];
    const stderr: string[] = [];
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-definition:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-definition Nx fixture: ${root}`);
  }
