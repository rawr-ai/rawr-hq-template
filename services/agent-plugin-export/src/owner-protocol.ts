...
export async function classifyExportOwnerStaged(input: Readonly<{
  ...
  try {
    ...
    const live = await inspectLiveEntry(destination, action);
  ...
  try {
    ...
    for (const { action } of actions) {
      ...
      if (captured === undefined) {
        const identity = await captureDestination(action.canonicalDestination);
        captured = Object.freeze({ identity, ledger: await captureLedger(identity, action.layout) });
        destinations.set(action.canonicalDestination, captured);
      }
      assertPriorAuthority(action, captured.ledger);
      const live = await inspectLiveEntry(captured.identity, action);
...
async function inspectLiveEntry(
  ...
  try {
    const captured = await capturePath(destination, action.relativePath, maximumFileStateBytes(action));
