...
not architecture authority, runtime proof authority, or production migration
authorization.
...
- Preserve the Phase Two correction: Phase Two proved contained spine
  composition, not production readiness.
...
- Phase Two wording that implies production readiness.
...
- Rejected: any interpretation that Phase Two proves production readiness,
...
- Stop if the recommended slice requires production mutation.
...
- Phase Three should prove a contained live passage as an inspectable lab
  runtime story, not as production readiness.
...
| --- | --- | --- | --- |
| Phase Two proved contained spine composition, not production readiness. | `../proof-manifest.json`, `../runtime-spine-verification-diagnostic.md`, `2026-04-30-phase-two-closeout-phase-three-handoff.md`, `../handoffs/2026-05-01-post-phase-two-runtime-reframe.md`. | Phase Three proof language stays lab-contained. | High |
| The live-passage dimensions are lifecycle, provider/config/Effect, process runtime/invocation, adapter/native callbacks, server, async, observation, handoff/control-plane, and falsifiers. | `2026-05-01-phase-three-program-workstream.md`; manifest entries listed in current experiment; local source/tests. | Use these dimensions as the child-1 claim ledger. | High |
...
| Server boundary | Real `@orpc/server/fetch` path crosses a Fetch request into the mini server harness. | Include contained oRPC Fetch request as the server ingress for the passage. | `simulation-proof` | Elysia mount, OpenAPI, production HTTP, auth/logging remain fenced. |
| Async boundary | Real `inngest/bun` serve/function/step path crosses into the mini async harness. | Include contained Inngest function/step request as the async ingress for the passage. | `simulation-proof` | Scheduling, retry, replay, idempotency, run history remain fenced. |
...
| --- | --- | --- | --- |
| 1 | Started process assembly plus stop/finalization passage | Pulls derivation, compilation, provisioning, mounting, invocation, observation, and finalization into one inspectable runtime story without production mutation. | Accepted as next child. |
...
- A selected toy-contained app/runtime story can derive and compile its runtime
  spine, provision provider resources through the mini bootgraph, assemble one
...
- new focused mini-runtime test for the started process assembly passage;
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Production migration readiness | `out-of-scope` | Phase Three remains lab-contained. | Manifest `audit.p2.phase-two-program-closeout`; Phase Two closeout; Phase Three program. | A later production migration workstream explicitly opens production code/topology as accepted control state. | Any claim says Phase Three authorizes migration. | Post-Phase-Three migration planning | Phase Three DRA until production-migration DRA accepts handoff. | `migration-only` |
| Elysia and production HTTP host lifecycle | `xfail` | Current lab proves contained oRPC Fetch only. | `audit.p2.server-orpc-fetch-boundary`; `vendor-fidelity.md`. | Real Elysia mount/request lifecycle is installed and gated in lab or production. | Started-passage child needs Elysia wording. | Server host passage or production host workstream | Phase Three DRA; server-host child DRA if opened. | `lab/migration` |
...
| Final structure/Nx/generator ratchet | `out-of-scope` | Live-passage shape must be proven before ratcheting structure. | Phase Three program; focus log. | Phase Three closeout says structure is the next domino. | Closeout or child evidence changes sequence. | Later structure/Nx/generator phase | Phase Three DRA until structure/Nx phase opens. | `lab` |
...
- Mini-runtime: no runtime proof added by this child.
...
- Adversarial evidence honesty: passed by DRA synthesis; no lab/vendor claim is
  promoted into production readiness.
...
- `evidence/workstreams/2026-05-01-phase-three-live-runtime-passage-scope-and-claim-ledger.md`
