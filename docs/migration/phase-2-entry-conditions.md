...
The migration plan identifies four design questions that should be resolved before or during M2-U00:

1. **Package topology**: flat (`packages/runtime-substrate`) vs nested (`packages/runtime/substrate`). Affects workspace configs, import paths, and lint rules.
2. **Service-boundary provisioning bridge**: how `bindService(...)` connects runtime substrate Effect services → oRPC context lanes → plugin `bind(...)`.
3. **Config split boundary**: where runtime config ends and service config begins for `example-todo` and `hq-ops`.
4. **Effect dependency scope**: workspace dependency only, or peer dependency of bootgraph.

See the migration plan's "Open design questions" section for full context.
