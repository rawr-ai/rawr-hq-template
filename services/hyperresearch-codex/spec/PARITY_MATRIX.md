...
| Claude `PreToolUse` source reminder | Codex `PreToolUse` source-bypass guard | guarded-workaround | Fixture-proven guardrail in `spec/evidence/20260503T235332Z-codex-hooks-proof/`; evidence records Bash payload shape and direct block/allow behavior. Service source capture remains authoritative |
| Claude Task/subagent lifecycle observability | No Codex `SubagentStart`/`SubagentStop` equivalent proven | unsupported-explicit | Use child-session evidence contract plus packet/service validation instead; do not call this hook parity |
| Claude compaction/session-end lifecycle concerns | No Codex `PreCompact`/`SessionEnd` equivalent proven | unsupported-explicit | Use durable ledger and resume packet; do not claim hook parity |
| Final closure guard | Codex `Stop` guard | guarded-workaround | Fixture-proven guardrail in `spec/evidence/20260503T235332Z-codex-hooks-proof/`; runtime invocation was observed and direct fixture red/green decisions are preserved. Codex-native guardrail, not installed Claude Hyperresearch hook parity; service validation remains authoritative |
...
- RAWR compatibility matrix says Codex skills are native, Codex custom agents require adapter projection, hooks require fixture proof, and official plugin packaging remains distinct from direct local mirrors. `HOOKS_GUARDRAIL_PLAN.md` defines the concrete hook proof ladder.
- Core Hyperresearch hook guardrail proof exists under `services/hyperresearch-codex/spec/evidence/20260503T235332Z-codex-hooks-proof/`: `PreToolUse` runtime payload capture, source-bypass block, Hyperresearch-command allow, `Stop` runtime payload capture, and direct red/green ledger decisions. Managed plugin projection remains unclaimed.
...
- `NATIVE_CODEX_SURFACE_REVIEW.md` records the deep native-surface review: app-server is the preferred reproduction surface, direct cold resume reproduced the failure, later explicit `resume_agent` evidence shows runtime recovery is possible, TypeScript Codex SDK is not a pivot because it wraps `codex exec`, and hosted OpenAI SDKs are not the local Codex runtime. Hyperresearch service plus packet-orchestration parity closes through parent resume plus explicit child resume for known child ids; replacement attempts remain fallback hardening for non-clean child attempts, and bare automatic descendant rehydration remains unclaimed.
