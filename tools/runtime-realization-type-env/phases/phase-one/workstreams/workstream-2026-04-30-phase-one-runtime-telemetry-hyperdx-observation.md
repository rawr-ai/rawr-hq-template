...
- Any exporter added here must consume already-redacted oracle records and
...
- `workstream-2026-04-30-phase-one-semantic-runtime-documentation-harvest.md`
...
- In-memory oracle records are not persisted runtime catalog authority.
...
- oracle record redaction and boundary policy records.
...
- `bunx nx run runtime-realization-type-env:oracle --skip-nx-cache`
...
- Implementation seam reviewer: recommended a pure helper under
  `src/oracle/telemetry-export.ts`, index export, and focused tests only.
...
- Add a lab-only telemetry helper that projects already-redacted
  oracle records into a minimal OTLP trace payload.
...
- Added `src/oracle/telemetry-export.ts`.
  - `projectRuntimeEventsToTelemetryRecords(...)` projects process/provider
    event streams after the oracle redaction boundary.
...
- Added `test/oracle/telemetry-export.test.ts` for process runtime
...
- Focused telemetry test passed:
  `bun test tools/runtime-realization-type-env/test/oracle/telemetry-export.test.ts`.
...
- Leaf review covered containment, mechanical placement, type/negative surface,
  semantic comments, vendor/HyperDX fidelity, oracle behavior, and
...
| Existing oracle records are a suitable export source only after redaction. | Focused tests project process events, provider traces, and catalog records while rejecting sentinel secrets and live handles. | Build the helper around already-redacted records; do not export raw provider values or runtime access objects. | High |
...
| Durable async telemetry | `xfail` | Oracle async records are not Inngest durable run history. | Async residual manifest entries and canonical async sections. | Native async host semantics are implemented or explicitly decided. | Telemetry work needs durable retries/status/idempotency correlation. | Durable async or migration/control-plane workstream | spec |
...
- Containment: passed. All edits remain under
  ...
  wiring was added.
- Mechanical: passed after review. The helper is exported from the oracle
...
- Oracle: passed. Focused telemetry export tests passed, and the full
...
- Adversarial evidence honesty: passed. The report explicitly rejects Docker
  uptime, OTLP acceptance, and comments as Lab-Production Proof.
...
- `phases/phase-one/_archive/default-research-program-2026-04-30/ref-runtime-realization-research-program.md`
- `phases/phase-one/workstreams/workstream-2026-04-30-phase-one-runtime-telemetry-hyperdx-observation.md`

Verification run:

- `bun test tools/runtime-realization-type-env/test/oracle/telemetry-export.test.ts`: passed, 4 tests and 28 expects.
...
- `bunx nx run runtime-realization-type-env:oracle --skip-nx-cache`: passed, 50 tests and 244 expects.
