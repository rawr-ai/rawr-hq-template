...
export default class PluginsSyncSourcesRemove extends RawrCommand {
  ...
  async run() {
    ...
    next.sync.sources.paths = existing.filter((p: string) => p !== resolved);

    await writeGlobalConfig(next);

    const result = this.ok({ path: rawrGlobalConfigPath(), removed: resolved, sources: next.sync.sources.paths });
