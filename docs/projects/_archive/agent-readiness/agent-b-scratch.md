...
- 2026-02-06: Verified CLI command contracts from source:
  ...
  - `plugins/cli/plugins/src/commands/plugins/web/{list,enable,disable,status}.ts`
...
- 2026-02-06: Ran command validation in template repo:
  ...
  - `plugins web list|enable|status|disable --json` outputs stable machine-readable payloads.
...
- 2026-02-06: Ran final command-surface scan on both runbook copies to confirm:
  ...
  - Channel B references remain `rawr plugins web ...`
...
- Provide two explicit runbook tracks:
  ...
  - Channel B workspace runtime path (`rawr plugins web list/enable/disable/status`)
