...
2. **Search the vault for counter-evidence.** Use
   `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch search "<keyword>" --tag <vault_tag> -j` to find interim
   ...
   (`/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch note show <id> -j`).
