...
#### Scenario: Export ignores disposable source-fixture availability
- **WHEN** a verified artifact handle is exported after its generated disposable source fixture has been made unavailable
- **THEN** the exact same layout plan and bytes are produced from immutable artifacts
- **AND** no repository code, source fallback, or tree-equivalence check executes; the actual personal checkout and protected research locations are neither accessed nor mutated
