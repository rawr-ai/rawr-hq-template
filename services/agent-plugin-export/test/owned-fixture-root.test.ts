...
describe("export owned fixture cleanup", () => {
  it("rejects a symlink substitution and preserves its target", async () => {
    ...
    const target = await mkdtemp(join(dirname(fixture.path), "rawr-export-target-"));
    ...
    try {
      ...
      await expect(fixture.cleanup()).rejects.toThrow(/fixture-root substitution|recursive cleanup/u);
