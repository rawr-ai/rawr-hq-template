...
Phase 3 may only begin after all of the following are stable:

- the public SDK authoring shapes in `packages/hq-sdk` are settled: `defineService(...)`, `defineServicePackage(...)`, `defineApp(...)`, `defineServerApiPlugin(...)`, `defineAsyncWorkflowPlugin(...)`, `defineAsyncSchedulePlugin(...)`, `bindService(...)`
- the plugin binding protocol is settled: plugins declare resources through `bind(...)`, the runtime compiler lowers those declarations into the process plan, and the bootgraph and substrate execute the plan
- the manifest insertion protocol is settled: adding a plugin or service to `apps/hq/rawr.hq.ts` follows one repeatable pattern
- the runtime substrate is stable enough that generated code does not need to account for internal substrate evolution
- at least one server and one async proof slice run end-to-end through the canonical runtime shell

If these are not stable at Phase 2 exit, Phase 3 cannot produce trustworthy generators.
...
7. generated plugins must use the canonical `bind(...)` / `bindService(...)` protocol, not raw service construction
8. generated code must not emit raw Effect types; the Effect quarantine boundary applies to generated output
...
- canonical service shell (using `defineService(...)`, `defineServicePackage(...)`, and the canonical `service/base.ts` → `service/contract.ts` → `service/impl.ts` → `client.ts` structure)
- canonical family-nested service shell (same pattern, nested under a family namespace)
- canonical server/api plugin (using `defineServerApiPlugin(...)` and `bindService(...)` to derive route handlers from bound service clients)
- canonical async/workflow plugin (using `defineAsyncWorkflowPlugin(...)` and `bindService(...)`)
- canonical async/schedule plugin (using `defineAsyncSchedulePlugin(...)`)
- manifest insertion/update (adding the generated plugin to `apps/hq/rawr.hq.ts` in the correct role/surface position)
...
The templates must emit code that uses only the public SDK vocabulary. They must not emit raw Effect imports, raw Layer construction, raw `Context.Tag`, or any internal `packages/runtime/*` vocabulary. If a generated plugin needs process resources, it declares them through `bind(...)` and receives them from the runtime substrate at mount time.
...
- no raw Effect types in generated service, plugin, or app code (Effect quarantine)
- generated plugins use `bindService(...)` for service client construction, not direct imports
...
- generated plugins use `bindService(...)` correctly
- generated code contains no raw Effect imports (`Layer`, `Context.Tag`, `ManagedRuntime`, `Effect.Service`)
...
- the generated plugin runs through the canonical runtime shell without any bespoke runtime substrate modifications
...
- generated plugins use canonical `bind(...)` / `bindService(...)` protocol
- generated code does not reference runtime substrate internals
...
These are real questions that need resolution during Phase 2 execution planning. They are called out here so they are visible to the team before work starts, not discovered in the middle of implementation.

### 1. Runtime package topology: flat vs nested

The two authoritative specs (`RAWR_Canonical_Architecture_and_Runtime_Spec_Integrated_Final.md` and `RAWR_Effect_Runtime_Subsystem_Canonical_Spec.md`) describe the runtime subsystem packages with flat names:
...
This migration plan uses nested names under a `packages/runtime/` family:
...
The nested layout is better for monorepo organization (one workspace glob, shared config, clear family boundary). The flat layout is what the canonical specs describe. **This discrepancy must be resolved before Phase 2 execution.** It affects workspace configs, npm package names, import paths, and boundary lint rules.

The migration plan currently uses nested names as the working assumption. If the team decides on flat names, all Phase 2 docs, issue references, and Phase 3 generator targets must be updated accordingly.

### 2. Service-boundary provisioning bridge

The specs describe `bindService(...)` and `BoundaryCache` conceptually. The critical integration question is:

How does `bindService(...)` actually connect the runtime substrate's Effect-backed process resources into oRPC service-boundary context lanes (`deps`, `scope`, `config`)?

The runtime substrate provides resources as Effect services (e.g., `DbPool`, `RuntimeConfig`, `WorkspaceRoot`). Plugins call `bindService(...)` to get a bound service client. The service expects `deps`, `scope`, and `config` populated through its oRPC context-lane middleware.

The bridge between these two worlds must be pinned down during M2-U00 or M2-U01. Possible shapes:

- `bindService(...)` internally runs an Effect program that acquires the needed resources from the `ManagedRuntime` and passes them as the service's `deps`/`scope`/`config` lanes
- `BoundaryCache` memoizes the bound client per process so repeated calls return the same client instance
- the plugin's `bind(...)` callback receives a typed view of process/role resources and calls `bindService(...)` within that scope

The answer determines the boundary between "what plugin authors write" and "what the runtime substrate does automatically."

### 3. Config split boundary

The spec says runtime config is process identity, harness, observability, and infra connection config. Service config is business behavior config.

In practice, some config straddles both:

- `WorkspaceRoot` / `RepoRoot` is a runtime resource, but `hq-ops` uses it as a business input (`scope.repoRoot`)
- database connection config is runtime infrastructure, but service-specific table/schema config is business

The canonical flow is: runtime config → process resource → plugin `bind()` → service config lane. But the exact boundary between "runtime config that the substrate loads" and "service config that the plugin derives" needs to be made concrete for at least the `example-todo` and `hq-ops` proof slices.

### 4. Effect dependency scope

Does Effect enter as:

- a **workspace dependency** only (used inside `packages/runtime/*` and invisible to everything else)?
- or also a **peer dependency** of `packages/runtime/bootgraph` (since bootgraph's implementation uses Effect even though its public API does not)?

The quarantine rule says raw Effect types stay inside `packages/runtime/*`. But `packages/runtime/bootgraph` is PUBLIC and uses Effect internally. Its `package.json` must declare Effect somewhere. If bootgraph is under `packages/runtime/`, then Effect is naturally scoped to that family. If bootgraph is flat (`packages/bootgraph`), the scoping is less natural.

This question is related to design question 1 (flat vs nested topology) and should be resolved together.
