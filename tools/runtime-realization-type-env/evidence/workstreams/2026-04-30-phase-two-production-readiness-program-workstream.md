...
Latest active branch checkpoint: `codex/runtime-phase-two-provider-config-effect-spine`.
PR: `none`.
...
- Production `apps/*`, `packages/*`, `resources/*`, `services/*`,
...
- Phase Two may model production-shaped semantics inside the lab, but it must
  not mutate production packages or claim production migration readiness.
- "Production-readiness" in this artifact means stronger contained evidence for
  migration decisions, not proof that the production application is ready.
...
- Do not promote vendor proof or simulation proof into production migration
  readiness.
...
- The Nx project `runtime-realization-type-env` has focused and composed
  ...
  vendor-boundaries, mini-runtime, middle-spine, simulate, and gate.
...
- `vendor-proof`: real Effect `3.21.2`, TypeBox, native oRPC shape, and
  ...
  integration.
- `simulation-proof`: mini-runtime derivation, registry identity, real Effect
...
- Phase Two may strengthen `vendor-proof` and `simulation-proof` entries by
  ...
  new proof category unless `design-guardrails.md`, manifest/report tooling,
  and diagnostic wording are explicitly revised.
- No Phase Two result may claim production migration readiness without a later
...
| Cross-spine readiness | Separate contained proofs across middle-spine, provider, adapter, harness, telemetry, and observation. | Whether an integrated rehearsal composes the spine end to end without hidden production mutation or false green labels. |
...
- `proof-manifest.json`, `runtime-spine-verification-diagnostic.md`,
  `spine-audit-map.md`, `focus-log.md`.
...
- Define the representative lab-contained runtime spine scenario that Phase Two
  ...
  production code.
- Treat "production-readiness scenario" as shorthand for contained
  production-critical migration evidence, not production application readiness.
...
- A hard label rule: if Elysia is not installed and exercised through a real
  ...
  requires a corresponding `vendor-fidelity.md` update.
...
- Integrated contained proof and migration-decision evidence. Not production
  migration readiness.
...
- Containment: no production mutation, workspace promotion, root export drift,
...
- Vendor fidelity: real dependency behavior only where the claim depends on
  vendor behavior.
- Mini-runtime/live-boundary behavior: RAWR-owned delegation, no fake-host
...
- Migration derivability: whether the result reduces production migration risk
  without claiming production migration readiness.
...
- phase-three handoff into production migration sequencing.
