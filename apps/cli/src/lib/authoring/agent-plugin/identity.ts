...
export type CuratedAgentPluginIdentity = string & { readonly [curatedAgentPluginIdentityBrand]: true };
export type CuratedAgentPluginIdentityIssue = Readonly<{ path: "contentWorkspace" | "id"; message: string }>;
...
export function parseCuratedAgentPluginIdentity(
  ...
  return typeof input === "string" && input.length <= 64 && PLUGIN_ID_PATTERN.test(input)
    ...
    : Object.freeze({ path: "id", message: "Curated agent-plugin identity must be bounded lowercase kebab-case" });
}
