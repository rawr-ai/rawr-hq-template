...
Task 1.1 records its exact commit and canonical blob identities after the docs
checkpoint is created. The candidate reconciles the frozen
...
implementation remains closed until that exact parent lands on canonical
`main`.
...
| `findWorkspaceRoot` and `RAWR_*` lookup | no retained owner | Delete discovery by Rawr environment variables and bind an explicit Habitat host/workspace input at the CLI boundary. |
...
| `packages/hq-sdk` API/workflow declarations and composition | Habitat authoring | Move `defineApiPlugin*`, `composeApiPlugins`, trace forwarding, and server surface merge to `@habitat-ai/sdk/plugins/server`; move `defineWorkflowPlugin*`, `composeWorkflowPlugins`, and async surface merge to `@habitat-ai/sdk/plugins/async`, with every reader. |
...
| `packages/hq-sdk` service binding declarations and mechanics | split Habitat owners | Move cold `ProcessView`, `RoleView`, `ServiceBoundary`, and `ServiceBinding` to `runtime-definition` and the SDK service face; move live `BoundService`, `bindService`, and cache mechanics to `runtime-process-runtime`. |
...
| Mixed `packages/runtime-context` / `@rawr/runtime-context` | none as an aggregate | Move generic service context lanes to `@habitat-ai/sdk/service`, move execution-facing contracts to `@habitat-ai/sdk/execution` and `runtime-process-runtime`, delete the unused workflow seam, move every reader, then delete the package and project. |
| `packages/ui-sdk` / `@rawr/ui-sdk` | Habitat core web authoring | Move its framework-neutral mount contract to `@habitat-ai/sdk/plugins/web` with its readers, then delete the standalone package and project. |
...
| Standalone TypeBox adapter | Habitat core | Move into the exact private runtime schema owner at `packages/core/runtime/schema` and delete the standalone package/project. |
| `packages/bootgraph` reservation package | Habitat runtime | Replace with the private package-less Nx owner `packages/core/runtime/bootgraph`. |
...
| `services/session-intelligence` and Session Tools commands | Habitat platform | Retain under a qualified Habitat service and topic with owner-local acceptance. |
...
| `config show` and `config validate` | Habitat platform | Retain as explicit Habitat config inspection against the selected host/workspace binding. |
| `journal tail/search/show` | Habitat platform | Retain as a Habitat journal topic with its qualified reader. |
| `security check` and `security report` | Habitat platform | Retain as Habitat security inspection and reporting; they do not authorize the deleted posture packet. |
...
- Reopening the canonical ontology, app/runtime lifecycle, or service law.
...
commands, such as corpus or Hyperresearch commands, are accepted only by the
...
Before any private runtime project lands, independent Rawr separation and the
Habitat semantic sieve have passed exact-main gates. `RawrCommand`,
...
separation. The independent Rawr repository owns any later product app and may
exercise only the ChatGPT corpus and Hyperresearch domain closure admitted by
its owner-local OpenSpec. Product ids, providers, plugin membership, config,
...
repository before private runtime owners are implemented. Habitat realizes its
catalog, policy, agent-plugin lifecycle, config, journal, security,
development/repository, session-intelligence, generator, Nx, and CLI
...
1. Record the reviewed canonical candidate's exact commit and two canonical
   blob identities in task 1.1, land that exact parent on canonical `main`, and
   keep this change directly above that exact revision.
...
3. In a clean worktree of existing Rawr repository `rawr-ai/rawr` at canonical
   ...
   OpenSpec, disposition its legacy Turborepo-era contents, and establish the Nx
   consumer shape. Use native `nx import` from the frozen Habitat source/ref
   with explicit source and destination directories to import only the proven
   ChatGPT corpus and Hyperresearch service/topic projects with filtered Git
   history. Import cohesive projects together where possible and otherwise
   ...
   relationship.
4. In Habitat, move every retained platform capability to its exact service,
   resource/provider, CLI, SDK, tool, mounting, observation, or owner-local
   fixture destination. Delete all product app/source, compatibility, example,
   and synthetic production owners and land the product-separation boundary.
   Verify Marketplace remains an independent content repository.
5. Finish the non-runtime Habitat cleanup: delete `RawrCommand` and
   `RawrResult`, consolidate one Habitat command contract, replace Rawr
   environment discovery with explicit Habitat host/workspace binding, classify
   existing SDK exports without empty shells, land the qualified telemetry
   owner, remove the mixed core identity, and move the terminal SDK atomically
   to `packages/core/sdk`. Pass Habitat exact-main with all Rawr-named, dead, and
   mixed predecessors absent. Write generic packets as design material, then
   activate each closed kind law only with its first conforming owner. The first
   private runtime implementation opens only after this gate.
6. Build and seal one artifact handoff at a time: normalized derivation,
   ...
   observation.
7. Realize the Habitat self-host through the Oclif vertical. Run `habitat
   ...
   mounting, and observation. Realize config, journal, security,
   development/repository, session-intelligence, native plugin, and generator
   capabilities through the same platform-owned path.
8. Add Elysia/oRPC, Inngest, and web adapter/harness realizations only with
   indispensable owner-local Habitat conformance fixtures. No fixture becomes a
   production app, service, plugin, or package.
9. Audit unreachable residue and pass the complete local Habitat gate. Publish
...
10. Land the complete Habitat stack on canonical `main`, pass exact-main CI,
    and use `nx release` to tag, publish, and registry-smoke
    ...
    package cohort.
11. The Rawr owner-local OpenSpec then upgrades the already installed consumer
    ...
    Genuine corpus and Hyperresearch commands receive Rawr-native runtime
    acceptance there; no source returns to Habitat.
12. Re-run exact-main gates in Habitat and Rawr, verify Marketplace remains
