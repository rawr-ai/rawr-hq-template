...
- AXIS_03: Split posture (`/api/workflows/*` caller-facing vs `/api/inngest` runtime ingress).
- AXIS_05: Durable run status/timeline recording path.
- AXIS_07: Host composition with one runtime-owned Inngest bundle.
- AXIS_08: Workflow trigger router and durable function separation.
...
Baseline note: all other core package and API-boundary files remain as shown in `e2e-01-basic-package-api.md`.
...
6. Snippet default keeps procedure/contract I/O schemas inline at `.input/.output`; if extraction is needed for shared or very large shapes, prefer paired schema objects (`const XSchema = { input, output }`) with `.input(std(XSchema.input))` + `.output(std(XSchema.output))`.
...
Next step: continue with `e2e-03-microfrontend-integration.md` to add a first-party browser caller path on `/rpc` while preserving this same split posture for external/public workflow routes and runtime ingress.
