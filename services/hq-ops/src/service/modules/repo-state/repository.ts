...
export async function setRepoState(resources: HqOpsResources, repoRoot: string, nextState: RepoState): Promise<void> {
  await mutateRepoStateAtomically(resources, repoRoot, async () => nextState);
}
...
export async function getRepoState(resources: HqOpsResources, repoRoot: string): Promise<RepoState> {
  const { state } = await getRepoStateWithAuthority(resources, repoRoot);
  return state;
}

export async function enablePlugin(resources: HqOpsResources, repoRoot: string, pluginId: string): Promise<RepoState> {
  const result = await mutateRepoStateAtomically(resources, repoRoot, async (current) => ({
    ...current,
    plugins: {
      ...current.plugins,
...
export async function disablePlugin(resources: HqOpsResources, repoRoot: string, pluginId: string): Promise<RepoState> {
  const result = await mutateRepoStateAtomically(resources, repoRoot, async (current) => ({
    ...current,
    plugins: {
      ...current.plugins,
...
export function createRepository(resources: HqOpsResources, repoRoot: string) {
  return {
    async getStateWithAuthority(): Promise<RepoStateSnapshot> {
      return await getRepoStateWithAuthority(resources, repoRoot);
    },
    async enablePlugin(pluginId: string): Promise<RepoState> {
      return await enablePlugin(resources, repoRoot, pluginId);
    },
    async disablePlugin(pluginId: string): Promise<RepoState> {
      return await disablePlugin(resources, repoRoot, pluginId);
    },
  };
}
