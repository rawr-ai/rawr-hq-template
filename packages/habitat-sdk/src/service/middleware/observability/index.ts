...
>(options: {
  ...
  return async ({ context, path, procedure, next }) => {
    ...
    observe(() => {
      ...
      span?.setAttributes(options.profile.getSpanAttributes({ context, meta, path, pathLabel }));
      span?.addEvent(options.profile.startedEvent, {
        path: pathLabel,
        ...options.profile.getStartEventAttributes?.({ context, meta, path, pathLabel }),
      });
      options.profile.onStart?.({ span, context, meta, path, pathLabel });
    });
