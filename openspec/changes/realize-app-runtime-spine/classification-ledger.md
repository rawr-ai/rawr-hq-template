...
|---|---|---|---|
| Public Habitat SDK and Oclif CLI | `nx run @habitat-ai/cli:acceptance:oclif-installed-package` | Build and locally pack both public products, inspect `dist/blueprints/service` for only the positive closed `service@1` kind and zero product/legacy rule vocabulary, install the exact SDK tarball into a disposable Nx consumer, invoke native `nx add @habitat-ai/cli@file:<absolute-packed-cli-tarball>`, construct and typecheck a generated oRPC 2/Effect 4 service, load every public cold subpath, run the installed `habitat` binary, project its Oclif manifest, and execute the installed initializer/plugin path. | A workspace import, unpublished dependency, missing service face, compatibility substrate, product term, legacy v2 or `forbids` packet, manifest drift, source/installed divergence, or nonzero installed command result fails before release. |
...
| `apps/cli` and mixed core command identity | Installed-package and owner-local command targets that co-land before deletion; later Oclif native-runtime targets are fresh-owner acceptance | Admitted command/result and manifest mechanics survive only when their final owner and behavior target co-land before deletion. The predecessor host, launcher, and runtime assembly are explicitly not preserved. |
