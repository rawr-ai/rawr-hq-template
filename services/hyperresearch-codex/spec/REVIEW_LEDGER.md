...
| HR-CODEX-013 | blocking | closed | Service root exposed runner, ledger, step loader, CLI helper, and integrity internals; service behavior lived in top-level files. | Moved reusable mechanics into `src/service/shared`, split callable surfaces into `fixtures` and `runs`, narrowed root exports, and added structural service-shape ratchets. |
...
| HR-CODEX-023 | blocking | closed | A failed topology refactor introduced artificial `runtime`/`common`/`modules/*/services` drift and left service examples/spec laws under-applied. | Reset invalid drift, split only into callable `fixtures` and `runs`, moved shared mechanics under `service/shared`, and added structural tests forbidding `runtime`, `common`, and module-local `services` directories. |
