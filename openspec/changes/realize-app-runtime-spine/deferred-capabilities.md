...
This is the durable handoff for capabilities retained outside the active runtime
release. Deferral changes delivery dependency, not the promised behavior or the
ownership boundary. Nothing here authorizes deleting, merging, restacking, or
editing a held source worktree.

The active [tasks](tasks.md) and [obligation disposition](obligation-disposition.md)
own runtime delivery. The [classification ledger](classification-ledger.md) owns
...
current native options cannot satisfy the contract. The active core runtime
...
[held temporal source matrix](stack-cut-sheet.md#temporal-source-disposition),
...
released the worktree; task 15.8 supplies this durable handoff, not automatic
adoption.

## D-4: Native Agent And Desktop Hosts

Only native host integration is deferred. Task 10.1 must implement real agent
authoring/executable/schema and desktop authoring/executable faces, including descriptor to
derivation to compilation to registry/process Effect invocation proof. No empty
export or declaration-only stub satisfies that obligation. The web-local Effect
