...
The Template-owned export request MUST accept exactly one closed artifact reference for a release or release set, one versioned `codex` or `claude` layout, one or more explicit absolute destinations, and an explicit overwrite policy whose default is `managed-only`. The application MUST invoke its consumer-owned read-only `ArtifactReader` internally and derive every planned byte from the returned ownership-transferred immutable snapshot rather than a request-supplied snapshot, mutable path, or content checkout. The reader's sole production adapter belongs to the build and artifact owner. C2 MUST expose this application only as an internal typed interface and MUST NOT register a command, controller-manifest member, runtime scan, alias, or generic projection fallback.

#### Scenario: Export ignores source workspace availability
- **WHEN** a verified artifact handle is exported after its personal content checkout and research locations have been removed
- **THEN** the exact same layout plan and bytes are produced from immutable artifacts
- **AND** no repository code, source fallback, or tree-equivalence check executes
...
- **AND** no destination can simultaneously report convergence, rejection, and an applied mutation
