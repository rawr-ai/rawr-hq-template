...
export type ExternalExtensionIdentity = string & { readonly [externalExtensionIdentityBrand]: true };
export type ExternalExtensionIdentityIssue = Readonly<{ path: "destination" | "id"; message: string }>;
...
export function parseExternalExtensionIdentity(
  ...
  return typeof input === "string" && input.length <= 64 && EXTENSION_ID_PATTERN.test(input)
    ...
    : Object.freeze({ path: "id", message: "Extension identity must be bounded lowercase kebab-case" });
}
