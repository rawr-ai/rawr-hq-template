    ...
    build(context) {
      ...
      return providerFx.acquireRelease({
        acquire: function* () {
          ...
          context.diagnostics.report("email provider acquired through Oracle bootgraph");
...
describe("provider provisioning lowering", () => {
  test("runs acquire and release through dependency-ordered Oracle bootgraph modules", async () => {
    const events: string[] = [];
