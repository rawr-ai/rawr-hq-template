...
- Do not treat fake adapter callbacks, vendor shape probes, or simulation
  bridges as Lab-Production Proof.
...
- Default program sequence from
  `../_archive/default-research-program-2026-04-30/workflow-runtime-research-program-dra.md`: workstream 6 is
  `Real Adapter Callback + Async Bridge Lowering`.
...
- `workstream-2026-04-30-phase-one-server-route-derivation.md`
- `workstream-2026-04-30-phase-one-dispatcher-access-async-step-membership.md`
- `../../src/spine/artifacts.ts`
- `../../src/spine/derive.ts`
- `../../src/spine/compiler.ts`
- `../../src/oracle/process-runtime.ts`
- `../../src/oracle/adapters/delegation.ts`
...
- Added focused oracle and middle-spine tests for payload delegation,
...
- Passed: reviewed `src/spine/artifacts.ts`, `src/spine/compiler.ts`,
  `src/oracle/adapters/delegation.ts`,
  `src/oracle/adapters/server.ts`, and
  `src/oracle/adapters/async.ts`.
...
- `bun test tools/runtime-realization-type-env/test/oracle/process-runtime.test.ts`: pass, 23 tests / 101 expectations.
...
- `bunx nx run runtime-realization-type-env:oracle`: pass, 31 tests / 154 expectations.
...
| --- | --- | --- | --- |
| Existing fake adapters prove delegation shape, not native-shaped payload lowering. | `src/oracle/adapters/server.ts`, `src/oracle/adapters/async.ts`, manifest entry `simulation.adapter-callback-delegation`. | Harden or split into route/async bridge lowering artifacts without claiming host mounting. | High |
...
| Vendor oRPC/Inngest probes are shape-only. | `vendors/README.md`, `guardrails-design.md`, and vendor-boundary tests. | Keep out of adapter proof except as downstream host context. | High |
...
- Did not promote real Elysia, oRPC, Inngest, HTTP, OpenAPI, durable async,
  boundary policy, package topology, or Lab-Production Proof.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Production native server harness mounting | `xfail` | This workstream may prove native-shaped lowering artifacts, but not Elysia/oRPC mounting or real HTTP behavior. | Diagnostic harness mounting row, `audit.p2.adapter-effect-callback-lowering`, server route workstream report. | First server harness mount workstream starts with accepted payload shape. | Lowering artifact is insufficient without `StartedHarness` evidence. | First Real Harness Mounts. | parent-repo-migration |
...
- Oracle: passed; payload helpers delegate through the contained process
