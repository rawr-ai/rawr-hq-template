...
export default class HyperresearchCodexSlice extends RawrCommand {
  ...
  async run() {
    ...
    const backend = String(flags.backend) === "fixture"
    ...
    try {
      ...
      const blockingFindings = resultData.integrity.filter((finding) => finding.severity === "blocking");
      const result = blockingFindings.length > 0
      ...
      this.outputResult(result, {
        flags: baseFlags,
        human: () => {
          this.log(`hyperresearch codex slice: ${resultData.ledger.completed ? "complete" : "incomplete"}`);
