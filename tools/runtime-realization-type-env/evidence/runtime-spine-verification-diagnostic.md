...
| Bootgraph and provisioning kernel | Dependency ordering, dedupe, rollback, reverse finalization, process/role scopes, provider acquisition through Effect. | Fake boot modules prove dependency ordering, rollback of started modules, reverse finalization, finalizer-failure recording, and redacted lifecycle records; provider provisioning modules now fail closed on provider graph diagnostics and run acquire/release through graph-driven dependencies, real Effect execution, dependency value flow, rollback, reverse finalization, and release-failure recording. | 🟡 | Process/role scope semantics, typed provider config binding, production bootgraph integration, and provider policy metadata remain open. |
...
The lab still does not prove cold SDK derivation from production declarations, final public `ProviderEffectPlan` shape, typed provider config binding, async step membership, dispatcher access declaration, server route derivation, production adapter/harness mounting, durable scheduling, telemetry export, catalog persistence, deployment placement, or production runtime readiness.
...
| Provider diagnostics and runtime profile config redaction | Prove config-bearing provider diagnostics and snapshots are redacted without locking config-source precedence or production secret stores. |
...
The highest-value next lab iteration is not more vendor probing. It is narrowing the remaining negative space: provider diagnostics/config redaction, async membership, dispatcher access, server route derivation, final `ProviderEffectPlan` shape, and the first real server/async harness mounting path.
