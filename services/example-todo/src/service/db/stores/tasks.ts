...
export function createTasksStore(sql: Sql, workspaceId: WorkspaceIdType): TasksStore {
  return {
    async findById(id: TodoIdentifierType): Promise<TaskType | null> {
      return await sql.queryOne<TaskType>(
        "SELECT * FROM tasks WHERE id = $1 AND workspace_id = $2",
        [id, workspaceId]
      );
    },

    async insert(task: TaskType): Promise<TaskType> {
      const row = await sql.queryOne<TaskType>(
    ...
    async findByIds(ids: TodoIdentifierType[]): Promise<TaskType[]> {
      return await sql.query<TaskType>(
