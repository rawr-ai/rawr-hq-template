...
import { createReadOnlyGitAdapter } from "../src/adapters/git";
import { createHostedApprovalAdapter } from "../src/adapters/hosted";
import {
  MAIN_REF,
  REPOSITORY,
  oid,
...
describe("hosted-governance adapter", () => {
  it("returns approval only when repository/ref/commit/tree/path/blob/outcome all match", async () => {
    ...
    const result = await adapter.read({ object: fixture.acceptanceObject, outcome: "accepted" });
