import type { ContentTreeEntry, ContentWorkspaceCapture } from "@rawr/resource-content-workspace";
...
/**
 * @purpose Own the cohesive Vendor source observation and authoring surface.
 * @capability Share exact content-workspace, versioned-content, and clock capabilities.
 * @behavior Report source status or author admitted fast-forward review changes.
 * @relation Exclude release construction, packaging, and native provider convergence.
 */
export const router = {
  status: module.status.effect(({ context, input: request }) =>
    Effect.gen(function* () {
      const readRecord = <T>(
        path: string,
        label: string,
      ...
      for (const member of releaseInput.value.body.members) {
        for (const sourceBinding of member.vendor.filter(
          ...
          if (!declarationRead.ok)
            return { kind: "Rejected" as const, issues: declarationRead.issues };
  ...
  update: module.update.effect(({ context, input: request }) =>
    Effect.gen(function* () {
      const rejected = (
            ...
            Effect.gen(function* () {
              ...
              if (receiptAttempt._tag === "Failure") {
                ...
                if (
                  ...
                  resourceFailureReason(error) === "HandleState"
                ) {
                  return vendorIssue(
                    "RestorationFailed",
              ...
              return validVendorReleaseReceipt(
                receiptAttempt.success,
                plan,
                captureHandle,
                disposition
              )
