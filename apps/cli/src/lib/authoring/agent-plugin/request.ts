...
export type CuratedAgentPluginRequestResult =
  ...
  | Readonly<{ ok: false; issues: readonly [CuratedAgentPluginIdentityIssue, ...CuratedAgentPluginIdentityIssue[]] }>;

export function parseCuratedAgentPluginAuthoringRequest(input: Readonly<{
  pluginId: unknown;
  ...
  if (typeof input.contentWorkspace !== "string" || input.contentWorkspace.length === 0) {
    issues.push(Object.freeze({ path: "contentWorkspace", message: "An explicit content workspace is required" }));
  }
  if (issues.length > 0 || isCuratedAgentPluginIdentityIssue(pluginId)) {
    return Object.freeze({
      ok: false,
      issues: Object.freeze(issues) as readonly [CuratedAgentPluginIdentityIssue, ...CuratedAgentPluginIdentityIssue[]],
    });
  }
  return Object.freeze({
    ok: true,
