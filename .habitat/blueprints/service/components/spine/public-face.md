...
predicate service_v1_public_face_is_client_target($target) {
  $target <: string(),
  $target <: r"^\"\./(?:src|dist)/client(?:\.d)?\.[cm]?[jt]s\"$"
}

// Recognizes a client target or a conditional container that can lead to one.
predicate service_v1_public_face_is_supported_value($value) {
  or {
    service_v1_public_face_is_client_target(target=$value),
    $value <: `{ $_ }`,
    $value <: `[$_]`
  }
}
...
predicate service_v1_public_face_has_closed_export($root, $client) {
  $root <: `{ $properties }`,
  $export_pairs = [],
...
predicate service_v1_public_face_is_closed_client_value($client) {
  service_v1_public_face_is_supported_value(value=$client),
  not { $client <: or { `{}`, `[]` } },
  not { $client <: contains or { `{}`, `[]` } },
  not {
    $client <: `[$items]`,
    $items <: some $item where {
      not { service_v1_public_face_is_supported_value(value=$item) }
    }
  },
  not {
  ...
  not {
    $client <: contains pair(value=$member) where {
      not { service_v1_public_face_is_supported_value(value=$member) }
    }
  }
}
...
document(value=$root) where {
  ...
  not {
    service_v1_public_face_has_closed_export(root=$root, client=$client),
    service_v1_public_face_is_closed_client_value(client=$client)
  }
}
```
