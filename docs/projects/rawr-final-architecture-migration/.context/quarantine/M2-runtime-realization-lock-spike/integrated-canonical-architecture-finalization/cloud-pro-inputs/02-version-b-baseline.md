...
| --- | --- | --- | --- |
| Public server API | Service + declared resources | `plugins/server/api/<capability>` | App selects plugin, profile, entrypoint, server harness |
| Trusted first-party server API | Service + workflow dispatcher or service clients | `plugins/server/internal/<capability>` | App selects internal projection and server harness |
| Durable workflow | Service + async provider/resource requirements | `plugins/async/workflows/<capability>` | App selects async projection and Inngest harness |
| Durable schedule | Service + schedule facts | `plugins/async/schedules/<capability>` | App selects schedule projection and Inngest harness |
| Durable consumer | Service + schema-backed event payload | `plugins/async/consumers/<capability>` | App selects consumer projection and Inngest harness |
| Human-facing shell | Service clients, machine resources, policy hooks | `plugins/agent/channels`, `plugins/agent/shell`, `plugins/agent/tools` | App selects agent projections and agent/OpenShell harness |
| Governed steward work | Service truth + worktree/steward resources | Async workflow projection | App selects async steward workflows |
| CLI tool | Service clients + command schema | `plugins/cli/commands/<capability>` | App selects CLI projection and OCLIF harness |
| Web product surface | Generated clients or surface contracts | `plugins/web/app/<capability>` | App selects web projection and web harness |
| Desktop product surface | Service clients or host resources | `plugins/desktop/menubar`, `plugins/desktop/windows`, `plugins/desktop/background` | App selects desktop projection and desktop harness |

The service stays the service. The projections multiply. The app selects which projections ship together. The entrypoint selects which role slices start in one process. The process shape changes; the semantic species does not.
...
RuntimeProfile       = app-owned provider and config-source selection
...
- apps select plugins, profiles, provider choices, and entrypoints but do not redefine service truth;
...
```text
runtime resource = typed capability interface + lifetime + config shape + provider implementation + provisioning scope
```

A runtime resource is not a service. It does not own business truth.

### 7.2 Provider posture
...
A provider may use Effect directly inside provider/runtime implementation. That use is public to runtime-resource authors, provider authors, substrate authors, process-runtime authors, and harness-integration authors. It is private to ordinary service, plugin, app, and entrypoint authoring.

### 7.3 RuntimeProfile posture

A `RuntimeProfile` is app-owned selection of providers, config sources, process defaults, harness choices, and environment-shaped wiring.
...
### 7.4 Resource/provider/profile laws
...
A plugin translates service truth or host capability into one role- and surface-specific runtime contribution.
...
### 8.3 Topology and builder agreement
...
| `plugins/server/internal/<capability>` | `defineServerInternalPlugin(...)` | Trusted first-party server API projection |
...
Route, command, function, shell, and native mount facts are builder-specific surface facts. They do not classify projection status. App selection and harness policy may select, mount, publish generated artifacts, or withhold already-classified projections. They do not reclassify a plugin projection.

A capability that needs both public and trusted first-party callable surfaces authors two projection packages.

### 8.4 Service use inside plugins
...
Plugins may also declare resource requirements. Resource requirements state what the projection or harness needs. They do not acquire providers.

The plugin owns projection. The service owns truth. The harness owns native host mechanics.

### 8.5 Server plugin rules
...
### 8.11 Plugin authoring invariants
...
profile selects provider implementation and config source
```
...
| Selection | App membership, runtime profile, provider choices, process roles, selected harnesses | App/entrypoint | SDK/runtime compiler |
...
The dispatcher does not own workflow semantics, expose product APIs by itself, construct native functions, classify projection status, or acquire the async provider.
...
- `RuntimeProfile` selects provider implementations and config sources;
...
flowchart LR
  ...
  P -. "support" .-> R
  R -. "capability contracts" .-> S
  V -. "implement contracts" .-> RP
...
apps select projections, runtime profiles, providers, entrypoints, and process shape
