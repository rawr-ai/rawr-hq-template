...
const forbiddenText = [
  ...
  [
    "workspace-derived Oclif repair",
    /reconcile-cli-command-plugin-links|assessInstallState|planInstallRepair/u,
  ],
  ["source CLI recursion", /bun\s+run\s+rawr|["']run["']\s*,\s*["']rawr["']/u],
  [
    "official install repair flags",
    /["']--checks["']\s*,\s*["']install["']|--checks\s+install|["']--repair["']/u,
  ],
...
for (const relativePath of forbiddenPaths) {
  if (fs.existsSync(path.join(root, relativePath)))
    findings.push(`forbidden path remains: ${relativePath}`);
}
...
if (findings.length > 0)
  throw new Error(`OFFICIAL_RELINK_AUTHORITY_REMAINS\n${findings.join("\n")}`);
