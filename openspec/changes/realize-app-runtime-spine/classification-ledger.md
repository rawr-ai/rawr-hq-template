...
| MCP server surface and native harnesses | **co-land with the MCP server vertical** `nx run runtime-harnesses:acceptance:mcp-native` | Author selected MCP tools, resources, and prompts as a `server` surface, lower them through the process-runtime adapter, and invoke one real tool and one real resource through official MCP stdio and Streamable HTTP servers installed from packed Habitat artifacts. Prove protocol errors, cancellation, origin/session policy, protocol-only stdout, process-local readiness/identity, and native stop before lease release. | An MCP role, kind, service, app, execution plane, companion attachment, direct provider acquisition, raw declaration consumption, stdout diagnostics, missing HTTP origin/session protection, early release, or source-only proof fails. |
| Deployment cold handoff | **co-land with the portable-plan boundary** `nx run runtime-derivation:acceptance:deployment-cold-plan` | Give deployment one portable process plan containing immutable app/process/entrypoint/source/deployment identity, placement requirements, and descriptor references only; prove a consumer can select placement without importing executables or starting runtime. | A live resource, executable closure, runtime handle, readiness gate, observation port, provider secret, or deployment-authored process lifecycle enters the handoff. |
