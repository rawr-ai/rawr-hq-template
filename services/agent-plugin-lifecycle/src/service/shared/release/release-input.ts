...
export function createAgentPluginReleaseInput(
  ...
  if (byteLength > MAX_RELEASE_INPUT_ENVELOPE_BYTES) {
    return failure([issue("ENVELOPE_TOO_LARGE", "releaseInput", "Release-input envelope exceeds its protocol bound", {
...
export function verifyAgentPluginReleaseInput(
  ...
  if (!isExactRecord(input, ["body", "releaseInputDigest", "schemaVersion"], "releaseInput", issues)) {
    return failure([issues[0] ?? issue("EXPECTED_OBJECT", "releaseInput", "Release input must be an object")]);
  }
  if (input.schemaVersion !== RELEASE_INPUT_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", "releaseInput.schemaVersion", "Unsupported release-input envelope version", {
  ...
  if (claimedDigest !== undefined && parsedBody.ok) {
    ...
    if (computed !== claimedDigest) {
      issues.push(issue("RELEASE_INPUT_DIGEST_MISMATCH", "releaseInput.releaseInputDigest", "Claimed digest differs from the release-input body", {
        expected: computed,
...
export function decodeAgentPluginReleaseInput(
  ...
  if (!(bytes instanceof Uint8Array) || !equalBytes(bytes, canonicalSerializeAgentPluginReleaseInput(verified.value))) {
    return failure([issue("NON_CANONICAL_ENVELOPE", "releaseInput", "Release-input bytes are not the unique canonical representation")]);
  }
  return verified;
}
...
export function canonicalSerializeAgentPluginReleaseInput(input: AgentPluginReleaseInput): Uint8Array {
...
export function parseCompletenessWitness(
  ...
  if (!isExactRecord(input, ["expectedMembers", "ownershipIndex", "releaseInputDigest"], path, issues)) return undefined;
  const digest = collect(parseReleaseInputDigest(input.releaseInputDigest, `${path}.releaseInputDigest`), issues);
  const ownershipIndex = parseDistributionOwnershipIndex(
    input.ownershipIndex,
    `${path}.ownershipIndex`,
    issues,
  );
  const rawMembers = parseBoundedArray(input.expectedMembers, `${path}.expectedMembers`, MAX_RELEASE_MEMBERS, issues);
  ...
  rawMembers?.forEach((candidate, index) => {
    ...
    const payload = collect(parsePayloadDigest(candidate.payloadDigest, `${memberPath}.payloadDigest`), issues);
    if (pluginId !== undefined && payload !== undefined) expectedMembers.push(Object.freeze({ pluginId, payloadDigest: payload }));
  });
  expectedMembers.sort((left, right) => compareCanonicalText(left.pluginId, right.pluginId));
  reportDuplicateMembers(expectedMembers.map((member) => member.pluginId), `${path}.expectedMembers`, issues);
  if (ownershipIndex !== undefined) {
    ...
    if (
      ...
      issues.push(issue("OWNERSHIP_INDEX_MISMATCH", `${path}.ownershipIndex`, "Completeness members and ownership members differ"));
    }
  }
...
function parseReleaseInputBody(
  ...
  if (!isExactRecord(
    input,
    ["contentAuthority", "locks", "members", "ownershipClaims", "qualityPolicies", "schemaVersion"],
    path,
    issues,
  )) {
    return failure([issues[0] ?? issue("EXPECTED_OBJECT", path, "Release-input body must be an object")]);
  }
  if (input.schemaVersion !== RELEASE_INPUT_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", `${path}.schemaVersion`, "Unsupported release-input body version", {
      expected: RELEASE_INPUT_SCHEMA_VERSION,
      actual: typeof input.schemaVersion === "number" ? input.schemaVersion : String(input.schemaVersion),
    }));
  }
  ...
  if (members !== undefined && ownershipClaims !== undefined) {
    const inventoryOverflow = issues.some((entry) => (
    ...
    const index = createDistributionOwnershipIndex(members.map((member) => member.pluginId), ownershipClaims);
...
function parseMembers(
  ...
  const aggregateSkillInventory = values.reduce<number>((total, candidate) => {
    if (candidate === null || typeof candidate !== "object" || Array.isArray(candidate)) return total;
  ...
  reportDuplicateMembers(members.map((member) => member.pluginId), path, issues);
  const aggregatePayloadBytes = members.reduce(
    (memberTotal, member) => memberTotal + member.payload.manifest.reduce(
      (entryTotal, entry) => entryTotal + entry.byteLength,
      0,
    ),
    0,
  );
  ...
  if (members.length === 0) {
    issues.push(issue("COUNT_LIMIT_EXCEEDED", path, "A curated release input must declare at least one member", {
      expected: "1..1024",
      actual: 0,
    }));
  }
...
function parseSkillInventory(
  ...
  admitted.forEach((candidate, index) => {
    ...
    const identity = collect(parseOwnershipIdentity(candidate.identity, `${entryPath}.identity`), issues);
    const manifestPath = collect(parseReleaseRelativePath(candidate.manifestPath, `${entryPath}.manifestPath`), issues);
...
function validateSkillOwnershipClosure(
  ...
  for (const member of members) {
    ...
    for (const manifestPath of skillManifestPaths) {
      const rowCount = inventoryByMemberPath.get(memberSkillKey(member.pluginId, manifestPath)) ?? 0;
      if (rowCount !== 1) {
        issues.push(issue(
    ...
    for (const entry of member.skillInventory) {
      ...
      if (!isCanonicalSkillManifestPath(entry.manifestPath) || !manifestPathSet.has(entry.manifestPath)) {
        issues.push(issue(
          "SKILL_INVENTORY_MISMATCH",
          entryPath,
      ...
      const claimCount = claimsByMemberIdentity.get(memberSkillKey(member.pluginId, entry.identity)) ?? 0;
      if (claimCount !== 1) {
        issues.push(issue(
          ...
          "A skill inventory row must have exactly one same-member skill ownership claim",
          {
            ...
            claimants: Object.freeze([...(claimOwnersByIdentity.get(entry.identity) ?? [])].sort(compareCanonicalText)),
          },
        ));
      }
    }
  ...
  for (const claim of skillClaims) {
    const rowCount = inventoryByMemberIdentity.get(memberSkillKey(claim.ownerPluginId, claim.identity)) ?? 0;
    if (rowCount !== 1) {
      issues.push(issue(
        "SKILL_OWNERSHIP_MISMATCH",
...
function parseUnitKind(value: unknown, path: string, issues: ReleaseIssue[]): "agent-plugin" | undefined {
  if (value === "agent-plugin") return value;
  if (value === "toolkit" || value === "agent-pack" || value === "composition") {
    issues.push(issue("FORBIDDEN_UNIT_KIND", path, `${value} cannot become an agent-plugin release member`));
    return undefined;
  }
...
function parseDeclaredPayload(
  ...
  if (!isExactRecord(input, ["manifest", "payloadDigest", "protocolVersion"], path, issues)) return undefined;
  ...
  if (totalBytes > MAX_PAYLOAD_BYTES_PER_MEMBER) {
    issues.push(issue("PAYLOAD_BYTES_LIMIT_EXCEEDED", `${path}.manifest`, "Declared payload exceeds its byte limit", {
      expected: MAX_PAYLOAD_BYTES_PER_MEMBER,
      actual: totalBytes,
    }));
  }
  if (manifest === undefined || digest === undefined || input.protocolVersion !== PAYLOAD_PROTOCOL_VERSION) return undefined;
  return Object.freeze({ protocolVersion: PAYLOAD_PROTOCOL_VERSION, manifest, payloadDigest: digest });
}
...
export function parseProvenanceBindings(
  ...
  values.forEach((candidate, index) => {
    ...
    const digest = collect(parseContentDigest(candidate.contentDigest, `${bindingPath}.contentDigest`), issues);
  ...
  for (let index = 1; index < bindings.length; index += 1) {
    if (bindings[index - 1]!.id === bindings[index]!.id) {
      issues.push(issue("DUPLICATE_VALUE", path, `Duplicate provenance binding: ${bindings[index]!.id}`));
    }
  }
...
function freezeReleaseInput(
  ...
  const expectedMembers = Object.freeze(body.members.map((member) => Object.freeze({
    pluginId: member.pluginId,
...
function compareSkillInventoryEntries(left: SkillInventoryEntry, right: SkillInventoryEntry): number {
  return compareCanonicalText(left.manifestPath, right.manifestPath)
    || compareCanonicalText(left.identity, right.identity);
}

function reportDuplicateMembers(memberIds: readonly PluginId[], path: string, issues: ReleaseIssue[]): void {
  for (let index = 1; index < memberIds.length; index += 1) {
    if (memberIds[index - 1] === memberIds[index]) {
      issues.push(issue("DUPLICATE_PLUGIN_ID", path, `Duplicate plugin identity: ${memberIds[index]}`));
    }
  }
}

export const AGENT_PLUGIN_BUILDER_PROTOCOL_VERSION: BuilderProtocolVersion = BUILDER_PROTOCOL_VERSION;
