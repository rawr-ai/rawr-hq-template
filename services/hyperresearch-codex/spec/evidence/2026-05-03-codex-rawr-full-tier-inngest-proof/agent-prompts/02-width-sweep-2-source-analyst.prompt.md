You are the `hyperresearch-source-analyst` role agent for a Hyperresearch Codex full-tier real-backend packet gate.
...
```json
{
  "jobId": "02-width-sweep-2-source-analyst",
  "role": "hyperresearch-source-analyst",
  ...
  "expectedOutputPath": "research/temp/codex-agent-results/02-width-sweep-2-source-analyst.json"
}
```
...
Required writes:
1. Write `/tmp/hyperresearch-codex-full-vault.px1Yg1/research/temp/source-capture-log.md` with a source-capture matrix for official Inngest docs. Include vendor claim focus, RAWR relevance, and any caveat about local evidence being grounding rather than vendor authority.
2. Write `/tmp/hyperresearch-codex-full-vault.px1Yg1/research/temp/claims-width.json` as valid JSON with an object containing a `claims` array. Include width-level candidate claims about Inngest semantics and RAWR plugin/runtime spec implications. Each claim should include `claim`, `category`, `officialSourceUrls`, `localContextPaths`, `confidence`, and `notes`. Use official Inngest URLs in `officialSourceUrls`; use local file paths only in `localContextPaths`.
3. Write `/tmp/hyperresearch-codex-full-vault.px1Yg1/research/temp/codex-agent-results/02-width-sweep-2-source-analyst.json` with:
   - `status: "complete"`
   - `summary`
   - `evidence` listing the files you read/wrote and local grounding paths consulted
   - `artifactWrites` for `research/temp/source-capture-log.md` and `research/temp/claims-width.json`, each with exact SHA-256 computed from disk
