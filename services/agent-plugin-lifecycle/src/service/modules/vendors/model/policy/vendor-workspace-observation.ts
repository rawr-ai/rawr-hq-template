...
export function observeVendorWorkspace(
  port: ContentWorkspaceResource<never>,
  requested: VendorContentWorkspaceRef
): Effect.Effect<VendorPolicyResult<VendorWorkspaceObservation>> {
  return Effect.gen(function* () {
    const identityAttempt = yield* Effect.result(
      port.inspectWorkspace({ locator: requested.locator })
    );
    ...
    const releaseInputBytes = releaseInputAttempt.success;
    ...
    for (const member of decodedInput.value.body.members) {
      for (const sourceBinding of member.vendor.filter(
        ...
        const observed = yield* observeDeclaredSource(
          port,
          requested.locator,
          identity,
...
function observeDeclaredSource(
  port: ContentWorkspaceResource<never>,
  root: string,
...
): Effect.Effect<VendorPolicyResult<VendorDeclaredSourceObservation>> {
  return Effect.gen(function* () {
    ...
    const lockRead =
      lockBinding === null
        ? null
        : yield* readRecord(port, root, lockBinding.id, "lock", decodeVendorLockRecord);
...
function observeDestination(
  ...
  return Effect.gen(function* () {
    ...
    const entries: readonly ContentTreeEntry[] = entriesAttempt.success;
...
function readRecord<T>(
  ...
  return Effect.gen(function* () {
    const bytesAttempt = yield* Effect.result(
      port.readFile({ root, path, maxBytes: MAX_VENDOR_RECORD_BYTES })
    );
    ...
    const bytes = bytesAttempt.success;
