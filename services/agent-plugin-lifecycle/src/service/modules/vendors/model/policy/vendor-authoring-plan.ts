...
export function createVendorAuthoringPlan(
  ...
  const changes = [...changesInput].sort((left, right) => compareText(left.sourceId, right.sourceId));
  ...
  for (const change of changes) {
    ...
    const declarationBinding = withContentDigest(change.declarationBinding, declarationContentDigest);
    ...
    bindingDigests.set(bindingKey(change.memberPluginId, declarationBinding), declarationContentDigest);
    bindingDigests.set(bindingKey(change.memberPluginId, provenanceBinding), provenanceContentDigest);
    bindingDigests.set(bindingKey("@locks", lockBinding), lockContentDigest);
    treeWrites.push(Object.freeze({
      kind: "ReplaceTree",
  ...
  if (writeSetIssue !== undefined) return policyFailure(vendorIssue("UnsupportedLayout", writeSetIssue));
  ...
  const planDigest = contentDigest(canonicalJsonLine({
    expectedReadToken,
  ...
  const created = createAgentPluginReleaseInput({
    ...
    members: body.members.map((member) => ({
      ...
      vendor: member.vendor.map((binding) => ({
        ...binding,
        contentDigest: bindingDigests.get(bindingKey(member.pluginId, binding)) ?? binding.contentDigest,
      })),
    })),
...
function expectedSourceObservations(
  ...
  const transitionById = new Map(transitions.map((transition) => [transition.sourceId, transition]));
  return Object.freeze(sources.map((source) => {
    ...
    return Object.freeze({
      ...
      destination: Object.freeze({
        kind: "Present",
        entries: change.payload.entries.map(({ path, mode, blob }) => Object.freeze({ path, mode, blob })),
