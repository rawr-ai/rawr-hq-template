...
import { buildDestinationExportPlan, type DestinationExportPlan } from "./plan";
import {
...
export interface ExportAgentPluginsDependencies {
...
export interface ExportAgentPluginsApplication {
  execute(request: unknown): Promise<ExportAgentPluginsResult>;
}
...
export function createExportAgentPluginsApplication(
  dependencies: ExportAgentPluginsDependencies,
): ExportAgentPluginsApplication {
  return Object.freeze({
    execute: (request: unknown) => executeExportAgentPlugins(request, dependencies),
  });
}
...
async function prepareDestinations(
  ...
  for (const requestPath of request.destinations) {
    ...
    try {
      const destination = await captureDestination(requestPath);
      ...
      const planning = await buildDestinationExportPlan(destination, request.layout, selection, request.overwritePolicy);
      if (!planning.ok) {
        prepared.push(Object.freeze({
          kind: "Resolved",
...
async function runAdmittedExportOperation(
  ...
  return withUndoSessionFinalizationGuard(admission.session, async (undoSession) => {
    ...
    const outcome = await executeAdmittedMutations(operation, session);
...
async function executeAdmittedMutations(
  ...
  for (const mutation of operation.mutations) {
    ...
    const transaction = await executeDestinationPlan(mutation.plan, session, actionEnd);
