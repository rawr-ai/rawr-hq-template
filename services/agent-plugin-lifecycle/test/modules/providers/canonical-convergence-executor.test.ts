...
import type { CanonicalConvergenceStep, CanonicalNativeMutationAction,
  CanonicalNativeObservation, CanonicalObservedConvergencePlan } from "../../../src/service/modules/providers/model/dto/canonical-convergence";
import { failure, issue, success, type DeploymentResult } from "../../../src/service/modules/providers/model/errors/deployment-result";
import { parseProviderTarget } from "../../../src/service/modules/providers/model/dto/provider-target";
import { createProviderMarketplaceRegistration, marketplaceState,
  type ProviderMarketplaceObservation, type ProviderMarketplaceRegistration } from "../../../src/service/modules/providers/model/policy/marketplace";
...
describe("canonical convergence executor", () => {
  ...
  it("executes replacement and omitted cleanup in exact forward order", async () => {
    ...
    expect(result.appliedPrefix.map(actionKind)).toEqual([
      "SetMarketplace", "RetireMember", "InstallMember", "RetireMember",
    ]);
    expect(result.verifiedSteps.map(stepKind)).toEqual([
      "verify-retired", "verify-selected", "verify-retired", "verify-final",
    ]);
  ...
  ] as const)("stops after a %s attempt without extending its definite prefix", async (
    _label, secondAttempt, expectedKind, lastKnown,
  ) => {
  ...
  it.each([
    ["ordinary failure", failure([issue("VISIBILITY_FAILED", "target.inventory", "unavailable")]), "VISIBILITY_FAILED"],
    ["ambiguous provenance", success({ kind: "ambiguous-provenance", target: TARGET, reason: "invalid metadata" } as const), "BLOCKED_COLLISION"],
  ] as const)("preserves the exact prefix on %s and stops the tail", async (
    _label, observation, expectedCode,
  ) => {
  ...
  it.each([
    ...
    const selectedLive = native(selected.members[0]!);
    const members = scenario === "member" ? [omitted] : scenario === "selected" ? [omitted] : [selectedLive];
    const exposures = scenario === "configuration"
      ? [exposure("docs@rawr-hq", omitted.nativeIdentity, OWNER)]
      : [];
    const steps: readonly CanonicalConvergenceStep[] = scenario === "selected"
      ? [
          { kind: "mutate", action: transition },
          { kind: "verify-selected", target: TARGET, projection: selected },
          { kind: "mutate", action: cleanup },
...
function applied(): NativeMutationAttempt { return { kind: "applied" }; }
...
function uncertain(lastKnown: "bridge-invoked" | "bridge-returned"): NativeMutationAttempt {
  return { kind: "uncertain", lastKnown, issues: [issue("MUTATION_FAILED", "target.mutation", "unknown")] };
}

function projection(members: readonly ProviderProjectionMember[], fill: string): AgentProviderProjection {
  const artifactAuthority = { protocol: "agent-plugin-artifact-authority@v1", contentAuthority: OWNER, sourceCommit: fill.repeat(40) };
  return {
    provider: "codex",
    adapterProtocol: ADAPTER,
    artifactAuthority,
    marketplace: { identity: OWNER },
    members: members.map((entry) => ({ ...entry, artifactAuthority, providerSourceIdentity: OWNER })),
...
function member(pluginId: string, fill: string, commitFill = "a"): ProviderProjectionMember {
  return {
    ...
    artifactAuthority: { protocol: "agent-plugin-artifact-authority@v1", contentAuthority: OWNER, sourceCommit: commitFill.repeat(40) },
    providerSourceIdentity: OWNER,
    visible: { pluginIdentity: `rawr:${pluginId}`, skills: [`${pluginId}-skill`], hooks: [`${pluginId}-hook`] },
...
function setMarketplace(registration: ProviderMarketplaceRegistration, expected: ProviderMarketplaceObservation): CanonicalNativeMutationAction {
