...
acquire/release lowering through RAWR-owned Oracle bootgraph/provisioning and real
...
- `workstream-2026-04-30-phase-one-middle-spine-verification.md`
- `../../src/spine/artifacts.ts`
- `../../src/spine/compiler.ts`
- `../../src/spine/derive.ts`
- `../../src/oracle/bootgraph.ts`
...
- `simulation-proof` for `audit.p1.provider-effect-plan-lowering` only if the
  RAWR-owned oracle path consumes provider acquire/release hooks through
...
- focused provider provisioning test or `bun test tools/runtime-realization-type-env/test/oracle/process-runtime.test.ts`
...
- Treating vendor proof or oracle proof as Lab-Production Proof.
...
- Lower provider modules into Oracle bootgraph/provisioning.
...
- Replaced the previous `lowerOpaqueProviderPlan(...)` experiment with
  ...
  selected providers into Oracle bootgraph modules.
...
- Extended Oracle bootgraph module context with dependency values so provider
...
- Updated the oracle target to run the full focused directory.
...
- Passed: `bun test tools/runtime-realization-type-env/test/oracle/provider-provisioning.test.ts`.
- Passed: `bun test tools/runtime-realization-type-env/test/oracle/process-runtime.test.ts`.
- Passed: `bunx nx run runtime-realization-type-env:oracle`.
...
| --- | --- | --- | --- | --- | --- | --- |
| Provider plan lowering belongs to bootgraph/provisioning, not process execution. | provisioning | canonical spec sections 12, 14, 15 | target `simulation-proof` | removing bootgraph/provisioning lowering breaks acquire/release tests | oracle/focused provider test | not Lab-Production Proof |
| Provider identity preserves resource, provider, lifetime, role, and instance. | compilation/provisioning | prior report repair demand, diagnostic | target `simulation-proof` | collapsing key fields causes dependency/order/catalog assertions to fail | middle-spine/oracle | not final identity string format |
| Provider dependency graph diagnostics and exact matched keys gate provisioning. | compilation/provisioning | canonical spec sections 13, 15, 24 | target `simulation-proof` | graph diagnostics throw before modules and scoped matched keys drive bootgraph dependencies | middle-spine/oracle | not final production compiler or bootgraph integration |
| Provider release runs on rollback and finalization. | provisioning/observation | canonical spec section 15 | target `simulation-proof` | failed acquire rolls back started providers and normal finalize releases in reverse order | oracle | not full Effect boundary policy |
| Provider lifecycle records are redacted and contain no live handles. | observation | canonical spec sections 20 and 21 | target `simulation-proof` | secret/live handle sentinel appears in catalog/diagnostics | oracle/report | not telemetry export or catalog persistence |
...
| --- | --- | --- | --- |
| Existing `lowerOpaqueProviderPlan(...)` is an experiment, not proof. | `src/oracle/provider-lowering.ts`, `test/oracle/process-runtime.test.ts` | Replace or supersede with real lab lowering. | high |
...
- `runtime-spine-verification-diagnostic.md` now records provider acquire/release
  lowering as contained oracle evidence while keeping the component
  yellow for final plan shape, config binding, policy, and production
  integration.
- `systems/runtime-spine-evidence-map.md`, `systems/effect-integration-map.md`, `current-lab-state.md`, and
  `ref-runtime-realization-research-program.md` were updated to reflect the same
  proof boundary.
...
| Production provider bootgraph integration | out-of-scope for this workstream | The proof target was the contained Oracle bootgraph path, not production harness or compiler integration. | Diagnostic, spine map, research program. | First real harness/production bootgraph work consumes provider provisioning output. | A production harness tries to mount provider-acquired resources. | First Real Harness Mounts | migration |
...
- Mechanical: passed after repair. Nx target now runs the full oracle test
...
- Oracle: passed. Focused provider tests and full oracle target
...
- `src/oracle/bootgraph.ts`
- `src/oracle/provider-lowering.ts`
- `test/oracle/provider-provisioning.test.ts`
- `test/oracle/process-runtime.test.ts`
...
- `bun test tools/runtime-realization-type-env/test/oracle/provider-provisioning.test.ts`
- `bun test tools/runtime-realization-type-env/test/oracle/process-runtime.test.ts`
