...
export async function observeVendorWorkspace(
  ...
  if (decodedInput.value.body.contentAuthority !== requested.contentAuthority) {
    return policyFailure(vendorIssue("WrongRepository", "Release input declares a different content authority."));
  }

  const sources: VendorDeclaredSourceObservation[] = [];
  for (const member of decodedInput.value.body.members) {
    for (const sourceBinding of member.vendor.filter((binding) => binding.protocol === VENDOR_SOURCE_PROTOCOL)) {
...
async function observeDeclaredSource(
  ...
  if (declarationBinding === undefined) {
    return policyFailure(vendorIssue("PayloadMismatch", "Vendor declaration binding is not a canonical repository record binding."));
  }
  ...
  const provenanceMatches = vendorBindings.filter((candidate) =>
    candidate.protocol === VENDOR_PROVENANCE_PROTOCOL && candidate.id === declaration.provenancePath);
  const lockMatches = locks.filter((candidate) =>
    candidate.protocol === VENDOR_LOCK_PROTOCOL && candidate.id === declaration.lockPath);
  const provenanceBinding = provenanceMatches.length === 1
    ? recordBinding(provenanceMatches[0], VENDOR_PROVENANCE_PROTOCOL) ?? null
    : null;
  const lockBinding = lockMatches.length === 1
    ? recordBinding(lockMatches[0], VENDOR_LOCK_PROTOCOL) ?? null
    : null;
  const provenanceRead = provenanceBinding === null
    ? null
    : await readRecord(port, root, provenanceBinding.id, "provenance", decodeVendorProvenanceRecord);
...
async function observeDestination(
  ...
  try {
    ...
    if (reason === "Aliased" || reason === "UnsupportedEntry" || reason === "LimitExceeded" || reason === "InvalidInput") {
      return policySuccess(Object.freeze({ kind: "Invalid", detail: resourceFailureDetail("read-tree", error) }));
    }
    return policyFailure(vendorIssue(
      "RuntimeFailure",
  ...
  if (layoutIssue !== undefined) return policySuccess(Object.freeze({ kind: "Invalid", detail: layoutIssue }));
...
function workspaceIdentityIssue(
  ...
  if (actual.root !== requested.locator) {
    return vendorIssue("RuntimeFailure", "Content workspace identity returned a different canonical root.");
  }
  if (!actual.remoteUrls.includes(requested.repositoryIdentity)) {
    return vendorIssue("WrongRepository", "Content workspace has no exact remote matching the requested repository identity.");
  }
  ...
  if (actual.commit !== requested.sourceCommit || actual.tree !== requested.sourceTree) {
    return vendorIssue("LocalDrift", "Content workspace commit or tree differs from the requested immutable identity.");
  }
  return undefined;
}
