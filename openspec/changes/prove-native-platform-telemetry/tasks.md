...
- [ ] 1.1 Add one focused Nx-owned compatibility fixture that pins oRPC, `@orpc/opentelemetry`, and `@orpc/evlog` at 2.0.0-beta.23, Effect and `@effect/opentelemetry` at 4.0.0-beta.101, Inngest at 3.51.0, OpenTelemetry API 1.9.0 plus Node/log 0.213.0 and stable 2.6.0 families, and EVlog at 2.24.0; assert every resolved public package version.
- [ ] 1.2 In that fixture, create one app telemetry lifecycle with one tracer, meter, and logger provider; attach exactly one `InngestSpanProcessor` to the same Inngest client used by the native handler.
- [ ] 1.3 Exercise matched, unmatched, and batched oRPC boundaries through the exact `EvlogHandlerPlugin` with Effect work, shared trace context, one metric, technical and product LogRecords, exactly one product event per operation attempt, no host duplicate or outcome filter, and bounded flush/shutdown.
- [ ] 1.4 Run the fixture and record the admitted imports, versions, client identity, correlation, and lifecycle result; stop on incompatibility rather than adding a shim, patch, manual transport, or peer signal provider.
...
- [ ] 3.1 Scaffold `resources/telemetry` and `resources/telemetry/providers/opentelemetry-node` with package exports, Nx ownership, closed test placement, and the [[../../../.habitat/blueprints/resource/README|resource]], [[../../../.habitat/blueprints/provider/README|provider]], and [[../../../.habitat/blueprints/app/skill|app]] ownership laws.
- [ ] 3.2 Define the provider-neutral contract for process identity, flat correlation attributes, spans/events/annotations, native-operation event scopes, non-throwing flush, lifecycle state, and bounded diagnostics without importing vendor types.
- [ ] 3.3 Implement the disabled provider branch before vendor construction and prove zero SDK, signal provider, exporter, processor, periodic reader, drain, timer, process-hook, Langfuse, and telemetry-network factory calls.
- [ ] 3.4 Implement the OpenTelemetry Node provider set with one app-owned lifecycle, one provider for each enabled trace/metric/log signal, the standard OTLP HTTP exporters, and one global W3C Trace Context plus Baggage configuration.
- [ ] 3.5 Bind Effect to the process tracer provider and admit the process-owned Inngest client as the sole input to its one `InngestSpanProcessor`; add no instrumentation-only client or alternate context manager.
- [ ] 3.6 Bridge bounded technical logs and finalized EVlog product events through the one logger provider with distinct record-kind attributes, flat allowlisted fields, and provider-boundary redaction; add no EVlog OTLP drain or direct backend client.
- [ ] 3.7 Implement the shared bounded lifecycle stages for product-event finalization, event-intake close, per-signal force flush, and signal-provider shutdown without provider-owned process hooks or `process.exit` calls.
- [ ] 3.8 Add focused resource/provider tests for enabled and disabled construction, one provider per signal, non-interfering constructor/export/finalizer failures, exactly-once product-event finalization, shared shutdown completion, stage continuation, and deadline expiry.
...
- [ ] 4.4 Route the existing host technical logger and request metrics through the lifecycle's logger and meter providers with the receipt id and transport request id as bounded correlation attributes.
- [ ] 4.5 Make the host edge the single ingress extraction point, configure native oRPC not to repeat extraction, and prove HTTP, oRPC, and Effect child spans share the expected trace lineage.
...
- [ ] 4.7 Coordinate server shutdown in order: stop HTTP/Inngest intake, drain admitted work, finalize product events, close event intake, flush each signal, then shut down the app lifecycle; preserve product-derived process status on every telemetry failure.
...
- [ ] 5.2 Use native Oclif lifecycle hooks to open one product event after command resolution, enrich it with bounded command/trace/outcome fields and technical logs, and finalize it once for success, declared error, or cancellation.
- [ ] 5.3 On command completion or signal, stop new command intake, drain the admitted invocation, finalize its event, close event intake, flush each signal, and shut down through the shared bounded lifecycle without replacing the command exit classification.
...
- [x] 7.4 Record the completed PostHog placement audit: a separate post-core `AnalyticsSinkResource` receives finalized allowlisted product events; PostHog receives no technical telemetry and owns no telemetry provider/runtime or core completion state.
- [ ] 7.5 After the core receipt and a deliberate actor/personless identity rule exist, admit `posthog-node@5.46.1` under Bun with executable import, capture, flush, and bounded-shutdown checks, or defer production enablement without blocking core completion.
- [ ] 7.6 If admitted, add the optional analytics-sink resource and PostHog provider as a separate semantic node, translate the same finalized event once, and retire duplicate per-procedure analytics emission without changing product outcomes.
