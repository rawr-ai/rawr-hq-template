...
/**
 * Lab proof trace for the oracle harnesses.
 *
 ...
 */
export interface OracleHarnessRecord {
  readonly kind: "oracle.harness-record";
  readonly harness: OracleHarnessKind;
...
interface StartedHarnessBase {
  ...
  records(): readonly OracleHarnessRecord[];
  stop(): Promise<readonly OracleHarnessRecord[]>;
}

export interface StartedOracleServerHarness extends StartedHarnessBase {
...
function createHarnessLifecycle(input: {
  ...
  let stopped = false;
  const records: OracleHarnessRecord[] = [
    record({
 ...
 */
export function mountOracleAsyncHarness(input: {
  readonly harnessId: string;
  readonly runtime: ProcessExecutionRuntime;
