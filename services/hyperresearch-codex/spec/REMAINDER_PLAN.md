...
- Do not pivot to app-server, Codex SDK, or OpenAI SDK by assumption. `NATIVE_CODEX_SURFACE_REVIEW.md` records the current review: the TypeScript Codex SDK wraps `codex exec`, raw OpenAI SDKs are a different runtime, and app-server is the preferred diagnostic surface. App-server already supports the start/resume/read/live-reconnect primitives we need, but the preserved cold-resume app-server smoke reproduced the child-handle failure as structured `notFound`. The remaining repair belongs in Codex/RAWR runtime descendant resume behavior or in an explicit re-scope of the child lifecycle claim.
...
The Codex-RAWR packet-provenance light proof, short multi-source runtime proof, and repaired full-tier Inngest V8 proof are green; see `evidence.md`. The focused child-agent lifecycle diagnostic has now failed the resume case: same-process children pass, but pre-resume child handles become `not_found` after `codex-rawr exec resume`. App-server smoke reproduced the same cold-resume child-handle failure with structured collab events. Explicit app-server `resume_agent` recovered the child handle to `pendingInit`, but `wait` timed out and clean child completion remained unproven. Active Hyperresearch Codex parity is not clean/green/done until `HR-CODEX-035` is resolved or the claim is explicitly re-scoped. The next concrete track is Codex/RAWR runtime repair or an explicit re-scope; it is not a Hyperresearch service redesign. Hooks, parked MCP work, production Inngest readiness, and unrelated global downstream plugin drift remain separate tracks unless explicitly promoted.
...
- 2026-05-03: Preserved app-server explicit-child-resume smoke under `spec/evidence/20260503T213000Z-app-server-explicit-child-resume/`. After cold parent resume, model-driven `resume_agent` recovered the original child handle to `pendingInit` and avoided `notFound`, but `wait` timed out and `closeAgent` only observed previous status `pendingInit`. This narrows the runtime issue to resumed-child execution/completion after descendant recovery.
