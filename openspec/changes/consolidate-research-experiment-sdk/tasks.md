...
- [x] 1.2 Accept exact cell identity, terminal adoption, attempt admission,
  evaluation durability, residue, observation, and artifact laws.
- [x] 1.3 Complete bounded vendor verification.
- [x] 1.4 Accept the Git/Bun behavior slice at
  `ce282cb062f0d4bdeb80117a021aa0c766537991`.
- [ ] 1.5 Accept this deletion-first service/resource/provider checkpoint from
  the Inngest director and architecture steward.
- [ ] 1.6 Restack onto the exact accepted Template upstream containing the
...
- [ ] 2.2 Implement TypeBox cell, terminal, evaluation, observation, attempt,
  and residue schemas in the service. As research-service choices within the
...
- [ ] 2.3 Move the exact adoption, attempt, terminal, evaluation, observation,
  residue, and instance laws from `packages/research-sdk` into service-owned
  DTO/policy modules.
...
- [ ] 2.5 Prove the service flow with injected resource ports: terminal
  adoption before effects, atomic attempt admission, write-once terminal,
  durable evaluation before projection, exact observation identity, and
  unresolved-residue blocking. Preparation and evaluation consume lane
  TypeBox data/configuration/policy plus provisioned resources, never
  lane-injected executable callbacks.
...
- [ ] 3.2 Move the accepted Git materialize/capture/apply behavior behind a
  Git-artifact resource/provider without changing its substrate, patch, or
  round-trip guarantees. Return the admitted substrate to service-owned
  preparation so the service persists it in `FrozenInput` and supplies it to
  capture/apply; lanes never carry the provider envelope as execution
  authority.
- [ ] 3.3 Move the accepted Bun pack/verify behavior into BUILD compatibility
  tooling outside the running service. Remove its hard-coded SDK root and admit
  only the explicit immutable service/resource closure needed by both lanes.
- [ ] 3.4 Add a durable cell-state resource contract and resource-local
  providers for atomic attempt, terminal, evaluation, orphan, and residue
  persistence. Keep identity checks, admissibility, write ordering, adoption,
  and interpretation in the service; lanes supply data/configuration and policy,
...
- [ ] 3.5 Implement the sandbox, agent, observation, operational-event,
  ...
  contracts and accepted vendor laws.
- [ ] 3.6 Derive the Codex-Langfuse upstream digest manifest from official Git
  objects and rebuild the exact Node 22 ESM provider artifact from maintained
  source.
...
- [ ] 5.1 Pack the required Template service/resource packages through the
  corrected immutable compatibility operation and verify their exact installed
  closures in isolated lane consumers.
...
- [ ] 5.3 Bind retained Inngest S09 through the same service, preserving its
  seven-file seed view, lane-owned control overlay, service-bound Git
  substrate, allowed product paths, and hidden verifier.
...
- [ ] 6.3 Pass all model-free behavioral, artifact, package, interruption,
...
- [ ] 6.5 Restack onto current accepted Template upstream, re-admit exact
  vendors, rerun every gate, and obtain exact-commit acceptance from both
  directors.
