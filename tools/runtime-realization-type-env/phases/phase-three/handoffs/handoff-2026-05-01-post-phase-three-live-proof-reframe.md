...
understanding of the runtime spine inside the Runtime Realization Lab. That
learning is useful only if the next phase treats Oracle as the conformance
proof harness and builds the Reference Runtime as the next proof target.
...
The next phase should be designed around that distinction. Oracle should be
kept as a conformance and failure-injection harness. It should not be allowed to
stand in for Reference Runtime or live integration proof.
...
| --- | --- | --- |
| Runtime spine semantics | The lab made the spine executable enough to reason about derivation, provisioning, invocation, observation, and stop/finalization. | Internal conformance, not live Lab-Production Proof. |
...
Keep Oracle as the existing proof harness inside the Runtime Realization Lab.
...
Parent-Repo Migration would add incidental risk. Delete or supersede old lab
fixtures when the full reference runtime makes them redundant.

### 5. Lab-Production Layer

Only after the Reference Runtime works should the program move toward
Lab-Production Proof: deployment/process lifecycle, config/secrets, host
telemetry, vendor product visibility, persistence, and Parent-Repo Migration
readiness evidence.

This layer is still inside the Runtime Realization Lab. Parent-Repo Migration
risk is accepted only in a later transfer slice, not smuggled into a contained
lab phase by tone or implication.
...
- Lab `simulation-proof` must not be promoted into Lab-Production Proof.
