...
const FIXTURE_PREFIX = "runtime-compiler-nx-cache-";
...
test("restores an unchanged build and invalidates on a relevant compiler input", async () => {
  const fixtureRoot = await createFixture();
  const ownerRoot = path.join(fixtureRoot, compilerPath);
  ...
  try {
    ...
    await writeFile(
      configPath,
      `${originalConfig}\n// Relevant-input mutation for Nx cache proof.\n`
    );
    await rm(outputRoot, { recursive: true, force: false });
...
async function createFixture(): Promise<string> {
  ...
  try {
    for (const relativePath of [schemaPath, definitionPath, derivationPath, compilerPath]) {
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-compiler:build", "--outputStyle=static"],
      {
        cwd: fixtureRoot,
        env,
      }
    ...
    const timer = setTimeout(() => {
      timeoutError = new Error("runtime-compiler:build exceeded its cache-proof timeout.");
      child.kill("SIGKILL");
    }, 45_000);
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-compiler:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-compiler Nx fixture: ${root}`);
  }
