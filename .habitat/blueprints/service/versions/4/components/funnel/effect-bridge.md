...
predicate service_v4_effect_bridge_uses_terminal($statements, $owner, $runner) {
  $statements <: contains `$owner.$runner` where {
    service_v4_effect_bridge_terminal_name(runner=$runner)
  }
}

// Relates root-module imports to the exact ManagedRuntime export.
predicate service_v4_effect_bridge_uses_managed_runtime($statements) {
  or {
    and {
      $statements <: contains `import { $..., ManagedRuntime, $... } from $source`,
      service_v4_effect_bridge_is_effect_root(source=$source)
    },
    and {
...
or {
  ...
  or {
    ...
    service_v4_effect_bridge_is_production_source(),
    service_v4_effect_bridge_is_vendor_source(source=$source)
  },
  ...
  or {
    ...
    service_v4_effect_bridge_is_production_source(),
    service_v4_effect_bridge_is_managed_runtime_source(source=$source)
  },
  // The SDK facade is admitted only as a bare static import in impl.ts.
  import_statement(source=$source) as $import where {
    service_v4_effect_bridge_is_production_source(),
    service_v4_effect_bridge_is_facade_source(source=$source),
    or {
      not { service_v4_effect_bridge_is_implementation_source() },
  ...
  program(statements=$statements) where {
    service_v4_effect_bridge_is_implementation_source(),
    $statements <: [
      $...,
    ...
    service_v4_effect_bridge_is_facade_source(source=$first),
    service_v4_effect_bridge_is_facade_source(source=$second)
  },
  program(statements=$statements) where {
    service_v4_effect_bridge_is_production_source(),
    or {
      and {
        ...
        $source <: r"^[\"']effect[\"']$",
        service_v4_effect_bridge_uses_terminal(
      ...
      service_v4_effect_bridge_uses_managed_runtime(statements=$statements)
    }
  }
}
```
