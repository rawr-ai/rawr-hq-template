...
const analyticsMiddlewarePath = path.join(
  root,
  "services",
  "state",
  "src",
...
const moduleMiddlewarePath = path.join(
  root,
  "services",
  "state",
  "src",
...
const moduleRouterPath = path.join(
  root,
  "services",
  "state",
  "src",
...
const moduleContractPath = path.join(
  root,
  "services",
  "state",
  "src",
...
const [
  ...
  baseSource,
  implSource,
  observabilityMiddlewareSource,
  analyticsMiddlewareSource,
  routerSource,
  contractSource,
  moduleSource,
  moduleMiddlewareSource,
  moduleRouterSource,
  moduleContractSource,
  indexSource,
  repoStateIndexSource,
  typingProofSource,
] = await Promise.all([
...
if (
  indexSource.includes("getRepoState") ||
  indexSource.includes("enablePlugin") ||
  indexSource.includes("RepoState")
) {
  fail("package root must stay thin and not export repo-state support helpers.");
}

if (
  !repoStateIndexSource.includes("getRepoState") ||
  !repoStateIndexSource.includes("enablePlugin")
) {
...
if (requiredExtensions.properties.length !== 2) {
  fail(
    "service impl must attach exactly one required observability middleware and one required analytics middleware."
  );
}
