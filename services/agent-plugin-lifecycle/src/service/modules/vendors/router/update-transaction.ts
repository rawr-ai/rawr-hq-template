...
export async function executeVendorAuthoringPlan(
  ...
  if (settlement !== undefined) {
    ...
    return restoreAfterFailure(contentWorkspace, plan, capture.handle, request.sourceIds, settlement);
  }
...
async function restoreAfterFailure(
  ...
  try {
    ...
    const cleanup = await releaseCapture(contentWorkspace, plan, captureHandle, "UnsettledRecovery");
    return {
