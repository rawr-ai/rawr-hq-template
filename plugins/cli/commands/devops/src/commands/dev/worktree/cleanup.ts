...
export default class DevWorktreeCleanup extends HabitatCommand {
  ...
  static flags = {
    ...
    "merged-only": Flags.boolean({
      description: "Only remove branches already merged into trunk",
      default: true,
      allowNo: true,
    }),
    trunk: Flags.string({ description: "Trunk branch used for merged checks", default: "main" }),
    "pin-path": Flags.string({
      description: "Worktree path to exclude (repeatable)",
      multiple: true,
    }),
  ...
  async run() {
    const { flags } = await this.parse(DevWorktreeCleanup);
