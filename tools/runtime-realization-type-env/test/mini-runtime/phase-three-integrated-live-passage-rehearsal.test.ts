...
import {
  ...
  type MiniRuntimeResourceAccessProbe,
  ...
  const serverDescriptor = {
    ...
    run(input) {
      const invocation = input as {
        readonly context: {
          readonly resources: MiniRuntimeResourceAccessProbe;
        };
      };
...
function createObservedResourceAccess(
  ...
  const resourceAccess = createMiniRuntimeResourceAccess(resources);
  ...
  } satisfies MiniRuntimeResourceAccessProbe;
}
