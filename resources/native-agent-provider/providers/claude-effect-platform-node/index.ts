...
> = Object.freeze({
  acquire: (input: NativeProviderSessionInput) => acquireEffectPlatformNodeProvider("claude", input).pipe(
    Effect.map((kernel): ClaudeNativeAgentProviderSession => {
      ...
      const listInstalledPlugins = () => kernel.run("plugin-read", [
        "plugin",
        "list",
        "--json",
      ]).pipe(Effect.flatMap((result) => parseJsonObservation("claude", "plugin-read", result)));
      ...
      return Object.freeze({
        ...
        probe,
        listMarketplaces,
        readMarketplace: (request: Parameters<ClaudeNativeAgentProviderSession["readMarketplace"]>[0]) =>
        ...
        addMarketplace: (source: Parameters<ClaudeNativeAgentProviderSession["addMarketplace"]>[0]) =>
        ...
        readPlugin: (request: Parameters<ClaudeNativeAgentProviderSession["readPlugin"]>[0]) =>
          requirePluginSelector("claude", "plugin-read", request.selector).pipe(
            Effect.flatMap((selector) => listInstalledPlugins().pipe(
              Effect.flatMap((observation) => selectInstalledPluginRoot(observation.json, selector)),
            )),
...
function selectMarketplaceRoot(
  ...
  for (const entry of json) {
    if (!isJsonRecord(entry) || typeof entry.name !== "string") {
      return Effect.fail(providerFailure("marketplace-read", "ProtocolFailed", "Claude marketplace entry is invalid"));
    }
...
function selectInstalledPluginRoot(
  ...
  for (const entry of json) {
    if (!isJsonRecord(entry) || typeof entry.id !== "string") {
      return Effect.fail(providerFailure("plugin-read", "ProtocolFailed", "Claude installed plugin entry is invalid"));
    }
    if (entry.id !== selector) continue;
    if (entry.scope !== "user" || typeof entry.enabled !== "boolean" || typeof entry.installPath !== "string") {
      return Effect.fail(providerFailure("plugin-read", "ProtocolFailed", "Claude installed plugin entry is invalid"));
    }
