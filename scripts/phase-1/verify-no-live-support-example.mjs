...
const supportExampleRoots = ["services/support-example", "plugins/workflows/support-example"];
...
for (const relPath of supportExampleRoots) {
  ...
  assertCondition(
    !ledger.parked.includes(relPath),
    `${relPath} must not appear in the parked lane`
  );
  assertCondition(
    !(relPath in ledger.reclassified),
    `${relPath} must not appear in reclassified targets`
  );
...
assertCondition(
  lessons.includes("queued -> running -> completed|failed"),
  "support-example lessons must preserve the lifecycle fixture"
);
assertCondition(
  lessons.includes(
    "runId, workItemId, repoRoot, queueId, requestedBy, dryRun, requestId, correlationId"
  ),
  "support-example lessons must preserve the representative payload"
);
assertCondition(
  lessons.includes("async acceptance language"),
  "support-example lessons must preserve the translated proof intent"
);

console.log(
  "support-example remains archived, removed from the live tree, and preserved only as archive lessons."
);
