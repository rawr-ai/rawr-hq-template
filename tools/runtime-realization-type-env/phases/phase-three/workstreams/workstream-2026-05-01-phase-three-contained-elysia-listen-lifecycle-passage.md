...
Parent-Repo Migration authorization, or permission to mutate production app code.
...
| Blocked claims | Production HTTP serving, deployment topology, auth/logging policy, OpenAPI/product API policy, native host telemetry/error mapping, Parent-Repo Migration authorization, durable async semantics, live HyperDX product visibility, RuntimeCatalog persistence, public API/DX law, and final Nx/generator ratchet. |
...
- Determine how much of a real local listener start/request/stop passage can be
  proven inside the Runtime Realization Lab without touching production `apps/*`.
...
| Durable report disposition | If this child discovers useful Elysia listener integration rules for later runtime design, update `vendors/README.md` or create a labeled reference report. Otherwise keep findings inside this child report. |

## Output Contract

Required executable proof outputs:

- A focused oracle test or test cluster that proves the selected local
...
- Updated proof manifest/current lab state/diagnostic/vendor maps as applicable.
...
- It cannot prove production HTTP serving, deployment lifecycle, OpenAPI/product
  policy, auth/logging, native host telemetry/error mapping, or Parent-Repo
...
- Stop if this child's contained listener proof is described as Lab-Production
  Proof or Parent-Repo Migration authorization.
...
oracle path, stop deterministically, and reject post-stop network passage
without further runtime delegation?

Accepted oracle:

- source adapter:
  `src/oracle/adapters/elysia-listener.ts`;
- test:
  `test/oracle/phase-three-contained-elysia-listen-lifecycle-passage.test.ts`;
...
- the test must fail if listener records are collapsed into Elysia host or oRPC
  records;
- the test must fail if Lab-Production Proof claims appear in promoted evidence.
...
- preserves the existing Elysia host, oRPC Fetch, Oracle server harness, adapter,
...
- `bun test
  tools/runtime-realization-type-env/test/oracle/phase-three-contained-elysia-listen-lifecycle-passage.test.ts`:
...
| Vendor fidelity | pass with wording repair | Accepted after recording in `vendors/README.md` that this proves contained Bun/Elysia local listener lifecycle only, not production serving, supervision, TLS/proxy, Node parity, OpenAPI/Eden behavior, auth/logging, deployment topology, or Parent-Repo Migration authorization. |
...
- No TLS, proxy, load-balancer, auth/logging, OpenAPI/product policy, Node
  adapter parity, native host telemetry/error mapping, or Parent-Repo Migration
  readiness.
...
| Stale residual wording can make a new proof look unacknowledged. | Reworded diagnostic and map language to distinguish contained local listen proof from production deployed lifecycle. | Future evidence updates must sweep component matrix, residual rows, current lab state, and current-state cards for stale "still open" language. | Mechanical/information-design review lane. | Accepted. |

Deferred Inventory:

| Residual | Why deferred | Authority home | Re-entry trigger | Next eligible workstream |
| --- | --- | --- | --- | --- |
| Production HTTP serving under deployment/process supervision | Requires production host/process topology and deployment lifecycle decisions outside the Runtime Realization Lab. | Runtime spec plus later Parent-Repo Migration/control-plane workstream. | A migration workstream opens production `apps/*` or deployment topology explicitly. | Parent-Repo Migration or deployment/control-plane phase. |
...
| Auth/logging/OpenAPI/product policy | Requires public product API/DX and security policy decisions. | Runtime spec plus future API/product policy decision packets. | Public API/DX or product policy becomes a blocker for a scoped proof/migration. | Externality/design phase or Parent-Repo Migration phase. |
| Native host telemetry/error mapping | Requires production host semantics and product observability policy. | Telemetry/error mapping decision packet; runtime spec remains boundary authority. | Production host integration or product telemetry policy opens. | Externality/design or Parent-Repo Migration phase. |
| Durable async semantics | This child is server/Elysia-only; Inngest durable scheduling/retry/replay/idempotency/run history remain outside this proof. | Runtime spec and future Inngest/durable async workstream. | A child explicitly opens durable Inngest semantics or production async host behavior. | Externality/design phase or Parent-Repo Migration phase. |
...
- `bun test
  tools/runtime-realization-type-env/test/oracle/phase-three-contained-elysia-listen-lifecycle-passage.test.ts
  tools/runtime-realization-type-env/test/oracle/migration-control-plane-observation.test.ts`:
  pass.
- `bunx nx run runtime-realization-type-env:typecheck`: pass.
- `bunx nx run runtime-realization-type-env:oracle`: pass.
...
- a real local Elysia/Bun listener can start on `127.0.0.1` with an ephemeral
  port around the existing contained Elysia -> oRPC -> oracle path;
- a direct network `globalThis.fetch(...)` request reaches `/rpc/invoke` and
  delegates through Elysia host, oRPC Fetch, Oracle server harness, adapter, and
...
It does not prove production HTTP readiness or any Parent-Repo Migration claim.
...
- Required first reads:
  ...
  - `workstream-2026-05-01-phase-three-layer-disagreement-failure-observation-proof.md`;
  - `workstream-2026-05-01-phase-three-contained-elysia-host-passage.md`;
  - manifest, diagnostic, spine map, vendor notes, current lab state, and DRA
    workflow reference.
- First action:
