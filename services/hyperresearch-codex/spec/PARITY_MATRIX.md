...
| Claude `PreToolUse` source reminder | Codex `PreToolUse` source-bypass guard after fixture proof | guarded-workaround | Only direct installed Claude Hyperresearch hook analogue; prove actual tool names, stdin payload, block/allow behavior, and transcript-visible feedback. Service source capture remains authoritative |
...
| Final closure guard | Codex `Stop` guard after fixture proof | guarded-workaround | Codex-native guardrail, not installed Claude Hyperresearch hook parity. Can block final answer until `validate` is green; service validation remains authoritative |
