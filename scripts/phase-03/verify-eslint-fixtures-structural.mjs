...
const negativePluginPath = path.join(
  root,
  "tools",
  "eslint-fixtures",
  "negative-plugin-imports-plugin.ts"
);
const negativeServicePath = path.join(
  root,
  "tools",
  "eslint-fixtures",
  "negative-service-imports-app.ts"
);
const negativePackagePath = path.join(
  root,
  "tools",
  "eslint-fixtures",
  "negative-package-imports-service.ts"
);
const positiveServicePath = path.join(
  root,
  "tools",
  "eslint-fixtures",
...
const [negativePluginSource, negativeServiceSource, negativePackageSource, positiveServiceSource] =
  await Promise.all([
...
if (
  ...
  console.error(
    "eslint-fixtures structural failed: fixture sources must not encode project tags directly."
  );
  process.exit(1);
}
...
if (!negativeServiceSource.includes("../../apps/server/src/index.ts")) {
  console.error(
    "eslint-fixtures structural failed: app-negative fixture must keep the app import edge."
  );
  process.exit(1);
}
...
if (!negativePackageSource.includes("../../services/hq-ops/src/index.ts")) {
  console.error(
    "eslint-fixtures structural failed: package-negative fixture must keep the service import edge."
  );
  process.exit(1);
}
...
if (!positiveServiceSource.includes("@rawr/core")) {
  console.error(
    "eslint-fixtures structural failed: positive package fixture must keep the package import edge."
  );
  process.exit(1);
}
