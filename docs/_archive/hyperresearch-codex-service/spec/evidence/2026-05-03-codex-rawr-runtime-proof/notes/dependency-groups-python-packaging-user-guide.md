---
title: Dependency Groups - Python Packaging User Guide
id: dependency-groups-python-packaging-user-guide
created: '2026-05-03T04:00:21.819144Z'
source: https://packaging.python.org/en/latest/specifications/dependency-groups/
source_domain: packaging.python.org
fetched_at: '2026-05-03T04:00:21.818896Z'
...
requirements in
pyproject.toml
files such that they are not included in
...
Examples
¶
This is a simple table which shows
docs
and
test
groups:
...
and a similar table which defines
docs
,
test
, and
...
Table
¶
Dependency groups are defined as a table in
pyproject.toml
named
dependency-groups
. The
...
requirements.
[dependency-groups]
keys, sometimes also called “group names”, must be
valid non-normalized names
. Tools which handle Dependency
Groups MUST
normalize
these names before
comparisons.
...
error.
Requirement lists, the values in
[dependency-groups]
, may contain strings,
tables (
dict
in Python), or a mix of strings and tables. Strings must be
valid
...
group.
An include is a table with exactly one key,
"include-group"
, whose value is
...
package metadata. This means that sdist
PKG-INFO
and wheel
METADATA
...
metadata, and
pyproject.toml
files included in sdists will still contain
[dependency-groups]
. However, the table’s contents are not part of a built
...
dependency groups.
Tools SHOULD NOT eagerly validate the contents of
all
dependency groups
...
the
foo
group to be used and only error if the
bar
group is used:
[dependency-groups]
foo
...
(
f
"Cyclic dependency group include:
{
group
}
->
...
TypeError
(
"Dependency Groups table is not a dict"
)
if
not
isinstance
...
TypeError
(
"Dependency group name is not a str"
)
return
_resolve_dependency_group
(
...
History
¶
October 2024: This specification was approved through
PEP 735
.