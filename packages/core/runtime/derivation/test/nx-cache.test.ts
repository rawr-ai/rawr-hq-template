...
const FIXTURE_PREFIX = "runtime-derivation-nx-cache-";
...
test("restores an unchanged build and invalidates on a relevant project input", async () => {
  const fixtureRoot = await createFixture();
  const fixtureOwnerRoot = path.join(fixtureRoot, derivationPath);
...
async function createFixture(): Promise<string> {
  ...
  try {
    for (const relativePath of [schemaPath, definitionPath, derivationPath]) {
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-derivation:build", "--outputStyle=static"],
      { cwd: fixtureRoot, env }
    );
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const timer = setTimeout(() => {
      timeoutError = new Error("runtime-derivation:build exceeded its cache-proof timeout.");
      child.kill("SIGKILL");
    }, 45_000);
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-derivation:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-derivation Nx fixture: ${root}`);
  }
