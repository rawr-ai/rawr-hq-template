...
assertNotMatches(
  orpcSource,
  /"\/api\/workflows\/\*"/,
  "orpc.ts must not own /api/workflows route family"
);
const hasInlineRpcPolicyForRoot = /app\.all\(\s*"\/rpc"[\s\S]*?isRpcRequestAllowedWithDedupe/m.test(
  orpcSource
);
const hasInlineRpcPolicyForWildcard =
  /app\.all\(\s*"\/rpc\/\*"[\s\S]*?isRpcRequestAllowedWithDedupe/m.test(orpcSource);
const hasCentralizedRpcPolicy =
...
const hasWildcardRpcRouteHelper = /app\.all\(\s*"\/rpc\/\*"[\s\S]*?handleRpcRoute\(\{/m.test(
  orpcSource
);
