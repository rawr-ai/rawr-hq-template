...
async function assertInstalledServiceConsumer(nx: string, fixturePath: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "  workflows: [workflow],",
      "})();",
      'const admissionClient = defineRuntimeResource<"installed.admission-client", WorkflowEventSender>({ id: "installed.admission-client", title: "Admission", purpose: "Peer-free event submission contract" });',
      'const otherWorkflow = defineWorkflow({ ...workflow, id: "other-workflow" });',
      'const admissionRequirement = requireResource({ resource: admissionClient, instance: "east", reason: "Named workflow admission" });',
      "const workflowUses = { submissions: useWorkflowDispatcher(workflowPlugin, { workflows: [workflow], client: admissionRequirement }) };",
      "const admissionApi = defineServerApiPlugin.factory()({",
      '  capability: "installed-admission", routeBase: "/admission", services, workflows: workflowUses,',
      "  api: () => publicRouter,",
      "})();",
      "const admissionInternal = defineServerInternalPlugin.factory()({",
      '  capability: "installed-admission", routeBase: "/admission-internal", services, workflows: workflowUses,',
      "  internal: () => internalRouter,",
      "})();",
      "function admissionTypeProof(context: ServerPluginContext<typeof services, typeof workflowUses>) {",
      '  const sent = context.workflows.submissions.send(workflow, { value: "accepted" }, { id: "source-event" });',
      "  const result: Promise<WorkflowDispatchResult> = sent;",
      "  // @ts-expect-error Workflow payload remains schema-inferred.",
      "  context.workflows.submissions.send(workflow, { value: 1 });",
      "  // @ts-expect-error An undeclared dispatcher name is unavailable.",
      "  context.workflows.other.send(workflow, { value: 'accepted' });",
      "  // @ts-expect-error Native event options require a string id.",
      "  context.workflows.submissions.send(workflow, { value: 'accepted' }, { id: 1 });",
      "  // @ts-expect-error Schedule definitions are not workflow admission targets.",
      "  context.workflows.submissions.send(schedule, { value: 'accepted' });",
      "  // @ts-expect-error Workflows outside the selected target cannot be sent.",
      "  context.workflows.submissions.send(otherWorkflow, { value: 'accepted' });",
      "  // @ts-expect-error Target membership is checked while declaring the use too.",
      "  useWorkflowDispatcher(workflowPlugin, { workflows: [otherWorkflow], client: admissionRequirement });",
      "  // @ts-expect-error A dispatcher must select a nonempty workflow subset.",
      "  useWorkflowDispatcher(workflowPlugin, { workflows: [], client: admissionRequirement });",
      "  // @ts-expect-error Optional client requirements cannot back admission.",
      "  useWorkflowDispatcher(workflowPlugin, { workflows: [workflow], client: requireResource({ resource: admissionClient, reason: 'optional', optional: true }) });",
      "  return result;",
      "}",
      "void admissionTypeProof;",
      ...
      '  Assert<Equal<keyof typeof admissionApi.workflows, "submissions">>,',
      '  Assert<Equal<keyof typeof admissionInternal.workflows, "submissions">>,',
      '  Assert<Equal<keyof WorkflowDispatchResult, "eventIds">>,',
      '  Assert<Equal<WorkflowDispatchResult["eventIds"], readonly string[]>>,',
