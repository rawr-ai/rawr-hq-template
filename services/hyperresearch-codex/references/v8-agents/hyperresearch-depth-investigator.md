...
1. **Start with the vault.** Before fetching anything new, read the notes
   ...
   `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch note show <id1> <id2> <id3> --json`
...
3. **Fetch new sources via the fetcher subagent.** Do NOT call
   `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch fetch` directly. Delegate to `hyperresearch-fetcher` via the
...
5. **Read the fetched sources.** Use `/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch note show <id> -j`. Quote
...
6. **Write ONE interim report note.** This is your single deliverable.
   ...
   **BEFORE calling `note new`**, check if an interim note for this locus
   ...
   ```bash
   /Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch search "" --tag locus-<locus-name> --type interim --json
   ```
...
```bash
/Users/mateicanavra/.local/pipx/venvs/hyperresearch/bin/hyperresearch note new "Interim report — <locus name>" \
  --tag <corpus_tag> \
