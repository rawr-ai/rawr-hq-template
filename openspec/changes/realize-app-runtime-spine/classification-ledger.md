...
| Complete-derivation contract and binding-source authority closure | Task 4.7a is an authority-only documentation gate with no Nx or source target | Across its only allowed edit scope, `docs/system/HABITAT_ARCHITECTURE.md`, `docs/system/HABITAT_RUNTIME_REALIZATION.md`, `openspec/changes/realize-app-runtime-spine/authority-amendment.md`, `openspec/changes/realize-app-runtime-spine/classification-ledger.md`, `openspec/changes/realize-app-runtime-spine/design.md`, `openspec/changes/realize-app-runtime-spine/execution-queue.md`, `openspec/changes/realize-app-runtime-spine/tasks.md`, and `openspec/changes/realize-app-runtime-spine/specs/app-runtime-realization/spec.md`, replace every canonical placeholder plus every illustrative or undefined public `RuntimeDerivationResult` and nested carrier with exact closed TypeBox-derived shapes; freeze exact producer/consumer and `ExecutionDescriptorTable` / `WebRouteModuleTable` method signatures plus finding, error/refusal, and deterministic ordering contracts without a public error API; and freeze exactly three runtime values (`deriveRuntimeArtifacts`, `PortableRuntimePlanArtifactSchema`, `decodePortableRuntimePlanArtifact`) and the finite type-only inventory `RuntimeDerivationInput`, `RuntimeDerivationResult`, `NormalizedPluginIdentity`, `NormalizedSurfaceRequirement`, `NormalizedResourceRequirementIdentity`, `NormalizedRuntimeTopologyEdge`, `NormalizedRuntimeTopology`, `NormalizedAppDefinition`, `NormalizedPluginDefinition`, `DerivedRoleSurfaceIndex`, `NormalizedServiceUse`, `NormalizedServiceDependency`, `NormalizedSemanticDependency`, `ResourceRequirement`, `ProviderSelection`, `NormalizedRuntimeProfile`, `ServiceBindingPlan`, `SurfaceRuntimePlan`, `WorkflowDispatcherDescriptor`, `ExecutionDescriptorRef`, `ExecutionDescriptorTable`, `WebRouteModuleRef`, `WebRouteModuleTableEntry`, `WebRouteModuleTable`, `PortableRuntimePlanArtifact`, and `DerivationFinding`. Also settle the scope/config authoring owner and declaration site; exact closed TypeBox source and normalized-ref unions; sealed `ServiceUse` carrier evolution; required/forbidden present/absent-schema bindings; transitive `serviceDep` propagation/override; `RuntimeConfigSource` order/key/missing semantics; normalization, identity, cache, and public/private projection; and rejection of callbacks, closures, executable resolvers, provider acquisition, and live values. Create or edit no implementation, project, blueprint, SDK edge, export map, or public export. | Any unresolved canonical placeholder or illustrative carrier, unspecified signature or ordering/refusal rule, canonical-to-active disagreement, open export inventory, placeholder discriminant, public error API, out-of-scope edit, implementation/project/blueprint/SDK-edge/export-map/public-export change, or attempt to start task 4.8 or archive before the complete contract is deterministic and both canonical documents and all six active OpenSpec authority artifacts agree fails task 4.7a. |
| Complete derivation and deployment-cold plan | At task 4.8, **co-land** independent no-fallback `runtime-derivation@2`, `nx run runtime-derivation:acceptance:deployment-cold-plan`, and the installed-package extension | Implement the frozen task-4.7a contract verbatim rather than inventing it. Inspect the retained exact root shell, exact declared version-2 source/test closure, no version 1 edit/inheritance/fallback, and no version 3. Through sole installed `@habitat-ai/sdk/runtime/derivation`, call the one derivation operation `deriveRuntimeArtifacts({ entrypoint, profileId })`; observe only `{ topology, graph, executionDescriptorTable, webRouteModuleTable, portableArtifact }`, one private topology call, `graph.topology === topology`, recursive freeze, no executable calls, and the complete accepted binding/graph/reference artifacts. Observe exactly the three frozen runtime values and the exact finite type-only inventory from task 4.7a, with no public error API. Distinguish private `plugin.web-surface(surfaceId)` Effect work from the lazy web loader. Roundtrip the exact seven-field artifact through its public schema/decoder and verify `artifactId` is exactly `sha256:` plus 64 lowercase hexadecimal SHA-256 characters for RFC 8785 canonical JSON of the other six fields. | Wrong source/test closure, invention or widening of the frozen contract, inheritance/fallback/v3, extra runtime value or type-only export, public error API, extra public result or portable field, topology reconstruction/nonidentity, mutable result, executable call, malformed/mismatched artifact id, incomplete ref ordering, loader-as-Effect classification, early web Effect face, portable table/live state, unresolved workspace dependency, or source/installed divergence fails task 4.8. |
