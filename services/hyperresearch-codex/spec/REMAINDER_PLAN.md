...
- 2026-05-03: Re-ran the app-server explicit-child-resume smoke after the Codex/RAWR status-seeding repair. Cold parent `thread/resume` plus explicit `resume_agent` recovered the original child id to `Completed`, `wait` returned without timeout, and `closeAgent` observed previous status `Completed`. This is runtime recovery evidence for known child ids after parent resume; it does not claim bare parent resume automatically rehydrates descendants, and it is not the Hyperresearch service parity closure.
- 2026-05-03: Closed Hyperresearch service plus packet-orchestration parity through ledgered replacement attempts for cold-resumed pending child attempts that classify non-clean. Replacement packet outputs preserve the original attempt classification, complete the same logical packet job through validated artifact/source/provenance gates, and avoid claiming native clean child-handle resume parity.
