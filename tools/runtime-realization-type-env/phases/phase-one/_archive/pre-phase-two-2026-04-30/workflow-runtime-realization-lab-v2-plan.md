...
V2 must become a contained Oracle model of the accepted runtime realization spine:
...
- Add a Oracle that models derivation, compilation, provisioning, registry assembly, invocation, provider lowering, process-local resources, host adapter callback lowering, diagnostics, and deployment handoff.
...
The lab remains separate from parent repo production surfaces. It is a proving ground for spec finalization and Parent-Repo Migration planning, not the parent SDK/runtime, and not a shortcut around Parent-Repo Migration authorization.
...
| Oracle/evidence reviewer | Current lab project, scripts, fixtures, report | V2 layer model, target names, red-team risks |
...
tools/runtime-realization-type-env/
  evidence/
    ...
    workflow-runtime-realization-lab-v2-plan.md
  src/
    sdk/
      effect.ts
      runtime/
...
| `src/oracle/**` | Oracle managed runtime, registry, provider lowering, invocation, adapter lowering | Bun runtime tests |
...
Vendor facts to encode in `vendors/README.md` during implementation:
...
| Claim | Required dependency | Oracle component | Evidence shape | Target status |
| --- | --- | --- | --- | --- |
...
| Execution registry pairs compiled plans with descriptor table entries before invocation | TypeScript, Effect | `src/oracle/process-runtime.ts` | Runtime tests for match, mismatch, duplicate, missing descriptor | `simulation-proof` |
| `ProcessExecutionRuntime` runs real Effect values through runtime-owned execution access | `effect@3.21.2` | `src/oracle/managed-runtime.ts`, `process-runtime.ts` | Runtime tests using `runPromiseExit`, failure, defect, interruption | `vendor-proof` |
| Provider build returns `ProviderEffectPlan`, not `CompiledExecutionPlan` or Promise | TypeScript, Effect | `src/sdk/runtime/providers`, `src/oracle/provider-lowering.ts` | Typecheck, negative tests, runtime acquire/release smoke | `xfail` until plan shape is locked |
| Provider lowering handles acquire/release, scope, finalizers, rollback, diagnostics | `effect@3.21.2` | `src/oracle/provider-lowering.ts` | Runtime finalizer/release order tests | `xfail` until `ProviderEffectPlan` fields are locked |
| Process-local queue/pubsub/ref/deferred/schedule/stream resources are local runtime infrastructure only | `effect@3.21.2` | `src/oracle/process-resources.ts` | Runtime coordination tests plus negatives against durable/cross-process claims | `vendor-proof` once facade law is locked |
| `RuntimeResourceAccess` is narrow and does not expose runtime internals | TypeScript, Effect | `src/sdk/runtime/resources`, `src/oracle/process-resources.ts` | Positive require/optional fixture plus raw access negatives | `xfail` until method law is locked |
| Workflow dispatcher access is explicit, not ambient unless the spec deliberately says otherwise | TypeScript, Inngest shape | `src/sdk/plugins/server`, `src/oracle/adapters/internal.ts` | Positive opt-in fixture plus ambient-access negative | `xfail` until dispatcher declaration is locked |
| Async step bodies are statically declared and host invocation lowers into process runtime | TypeScript, Effect, Inngest shape | `src/sdk/plugins/async`, `src/oracle/adapters/async.ts` | Positive step invocation test plus membership experiment | `xfail` until workflow/schedule/consumer-to-step association is locked |
| Server/internal/CLI/agent/desktop/web adapters do not execute effects themselves | TypeScript, selected real boundary deps | `src/oracle/adapters/**` | Adapter smoke tests asserting registry delegation | `simulation-proof` |
...
| Deployment consumes compile-only runtime handoff and never starts/provisions/mounts runtime | TypeScript | `src/oracle/deployment-handoff.ts` | Positive handoff fixture plus negative live-handle leakage fixture | `proof` |
...
- Update `vendors/README.md` to distinguish previous pseudo-Effect proof from new vendor-native proof.
...
- Add internal raw Effect imports only under `src/vendor/effect/**` and `src/oracle/**`.
...
- Use real `Layer`, `Scope`, and finalizers in the Oracle where the experiment is explicit.
...
- Add adapter Oracle components that receive native callbacks and delegate to `ProcessExecutionRuntime`.
...
- Update `report-results.ts` so the report separates:
  ...
  - todo;
  - out-of-scope.
- Keep one `current-lab-state.md`; do not add owner/date/kanban workflow.
...
| `oracle` | Run Oracle process runtime/provider/resource/adapter tests | Yes after Phase C |
...
- The Oracle executes descriptors in adapters instead of delegating through the registry and process runtime.
...
- Oracle tests for descriptor derivation, registry assembly, invocation context binding, provider lowering, process-local resources, adapter callback lowering, async bridge lowering, diagnostics, and deployment handoff.
...
- The Oracle executes real Effect descriptors through runtime-owned access.
