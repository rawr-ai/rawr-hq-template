...
- `simulation-proof` design when a RAWR-owned mini-runtime adapter consumes the
  shape under a gate;
...
- `tools/runtime-realization-type-env/src/mini-runtime/catalog.ts`
...
- In the lab, this can support the contained oRPC Fetch boundary only after the
  RAWR mini-runtime harness delegates to `ProcessExecutionRuntime`.
...
- The mini-runtime lab currently proves deterministic OTLP-shaped trace payload
...
- keep RAWR ownership at lifecycle, runtime construction, import law,
  ...
  finalization seams;
- avoid a custom RAWR mini-language where a vendor already has a strong native
  idiom;
...
- Keep parent-app telemetry and mini-runtime telemetry separate.
