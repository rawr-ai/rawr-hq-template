...
This directory is the durable execution packet for M2 / Phase 2 of the architecture migration. Treat it as the live M2 session packet, not as an extension of the closed M1 packet.
...
- `../../resources/spec/m2-guardrails-and-enforcement.md`: active M2 guardrails and gate classification

## Historical Coordination

`handoffs/` is reserved for takeover and peer-coordination artifacts that become useful during M2 execution but should not live in the first-hop lane.
...
`notes/` holds retained M2 investigations, slice notes, and carry-forward risks that matter but are more specific than the live packet.

The first retained note in this packet is the carried-forward HQ Ops runtime-verification risk from Phase 1.

## Starting Posture

M2 does not begin from a cleanup swamp. It begins from a frozen Phase 1 plateau:
...
The first deliberate M2 move is to replace that bridge with the canonical runtime path.
