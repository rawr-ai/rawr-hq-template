...
export function createResolveCurrentMain(
  dependencies: ResolveCurrentMainDependencies,
): (input: ResolveCurrentMainInput) => Promise<CurrentMainResolution> {
  const validateAcceptance = createValidateGovernedAcceptance(dependencies);
