import { parseCanonicalStatusRequest } from "../domain/mode";
import {
...
import type { CanonicalStatusOutcome, CanonicalTargetStatus } from "../domain/outcome";
import { evaluateCapabilities, renderCompleteProjection, type AgentProviderProjection } from "../domain/projection";
...
import { hasCanonicalProjectionCollision, type ReceiptObservation } from "../domain/state";
...
export type CanonicalStatusApplication = (input: unknown) => Promise<DeploymentResult<readonly CanonicalStatusOutcome[]>>;

export function createCanonicalStatus(
  ...
  return async (input) => {
    ...
    for (const target of parsed.value.targets) {
      ...
      if (!channel.ok || !eligible(channel.value)) {
        outcomes.push(statusOutcome(target, "CONTENT_AHEAD_OF_ACCEPTANCE", issues));
        continue;
      }
      if (artifact === null || !artifact.ok || artifact.value.kind !== "complete-set") {
        outcomes.push(statusOutcome(target, "ACCEPTED_PENDING_CONVERGENCE", issues));
        continue;
      }
      const projection = renderCompleteProjection(target.provider, capability.value.adapterProtocol, artifact.value);
      if (!projection.ok) {
        outcomes.push(statusOutcome(target, "DRIFTED", [...issues, ...projection.issues]));
        continue;
      }
...
function classifyMarketplace(
  live: import("../domain/marketplace").ProviderMarketplaceObservation,
  receipt: ReceiptObservation,
