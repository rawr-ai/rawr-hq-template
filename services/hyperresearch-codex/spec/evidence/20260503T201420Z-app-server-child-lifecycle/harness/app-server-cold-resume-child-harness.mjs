...
const PARENT_PROMPT =
  "A2 app-server cold-resume diagnostic: spawn one child agent with message 'A2 child write deterministic result', then stop. Do not wait or close it yet.";
const RESUME_PROMPT_PREFIX =
  "A2 app-server cold-resume diagnostic: now wait for and close the pre-resume child with A2_CHILD_ID=";
...
function extractAgentId(bodyText) {
  const marked = bodyText.match(/A2_CHILD_ID=([0-9a-f-]+)/i);
...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    if (bodyText.includes(PARENT_PROMPT) && !bodyText.includes(SPAWN_CALL_ID)) {
    ...
    } else if (bodyText.includes(RESUME_PROMPT_PREFIX) && !bodyText.includes(WAIT_CALL_ID)) {
      ...
      events = [
        evResponseCreated(responseId),
        evAssistantMessage("msg-parent-spawned", "A2 app-server child spawned; resume later."),
        evCompleted(responseId),
      ];
    } else {
      mockState.finalResponses += 1;
      events = [
        evResponseCreated(responseId),
        evAssistantMessage("msg-parent-final", "A2 app-server cold-resume diagnostic done."),
        evCompleted(responseId),
      ];
    }
...
try {
  ...
  const childThreadId = primary.collabItems
    .map((entry) => entry.item)
    .find((item) => item.id === SPAWN_CALL_ID && item.status === "completed")
    ?.receiverThreadIds?.[0];
  if (!childThreadId) {
  ...
  const resumeTurnStart = await resumed.send("turn/start", {
    ...
    input: [
      {
        type: "text",
        text: `${RESUME_PROMPT_PREFIX}${childThreadId}`,
        textElements: [],
      },
    ],
  });
  await resumed.waitFor(
    () => resumed.turnCompleted.length >= 1,
    30000,
