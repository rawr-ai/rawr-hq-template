...
describe("hq runtime commands", () => {
  ...
  it("writes stale HQ status once and prunes the dead state file", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: "status-fixture", private: true }, null, 2)
    );
    writeFileSync(
    ...
    expect(readFileSync(path.join(workspaceRoot, ".rawr", "hq", "status.json"), "utf8")).toContain(
      '"schemaVersion": 1'
    );
    expect(() =>
      readFileSync(path.join(workspaceRoot, ".rawr", "hq", "state.env"), "utf8")
    ).toThrow();
  });
  ...
  it("rejects invalid RAWR_HQ_OBSERVABILITY values before writing status", () => {
    ...
    expect(() =>
      readFileSync(path.join(workspaceRoot, ".rawr", "hq", "status.json"), "utf8")
    ).toThrow();
  });
  ...
  it("writes shell-managed status into the workspace root instead of creating mode-named directories", () => {
    ...
    writeFileSync(
      path.join(fixtureRoot, "scripts", "dev", "hq.sh"),
      readFileSync(path.join(cliRoot, "..", "..", "scripts", "dev", "hq.sh"), "utf8")
    );
    writeFileSync(
    ...
    const written = JSON.parse(
      readFileSync(path.join(fixtureRoot, ".rawr", "hq", "status.json"), "utf8")
    ) as {
