...
function service_v3_context_funnel_branch_status($filename, $branch) js {
...
function service_v3_context_funnel_service_status($service) js {
  const service = $service.text.replace(/\s+/g, "");
  return service === "impl" || /^impl(?:\.use\([A-Za-z_$][\w$]*\))+$/.test(service) ? "ok" : "wrong-root";
}

predicate service_v3_context_funnel_raw($name) {
  $name <: r"^(?:deps|scope|config|invocation|provided)$"
}

predicate service_v3_context_funnel_shape($statements) {
...
predicate service_v3_context_funnel_native($statements, $name, $source) {
  or {
...
or {
  program(statements=$statements) where {
    ...
    not {
      service_v3_context_funnel_shape(statements=$statements),
      service_v3_context_funnel_native(statements=$statements, name=$orpc, source=`"@orpc/server"`),
  ...
  program(statements=$statements) where {
    ...
    not {
      ...
      service_v3_context_funnel_native(statements=$statements, name=$implement, source=`"@orpc/server"`),
      $statements <: contains `export const impl = $implement(contract).$context<Context>()`,
      $statements <: contains `export const service = $service`,
      $service_status = service_v3_context_funnel_service_status(service=$service),
  ...
  program(statements=$statements) where {
    ...
    not {
      ...
      $statements <: contains or {
        ...
        $status = service_v3_context_funnel_branch_status(filename=$filename, branch=$branch),
        $status <: includes "ok",
  ...
  `context.$name` where {
    ...
    service_v3_context_funnel_raw(name=$name)
  }
}
```
