...
const FIXTURE_PREFIX = "runtime-mounting-nx-cache-";
const workspaceRoot = fileURLToPath(new URL("../../../../..", import.meta.url));
const mountingPath = "packages/core/runtime/mounting";
const harnessPath = "packages/core/runtime/harnesses";
const processPath = "packages/core/runtime/process-runtime";
const substratePath = "packages/core/runtime/substrate/effect";
const bootgraphPath = "packages/core/runtime/bootgraph";
const compilerPath = "packages/core/runtime/compiler";
const derivationPath = "packages/core/runtime/derivation";
...
test("restores an unchanged mounting build and invalidates owner and upstream inputs", async () => {
  const fixtureRoot = await createFixture();
  const ownerRoot = path.join(fixtureRoot, mountingPath);
  ...
  try {
    ...
    const upstreamConfig = path.join(fixtureRoot, harnessPath, "tsdown.config.ts");
    await writeFile(
      upstreamConfig,
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-mounting:build", "--outputStyle=static"],
      { cwd: fixtureRoot, env }
    );
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const timer = setTimeout(() => {
      timeoutError = new Error("runtime-mounting:build exceeded its cache-proof timeout.");
      child.kill("SIGKILL");
    }, 45_000);
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-mounting:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-mounting Nx fixture: ${root}`);
  }
