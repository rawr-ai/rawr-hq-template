...
assertCondition(
  telemetrySource.includes("OTLPMetricExporter") &&
    telemetrySource.includes("PeriodicExportingMetricReader"),
...
assertCondition(
  telemetryTestSource.includes(
    "fails loudly when telemetry is re-installed with incompatible metrics options"
  ),
  "telemetry.test.ts must cover incompatible metrics options"
);
assertCondition(
  telemetryTestSource.includes(
    "derives trace and metrics OTLP HTTP endpoints from the base OTLP endpoint"
  ),
