...
| Proof target | Contained `simulation-proof` for a real Elysia app/request route-forwarding host around the oracle server boundary. |
...
| Durable report disposition | If this child discovers useful Elysia integration rules for later runtime design, update `vendors/README.md` or create a labeled reference report. Otherwise keep findings inside this child report. |

## Output Contract

Required executable proof outputs:

- A focused oracle test or test cluster that proves the selected Elysia
  host passage.
...
| Runtime/source evidence | complete | DRA disposition: accepted. The least invasive adapter home is `tools/runtime-realization-type-env/src/oracle/adapters/`, beside `orpc-server.ts`, wrapping `StartedRuntimeOrpcServerBoundary` and leaving production `apps/*` untouched. |
...
- The Elysia host delegates to the existing contained
  ...
  `@orpc/server/fetch` `RPCHandler`, the Oracle server harness, adapter
...
| Move the proof into `apps/server` | Would stop being a contained Runtime Realization Lab proof and risk incidental production surface changes. |
...
- `package.json`
  - Adds root/lab `elysia: ^1.3.12` so the Runtime Realization Lab can import the
    real vendor package honestly.
- `tools/runtime-realization-type-env/src/oracle/adapters/elysia-host.ts`
...
- `tools/runtime-realization-type-env/src/oracle/index.ts`
  - Exports the lab-only Elysia host adapter.
- `tools/runtime-realization-type-env/test/oracle/phase-three-contained-elysia-host-passage.test.ts`
...
- Success path:
  ...
  - oRPC records `fetch.received`, `handler.enter`, `handler.finished`, and
    `fetch.matched`;
  - the Oracle server harness delegates into `ProcessExecutionRuntime`;
...
- `bunx nx run runtime-realization-type-env:oracle`: passed; 67 tests.
...
| Mechanical/repo/Nx | Leibniz `019de1d9-cde0-7680-b588-a6db8ac97744` | Mechanical fail for closure, pass for implementation shape | Manifest/vendor-fidelity were stale and manifest gates must use known target names, not raw commands or aggregate `gate`. | Accepted and repaired. Manifest promoted with valid gates: `typecheck`, `oracle`, `structural`, `report`; vendor-fidelity updated. |
...
| Dependency evidence can lag behind repaired package resolution. | Updated `vendors/README.md` after adding root/lab `elysia`. | Vendor dependency repairs must update vendor-fidelity in the same child closeout. | Vendor/integration inheritance and mechanical review. | Accepted. |
...
- The lab now proves a real contained Elysia app/request route-forwarding
  passage into the existing contained oRPC Fetch boundary and oracle
  server path.
- The proof crosses:
...
- The proof does not prove production `listen(...)`, production port/server
  ...
  native host telemetry/error mapping, public API/DX law, Parent-Repo Migration
...
- Active question:
  Can the Runtime Realization Lab start a real local Elysia listener around the same
...
- Required first reads:
  ...
  - `src/oracle/adapters/elysia-host.ts`;
  - `test/oracle/phase-three-contained-elysia-host-passage.test.ts`.
- Initial stop rules:
  ...
  - stop if this child's contained host proof is described as Lab-Production
...
- Superseded by:
  ...
  live-passage closeout. Phase Three is now closed below Lab-Production Proof.
