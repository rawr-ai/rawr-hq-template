...
export function decodeMechanicalProviderEvidence(
  ...
  if (!(bytes instanceof Uint8Array) || bytes.byteLength > MAX_EVIDENCE_BYTES) {
    return failure([
      issue("EVIDENCE_FAILED", "evidence", "Evidence must be bounded canonical bytes"),
    ]);
  }
  let input: unknown;
  try {
  ...
  const evidenceDigest = canonicalDigest(
    "me1_",
    evidenceBodyValue(body.value)
  ) as MechanicalEvidenceDigest;
  if (expectedDigest !== undefined && evidenceDigest !== expectedDigest) {
    return failure([
      issue(
        "INVALID_DIGEST",
        "evidence",
        "Evidence digest differs from the requested immutable handle",
        expectedDigest,
        evidenceDigest
      ),
    ]);
...
function parseBody(input: unknown): DeploymentResult<MechanicalProviderEvidenceBody> {
  ...
  if (
    ...
    return failure(
      firstIssue(issues, issue("EVIDENCE_FAILED", "evidence", "Evidence body is invalid"))
    );
  }
  ...
  if (input.schemaProtocol !== PROVIDER_EVIDENCE_SCHEMA_PROTOCOL)
    issues.push(
      issue(
        "INVALID_PROTOCOL",
        "evidence.schemaProtocol",
        "Unsupported evidence protocol",
        PROVIDER_EVIDENCE_SCHEMA_PROTOCOL,
        String(input.schemaProtocol)
      )
    );
  ...
  const evaluationProfile = canonicalString(
    input.evaluationProfile,
    "evidence.evaluationProfile",
    issues,
    { maxBytes: 256, pattern: ID_PATTERN, code: "INVALID_EVALUATION_PROFILE" }
...
): MechanicalEvidenceSource | undefined {
  if (
    input === null ||
    typeof input !== "object" ||
    Array.isArray(input) ||
    ...
    issues.push(
      issue("EVIDENCE_FAILED", "evidence.source", "Evidence source must be a closed object")
    );
    return undefined;
  }
    ...
    for (const candidate of raw ?? []) {
      ...
      if (!parsed.ok || parsed.value.kind !== "release")
        issues.push(
          issue(
            "INVALID_ARTIFACT_REF",
            "evidence.source.releases",
            "Targeted evidence requires release refs"
          )
        );
  ...
  if (kind === "complete-test") {
    ...
    if (!parsed.ok || parsed.value.kind !== "complete-set") {
      issues.push(
        issue(
          "INVALID_ARTIFACT_REF",
          "evidence.source.releaseSet",
          "Complete evidence requires one complete-set ref"
        )
      );
      return undefined;
    }
  ...
  if (
    ...
    issues.push(
      issue(
        "EVIDENCE_FAILED",
        "evidence.procedures",
        "Evidence procedures must be the exact supported verification sequence"
      )
    );
    return undefined;
  }
...
function parseFacts(
  input: unknown,
  issues: ProviderDeploymentIssue[]
): readonly ProviderVerificationFact[] | undefined {
  ...
  for (let index = 1; index < facts.length; index += 1) {
    if (facts[index - 1]?.targetDigest === facts[index]?.targetDigest)
      issues.push(
        issue("DUPLICATE_TARGET", "evidence.targets", "Evidence targets must be distinct")
      );
  }
...
function parseFact(
  ...
  path: string,
  issues: ProviderDeploymentIssue[]
): ProviderVerificationFact | undefined {
  if (
    input === null ||
    typeof input !== "object" ||
    Array.isArray(input) ||
  ...
  const kind = (input as { readonly kind?: unknown }).kind;
  const finalKey =
    kind === "verified" ? "visibleFingerprint" : kind === "failed" ? "failureCodes" : null;
  if (
    finalKey === null ||
    !exactRecord(
      input,
    ...
    if (finalKey === null)
      issues.push(
        issue(
          "EVIDENCE_FAILED",
          `${path}.kind`,
          "Evidence fact kind is unsupported",
          "verified|failed",
          String(kind)
        )
      );
  ...
  if (provider === undefined)
    issues.push(
      issue("UNSUPPORTED_PROVIDER", `${path}.provider`, "Evidence provider is unsupported")
    );
  const targetDigest = parseDigest(input.targetDigest, `${path}.targetDigest`, "pt1_", issues) as
  ...
  const adapterProtocol = canonicalString(
    input.adapterProtocol,
    `${path}.adapterProtocol`,
    issues,
    { maxBytes: 256, pattern: PROTOCOL_PATTERN, code: "INVALID_PROTOCOL" }
  ...
  if (kind === "verified") {
    const fingerprint = parseDigest(
      ...
      `${path}.visibleFingerprint`,
      "vf1_",
      issues
    );
    return fingerprint === undefined
      ? undefined
      : Object.freeze({ kind, ...common, visibleFingerprint: fingerprint });
  }
  const codes = boundedArray(
    input.failureCodes,
    `${path}.failureCodes`,
    FAILURE_CODES.size,
    issues
  ...
  for (const code of codes ?? []) {
    if (typeof code !== "string" || !FAILURE_CODES.has(code as ProviderDeploymentIssueCode))
      issues.push(
        issue("EVIDENCE_FAILED", `${path}.failureCodes`, "Unknown provider failure code")
      );
  ...
  return codes === undefined
    ...
    : Object.freeze({
        kind: "failed",
        ...common,
        failureCodes: Object.freeze(failureCodes.sort(compareCanonical)),
      });
}

function parsePayloadDigests(
  input: unknown,
  path: string,
  issues: ProviderDeploymentIssue[]
): readonly string[] | undefined {
