...
class CoreOntologyTests(WorkbenchTestCase):
    def test_core_ontology_validates(self) -> None:
        ...
        self.assertGreaterEqual(report["summary"]["canonical_entity_count"], 3)

    def test_core_ontology_rejects_unknown_predicates(self) -> None:
        ontology = deepcopy(load_core_ontology(FIXTURE_ONTOLOGY_ROOT))
    ...
    def test_semantica_graph_probe_preserves_target_boundaries(self) -> None:
        ontology = load_core_ontology(FIXTURE_ONTOLOGY_ROOT)
        validation = validate_loaded_core_ontology(ontology)
        graph = build_graph_payload(ontology, validation)
        proof = graph["semantica_graph"]
        self.assertEqual("semantica-workbench-graph-proof-v1", proof["schema_version"])
        self.assertFalse(proof["candidate_handling"]["promotion_allowed"])
        self.assertTrue(proof["fallback"]["reviewed_input_id_authority"])
        self.assertTrue(proof["fallback"]["contract_predicate_authority"])
        guards = proof["workbench_guards"]
    ...
    def test_semantica_graph_probe_uses_contract_predicate_allow_list(self) -> None:
        ...
        self.assertFalse(proof["workbench_guards"]["controlled_predicates_preserved"])
    ...
    def test_fixture_authority_only_targets_verification_policy(self) -> None:
        ontology = load_core_ontology(FIXTURE_ONTOLOGY_ROOT)
        testing_authority_edges = [
            relation
            for relation in ontology["relations"]
            if relation["subject"] == "fixture.authority.review-guide"
    ...
    def test_canonical_source_refs_reject_archive_and_quarantine_provenance(self) -> None:
        ontology = deepcopy(load_core_ontology(FIXTURE_ONTOLOGY_ROOT))
        ontology["entities"][0]["source_refs"] = [
            {
                "path": "tools/semantica-workbench/fixtures/docs/sweep/archive/skipped.md",
                "section": "Archived Sweep Fixture",
            }
        ]
        report = validate_loaded_core_ontology(ontology)
        self.assertIn("canonical_source_ref_uses_provenance", {error["kind"] for error in report["errors"]})
    ...
    def test_testing_plan_diff_has_review_signal(self) -> None:
        ...
        validation = validate_loaded_core_ontology(ontology)
        graph = build_graph_payload(ontology, validation)
        diff = build_document_diff(fixture_document_path(), graph["layered_graph"], graph["candidate_queue"])
        self.assertEqual(rel(fixture_document_path()), diff["document"])
