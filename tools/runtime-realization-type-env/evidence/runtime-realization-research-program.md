...
| `audit.p1.provider-effect-plan-shape` | `xfail` | Acquire/release payload, diagnostics, error, telemetry, and lowering fields are not fully locked. | Manifest, diagnostic, todo fixture. | A minimal acquire/release-only lab shape can be accepted without locking retry, refresh, telemetry export, or public API. | Provider lowering needs a minimal lab plan shape. | ProviderEffectPlan -> Bootgraph/Provisioning Lowering | lab/spec |
| `audit.p1.effect-managed-runtime-substrate` | `xfail` | Vendor behavior is proven, but final RAWR-owned substrate contract remains open. | Manifest, spine map, todo fixture. | Provider provisioning needs a RAWR-owned managed-runtime contract beyond isolated vendor proof. | Provider provisioning needs managed runtime ownership beyond vendor proof. | ProviderEffectPlan -> Bootgraph/Provisioning Lowering | lab/spec |
...
| `audit.p1.provider-effect-plan-lowering` | `xfail` | Provider acquire/release must lower through bootgraph/provisioning, not process execution. | Manifest, diagnostic, todo fixture. | Minimal provider plan shape is chosen for the lab. | Bootgraph proof needs real provider acquire/release instead of fake modules. | ProviderEffectPlan -> Bootgraph/Provisioning Lowering | lab/spec |
...
| `audit.p2.runtime-profile-config-redaction` | `todo` | Typed config binding and diagnostic-safe secret emission are not locked. | Manifest, diagnostic, spine map. | Provider acquire/release emits config-bearing diagnostics or snapshots. | Provider acquire/release emits diagnostics or config snapshots. | Provider Diagnostics + Runtime Profile Config Redaction | lab/spec |
