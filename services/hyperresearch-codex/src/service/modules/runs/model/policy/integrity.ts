...
export type HyperresearchRunIntegrityObservations = {
  requiredArtifacts: ReadonlyArray<{
    stepId: string;
    artifact: string;
    exists: boolean;
  }>;
  acceptedAgentOutputs: ReadonlyArray<{
    jobId: string;
    path: string;
    text: string | null;
    sha256?: string;
  }>;
  claimTraceText?: string | null;
  claimReports: ReadonlyArray<{
    index: number;
    path: string;
    text: string | null;
  }>;
...
export function claimTraceReportPaths(
  claimTraceText: string | null
): Array<{ index: number; path: string }> {
  if (!claimTraceText) return [];
  let parsed: unknown;
  try {
    parsed = JSON.parse(claimTraceText);
  } catch {
    return [];
  }
  const paths: Array<{ index: number; path: string }> = [];
  for (const [index, claim] of claimTraceEntries(parsed).entries()) {
    if (!claim || typeof claim !== "object") continue;
    const reportLocation = (claim as Record<string, unknown>).reportLocation;
    if (typeof reportLocation !== "string" || reportLocation.length === 0) continue;
    const path = reportPathFromLocation(reportLocation);
    if (safeRelativePath(path)) paths.push({ index, path });
  }
  return paths;
}
...
function patchLogCoversReportChange(input: {
  patchLogText?: string | null;
  beforeSha256: string;
  afterSha256: string;
  snapshotText: string;
  ...
  if (!input.patchLogText) return false;
  let parsed: unknown;
  try {
    parsed = JSON.parse(input.patchLogText);
  } catch {
    return false;
  }
...
export function validateHyperresearchRunIntegrity(input: {
  ledger: HyperresearchRunLedger;
  observations: HyperresearchRunIntegrityObservations;
  ...
  for (const step of input.ledger.steps) {
    ...
    if (step.status === "complete") {
      for (const artifact of step.requiredArtifacts) {
        const observation = input.observations.requiredArtifacts.find(
  ...
  for (const job of input.ledger.agentJobs ?? []) {
    ...
    if (job.status === "complete" && job.acceptedOutputPath && job.acceptedOutputSha256) {
      const acceptedOutput = input.observations.acceptedAgentOutputs.find(
        (item) => item.jobId === job.id && item.path === job.acceptedOutputPath
      );
      if (!acceptedOutput || acceptedOutput.text === null) {
        findings.push({
          severity: "blocking",
        ...
        if (acceptedOutput.sha256 !== job.acceptedOutputSha256) {
          findings.push({
            severity: "blocking",
  ...
  if (input.ledger.version === 2 && input.ledger.completed) {
    ...
    const claimTraceText = input.observations.claimTraceText;
    if (!claimTraceText) {
      findings.push({
        severity: "blocking",
      ...
      if (parsedTrace !== undefined) {
        ...
        for (const [index, claim] of claims.entries()) {
          ...
          if (
            ...
            const reportText = input.observations.claimReports.find(
              (item) => item.index === index && item.path === reportPath
            )?.text;
            if (!reportText) {
              findings.push({
                severity: "blocking",
  ...
  if (patchGuard?.snapshotPath && patchGuard.snapshotSha256) {
    const currentReport = input.observations.patch?.currentReport;
    if (currentReport) {
      const currentSha = input.observations.patch?.currentSha256;
      if (currentSha !== patchGuard.snapshotSha256) {
        const snapshotText = input.observations.patch?.snapshotText;
        ...
        if (
          !patchLogCoversReportChange({
            patchLogText: input.observations.patch?.patchLogText,
