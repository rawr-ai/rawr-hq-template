...
import { canonicalBytes, canonicalDigest } from "@rawr/agent-plugin-lifecycle/ports/providers";
import {
...
import { failure, issue } from "@rawr/agent-plugin-lifecycle/ports/providers";
import { createTargetReceipt, visibleFingerprint, type VerifiedMemberIdentity } from "@rawr/agent-plugin-lifecycle/ports/providers";
import { createTargetIdentitySidecar, type NativeMemberObservation } from "@rawr/agent-plugin-lifecycle/ports/providers";
...
describe("node provider state", () => {
  ...
  it("replays a last-member retire from the stable projection archive and re-normalizes absence", async () => {
    ...
    const native = {
      ...
      setMarketplaceExact: async (input: Parameters<import("../../../../src/lib/agent-plugins/service-runtime/providers/node-state").NativeMemberRestorationPort["setMarketplaceExact"]>[0]) => {
      ...
      restoreExact: async (input: Parameters<import("../../../../src/lib/agent-plugins/service-runtime/providers/node-state").NativeMemberRestorationPort["restoreExact"]>[0]) => {
        restores += 1;
