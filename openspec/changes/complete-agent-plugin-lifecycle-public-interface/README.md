...
JavaScript check wrapper, SDK source, or worktree path remains in the gate. The
router packet runs cold in roughly 80-95 seconds and from the local Nx cache in
under 50 ms. Its
Nx input set covers every Git-visible tree the packet inspects, so adding a new
...
incomplete blueprint packet both miss the prior successful cache entry and
fail their owning rule.
Task 1.5 still owns the native packet-fixture gap.

The separately owned research SDK is not an input to this initiative while its
package fit is unresolved. Its writer lane must return one deletion-first,
peer-accepted disposition: pure TypeBox contracts and transformations may
remain a package, while resource acquisition, `Layer`/runtime construction,
and process-scoped execution must move behind a service and explicit
resource/provider boundaries.
