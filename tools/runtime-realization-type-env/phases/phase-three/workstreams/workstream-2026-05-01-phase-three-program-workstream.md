...
| Child 7 control decision | DRA accepted contained `simulation-proof` for the integrated contained Oracle live-passage rehearsal and closed Phase Three with residuals routed. |
| Active child | None. |
...
- Phase Three should make the runtime spine feel materially less speculative:
  an operator can inspect a contained Oracle runtime passage from selected
...
- Phase Three may use real installed vendor packages only through lab-contained
  probes, adapters, and oracle tests whose proof categories stay honest.
...
| Closeout | Reconcile proof, non-proof, residuals, and next phase. | Classify residuals into externality/design, structure/Nx/generator, and Parent-Repo Migration buckets. |
...
- Phase Two wording that over-claims Lab-Production Proof rather than contained
  production-critical evidence.
- Semantica or source-mining output as architecture authority.

Control inputs:

- Phase Three opened after explicit user approval on 2026-05-01.
- The DRA workflow under `../` was updated from this
...
- Phase Two closeout itself added no runtime behavior proof and did not
  authorize Parent-Repo Migration.
...
- Manifest, diagnostic, spine map, vendor maps, and current lab state updates only
...
- Primary target: contained `simulation-proof` for live-runtime-passage slices
  that cross RAWR-owned oracle boundaries with falsifiable gates.
...
- Coordination target: `out-of-scope` for program documents, handoffs, and DRA
  workflow artifacts.
- Phase Three closed below Lab-Production Proof. Future Reference Runtime work
  may attempt Lab-Production Proof inside the Lab with explicit gates.
...
- If a proof target needs an unaccepted public API/DX, vendor/product policy,
  ...
  only when the issue would force Parent-Repo Migration, false-green wording, or
  an unowned residual.
- Stop if a proposed green claim relies only on vendor or simulation evidence
  while wording implies Lab-Production Proof.
...
- Existing parent-app telemetry is evidence to mine, not automatic
  oracle authority. The oracle telemetry proof must name the Effect
...
- every child began by gathering information, analyzing it, assimilating the
  ...
  and executing;
- every promoted proof has a test oracle and named gate that would fail on
  regression;
...
- manifest, diagnostic, spine map, current lab state, vendor notes, and reports agree
  on proof strength;
...
- closeout residuals are classified into externality/design, final
  structure/Nx/generator, Parent-Repo Migration, or deliberately abandoned
...
- Temporary scratch path:
  `tools/runtime-realization-type-env/phases/phase-three/_scratch/<YYYY-MM-DD>-<child-slug>/<lane>.md`
...
- Closeout disposition must be one of: deleted as superseded, archived under
  `tools/runtime-realization-type-env/phases/phase-one/_archive/phase-three-scratch/`
...
- Comments are migration/documentation substrate only; they cannot promote
  manifest or diagnostic status without a falsifiable test oracle and named gate.
...
| --- | --- | --- | --- |
| 0. Post-approval DRA workflow fill | Convert approved program structure into the persistent DRA operating workflow. | Updated `../workflow-phase-three-program-dra.md`. | DRA can recover level map, authority inputs, current child, gates, and stop conditions after compaction. |
...
| --- | --- | --- | --- |
| Phase Two proved contained spine composition, mostly as `simulation-proof`, not Lab-Production Proof. | `tools/runtime-realization-type-env/evidence/proof-manifest.json`, `tools/runtime-realization-type-env/evidence/runtime-spine-verification-diagnostic.md`, `workstream-2026-04-30-phase-two-closeout-phase-three-handoff.md`, `tools/runtime-realization-type-env/phases/phase-two/handoffs/handoff-2026-05-01-post-phase-two-runtime-reframe.md`. | Phase Three starts from live-runtime-passage investigation. | High |
| The next best domino is a live-passage scope and claim-ledger child, not immediate Elysia/Inngest/HyperDX/Nx implementation. | `tools/runtime-realization-type-env/evidence/current-lab-state.md`, `tools/runtime-realization-type-env/phases/phase-two/handoffs/handoff-2026-05-01-post-phase-two-runtime-reframe.md`, `tools/runtime-realization-type-env/evidence/runtime-spine-verification-diagnostic.md`, DRA-dispositioned domino review. | Make child 1 the scope/claim-ledger workstream. | High |
| The program must be recursive. | User control input, `workstream-2026-04-30-phase-two-spine-composition-program-workstream.md`, `tools/runtime-realization-type-env/guidance/workflow-phased-agent-verification.md`. | Define child entry/exit behavior and artifact flow; leave detailed child internals to children. | High |
| Explorer agents are retrieval lanes, not judgment authority. | User operational note and `tools/runtime-realization-type-env/guidance/workflow-phased-agent-verification.md` role rules. | Require DRA/default/strategy judgment review for conclusions and proof promotion. | High |
| Final structure/Nx/generator ratchet remains important but later by default. | `tools/runtime-realization-type-env/phases/phase-two/handoffs/handoff-2026-05-01-post-phase-two-runtime-reframe.md` and `tools/runtime-realization-type-env/evidence/current-lab-state.md`. | Keep as deferred likely later phase unless Phase Three child 1 produces evidence accepted by the DRA as a control decision. | High |
...
- Production-only: Parent-Repo Migration authorization, production host lifecycle,
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Parent-Repo Migration authorization | `out-of-scope` | Phase Three remains lab-contained. | Manifest `audit.p2.phase-two-program-closeout`; Phase Two closeout; this report. | A later Parent-Repo Migration workstream opens production code/topology explicitly. | Any claim says Phase Three authorizes Parent-Repo Migration. | Post-Phase-Three migration planning | Phase Three DRA until Parent-Repo Migration DRA accepts handoff. | `parent-repo-migration` |
| Final structure/Nx/generator ratchet | `out-of-scope` | Current sequence says live passage comes first. | Post-Phase-Two reframe; current lab state; this report. | Live-passage proof or closeout determines the remaining uncertainty is ready for structure ratchet. | Child 1 or closeout says structure is now the next domino. | Later structure/Nx/generator phase | Phase Three DRA until structure/Nx phase opens. | `lab` |
...
| Production config precedence and secret-store policy | `xfail` | Lab config validation/redaction does not settle platform precedence/secrets. | Runtime spec config/secrets sections; manifest config residuals. | Platform source order and secret-store policy are accepted and gated. | Provider/config child needs platform-secret proof. | Config/secret decision packet or Parent-Repo Migration | Phase Three DRA; config/secret decision-packet DRA if opened. | `spec/migration` |
| Native host telemetry/error mapping | `xfail` | Contained boundary records do not settle native host error taxonomy. | `audit.p1.effect-boundary-policy-matrix.residual`; diagnostic observation row. | Native host telemetry/error mapping policy is opened and gated. | Server/async child needs native error behavior. | Host passage child or Parent-Repo Migration integration | Phase Three DRA; host-passage child DRA if opened. | `parent-repo-migration` |
| First production resource/provider catalog cut | `todo` | Phase Three may use representative resources but does not define final production ids. | Manifest `audit.p2.first-resource-provider-cut`; diagnostic provider row. | Started-passage or Phase Three closeout evidence shows representative resource identity semantics are stable, and a later structure or migration phase explicitly needs production ids. | Parent-Repo Migration or structure ratchet needs real provider catalog. | Later provider catalog or migration workstream | Phase Three DRA until provider-catalog DRA accepts handoff. | `lab/migration` |
...
- Oracle: live-passage proof is required from future children; this
  document adds no oracle behavior proof.
...
- `phases/phase-three/workstreams/workstream-2026-05-01-phase-three-integrated-live-passage-rehearsal-and-closeout.md`
...
- `test/oracle/phase-three-integrated-live-passage-rehearsal.test.ts`
...
- `bun test
  tools/runtime-realization-type-env/test/oracle/phase-three-integrated-live-passage-rehearsal.test.ts`:
  passed with 1 test and 113 assertions after review repairs.
- `bunx nx run runtime-realization-type-env:oracle`: passed with 69
...
- Phase Three closed as contained live-runtime-passage simulation evidence.
...
- `workstream-2026-05-01-phase-three-native-boundary-observation-and-failure-semantics-ledger.md`
- `workstream-2026-05-01-phase-three-layer-disagreement-failure-observation-proof.md`
- `workstream-2026-05-01-phase-three-contained-elysia-host-passage.md`
- `workstream-2026-05-01-phase-three-contained-elysia-listen-lifecycle-passage.md`
