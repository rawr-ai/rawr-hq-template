...
import { revalidateCapturedLedger, type DestinationExportPlan } from "./plan";
...
export function planDestinationInverseActions(plan: DestinationExportPlan): readonly ExportInverseActionV1[] {
  ...
  for (const retirement of plan.retirements) {
    actions.push(createExportInverseAction({
      ...actionBase(plan, pluginAuthority("plugin-claim", retirement.pluginId, retirement.releaseDigest), retirement.claim.relativePath),
  ...
  for (const retirement of plan.directoryRetirements) {
    actions.push(createExportInverseAction({
      ...actionBase(plan, pluginAuthority("plugin-claim", retirement.pluginId, retirement.releaseDigest), mustRelativePath(retirement.relativePath)),
  ...
  if (plan.ledgerChange) {
    actions.push(createExportInverseAction({
      ...actionBase(plan, Object.freeze({ kind: "destination-ledger", nextGeneration: plan.nextLedger.body.generation }), mustRelativePath(EXPORT_LEDGER_FILENAME)),
      mutation: "write-ledger",
      prior: plan.current.slot.kind === "Absent" ? Object.freeze({ kind: "Absent" }) : fileStateFromCaptured(plan.current.slot.file),
