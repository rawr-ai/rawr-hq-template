...
The highest-value next lab iteration is not more vendor probing. It is narrowing the remaining negative space: runtime resource access/service binding law, async membership, dispatcher access, server route derivation, final `ProviderEffectPlan` shape, and the first real server/async harness mounting path.
