...
import { canonicalDigest, compareCanonical, type CanonicalValue } from "../domain/canonical";
import {
...
export type { ProviderOwnerRuntime } from "../ports/owner";
...
export function providerOwnerTargetBinding(
  ...
  return prior.kind === "absent"
    ? Object.freeze({
      canonicalTarget: target.targetDigest,
...
export function selectProviderOwnerTargetBindings(input: Readonly<{
  ...
  for (const action of input.actions) {
    const binding = available.get(action.target.targetDigest);
...
async function readActionState(
  ...
  if (action.kind === "SetMarketplace") {
    const result = await runtime.readMarketplace(action.target);
  ...
  const result = await runtime.readMember(action.target, actionNativeIdentity(action));
...
function parseBinding(input: unknown): ProviderOwnerTargetBinding {
  ...
  return Object.freeze({
    canonicalTarget: requireTargetDigest(record.canonicalTarget),
...
function requireActionBinding(action: ProviderOwnerAction, targets: readonly ProviderOwnerTargetBinding[]): ProviderOwnerTargetBinding {
  const selected = targets.filter((target) => target.canonicalTarget === action.target.targetDigest);
...
function assertBinding(action: ProviderOwnerAction, binding: ProviderOwnerTargetBinding): void {
  if (binding.canonicalTarget !== action.target.targetDigest) throw new Error("Provider target binding belongs to another action target");
}
...
function requireTargetDigest(input: unknown): ProviderTarget["targetDigest"] {
  const value = requireString(input, "targetDigest");
  if (!/^pt1_[0-9a-f]{64}$/u.test(value)) throw new Error("Provider target digest is invalid");
  return value as ProviderTarget["targetDigest"];
}
