...
it must not claim Lab-Production Proof.
...
| Runtime compiler emits `CompiledProcessPlan`, provider dependency graph, registry input, harness plans, topology seed, diagnostics, and `BootgraphInput`. | `simulation-proof` | `oracle`, `simulate` | Does not acquire resources, lower provider plans, or mount real harnesses. |
| Bootgraph/provisioning skeleton orders fake modules, rolls back started modules, finalizes in reverse order, and emits lifecycle records. | `simulation-proof` | `oracle` | Does not decide `ProviderEffectPlan` shape, refresh/retry policy, or provider-specific lowering. |
| Runtime access and service binding cache enforce sanctioned access, no raw runtime internals, construction-time cache identity, and invocation exclusion. | `proof` / `simulation-proof` | `typecheck`, `negative`, `oracle` | Does not expose broader runtime resource method law beyond the pinned spec. |
| Adapter callbacks consume compiled/runtime artifacts and delegate into `ProcessExecutionRuntime`. | `simulation-proof` | `oracle` | Does not prove real Elysia, Inngest, OCLIF, web, agent, or desktop host mounting. |
| Runtime catalog, diagnostics, redaction, rollback, and finalization records are in-memory observations. | `simulation-proof` | `oracle` | Does not prove telemetry export, catalog persistence, deployment placement, or storage/indexing policy. |
...
- Real production harness paths, durable Inngest scheduling/retry/idempotency,
  ...
  provider integrations stay parent-repo-migration or reserved.
