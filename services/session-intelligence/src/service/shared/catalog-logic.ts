...
import { detectSessionFormat, getClaudeSessionMetadata, getCodexSessionMetadata, inferProjectFromCwd, inferStatusFromPath } from "./normalization";
import { looksLikePath, stem } from "./path-utils";
import type { DiscoveredSessionFile, ResolveResult, SessionFilters, SessionListItem, SessionSourceFilter } from "./schemas";
...
export async function listSessions(input: {
  ...
  const discovered = await input.runtime.discoverSessions({
    source: input.source,
  ...
  for (const candidate of discovered) {
    if (candidate.source === "claude") {
      const meta = await getClaudeSessionMetadata(input.runtime, candidate.path);
      sessions.push({
        path: candidate.path,
      ...
      const meta = await getCodexSessionMetadata(input.runtime, candidate.path);
      sessions.push({
        path: candidate.path,
        sessionId: meta.sessionId,
        source: "codex",
  ...
  const filtered = sessions.filter((s) => {
    if (filters.project) {
      ...
      if (projectFilter && looksLikePath(projectFilter) && s.source !== "claude") return false;
      if (projectFilter && !looksLikePath(projectFilter) && !containsFilter(s.project, projectFilter)) return false;
    }
    if (!containsFilter(s.cwd, filters.cwdContains)) return false;
    if (!containsFilter(s.gitBranch, filters.branch)) return false;
    if (!containsFilter(s.model, filters.model)) return false;
    if ((filters.since || filters.until) && !sessionWithinWindow(s.modified, filters.since, filters.until)) return false;
    return true;
  });
...
export async function resolveSession(input: {
  ...
  if (looksLikePath(clean)) {
    if (await input.runtime.statFile({ path: clean })) resolvedPath = clean;
  } else {
    const candidates = await input.runtime.discoverSessions({ source: input.source });
    const claude = candidates.find((c) => c.source === "claude" && stem(c.path) === clean);
    const codex = candidates.find((c) => c.source === "codex" && stem(c.path).toLowerCase().includes(clean.toLowerCase()));
  ...
  const fmt = await detectSessionFormat(input.runtime, resolvedPath);
  const stat = await input.runtime.statFile({ path: resolvedPath });
  if (!stat) return { error: `Session not found: ${input.session}` };

  if (fmt === "claude") {
    return {
      resolved: { path: resolvedPath, source: "claude", modified: new Date(stat.modifiedMs).toISOString(), sizeBytes: stat.sizeBytes },
      metadata: await getClaudeSessionMetadata(input.runtime, resolvedPath),
    };
  }
  if (fmt === "codex") {
    return {
      resolved: { path: resolvedPath, source: "codex", status: status ?? inferStatusFromPath(resolvedPath), modified: new Date(stat.modifiedMs).toISOString(), sizeBytes: stat.sizeBytes },
      metadata: await getCodexSessionMetadata(input.runtime, resolvedPath),
    };
  }
