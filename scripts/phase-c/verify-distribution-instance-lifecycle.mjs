...
for (const [name, source, operation] of [
  ...
  for (const forbidden of ["global-rawr-owner-path", "apps/cli/bin/run.js", "ln -s", "ownerWorkspacePath"]) {
    assertCondition(!source.includes(forbidden), `${name}-global script retains legacy authority: ${forbidden}`);
  }
}
...
for (const field of [
  ...
  assertCondition(doctorSource.includes(field), `doctor global must report controller provenance field: ${field}`);
}
