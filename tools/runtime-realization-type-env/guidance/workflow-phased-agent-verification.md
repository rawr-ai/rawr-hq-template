...
All agents using this workflow must keep these concepts distinct:
...
| Lens | Use when |
| --- | --- |
| `architecture` | Target/current/transition separation, lifecycle boundaries, Parent-Repo Migration slices, and decision packets. |
...
| Evidence Auditor | Manifest status, proof strength, test-theater risk, and test-oracle quality. |
...
| 7. Host synthesis and design lock | Integration report | Source authority, vendor behavior, architecture truth, DX, Lab-Production Proof, and Parent-Repo Migration are not collapsed. |
...
- Semantic JSDoc/comments: high-signal comments for non-obvious lifecycle,
  ...
  edits.
- Evidence honesty: proof category, test oracle, gate, status wording, and test-theater risk.
...
6. Oracle behavior: contained runtime simulation, fake-host honesty, and runtime-owned delegation.
7. Manifest/report consistency: proof entries, gates, diagnostics, current lab state, and report output agree.
...
2. Migration derivability: whether the proof reduces actual Parent-Repo
   Migration risk without claiming Lab-Production Proof or Parent-Repo Migration
   authorization it has not earned.
...
| Yellow | Partially verified, vendor-only, simulation-only, known `xfail`/`todo`, or needs Parent-Repo Migration validation. |
...
- Promotion condition.
- Whether it is a Lab experiment, spec decision, or Parent-Repo Migration slice.
- Authority home.
- Re-entry trigger.
- Next eligible workstream.

No deferred item may exist only in chat or only in a workstream report. It must
also live in the manifest, diagnostic, todo fixture, research program, spec
patch proposal, Parent-Repo Migration note, or out-of-scope note as appropriate.
...
- An `xfail` can promote only after the test oracle is clear, the focused gate fails or passes for the right reason, and the manifest and diagnostic agree.
- Vendor proof never becomes RAWR runtime proof unless the claim is explicitly only about vendor behavior.
- Oracle or simulation proof never becomes Lab-Production Proof by itself and
  never authorizes Parent-Repo Migration without an explicit Parent-Repo Migration slice.
...
- Good repair demand: `lower provider acquire/release through bootgraph, keep retry policy fenced, rerun oracle, report, and architecture review`.
...
- A green label depends only on type, vendor, Oracle, or simulation evidence
  while claiming Lab-Production Proof or Parent-Repo Migration authorization beyond
  its earned ceiling.
- The next step would change parent repo packages, apps, services, deployment
  topology, or public surfaces when the current artifact is only a Lab proof.
...
authorization unless the diagnostic names the Lab evidence, its proof ceiling,
and what remains Parent-Repo Migration-only.
