...
function service_v2_contract_composition_status($operation, $binding) js {
  const operation = $operation.text.replace(/\s+/g, "");
  return operation.startsWith($binding.text + ".") ? "ok" : "wrong-root";
}

predicate service_v2_contract_composition_native($statements, $operation) {
  or {
    and {
      $statements <: contains `import { $..., oc, $... } from "@orpc/contract"`,
      $status = service_v2_contract_composition_status(operation=$operation, binding=`oc`),
      $status <: includes "ok"
    },
    and {
      $statements <: contains `import { $..., oc as $binding, $... } from "@orpc/contract"`,
      $status = service_v2_contract_composition_status(operation=$operation, binding=$binding),
      $status <: includes "ok"
    }
  }
}

predicate service_v2_contract_composition_projects($surface, $binding) {
  $surface <: object(),
  $surface <: contains or {
...
predicate service_v2_contract_composition_routes($statements, $routes, $module) {
  or {
    service_v2_contract_composition_projects(surface=$routes, binding=$module),
    and {
      $routes <: identifier(),
      or {
      ...
      service_v2_contract_composition_projects(surface=$surface, binding=$module)
    }
  }
}
...
or {
  program(statements=$statements) where {
    ...
    not {
      ...
      service_v2_contract_composition_native(statements=$statements, operation=$operation),
      $operation <: contains `$builder.router($routes)` where {
        service_v2_contract_composition_routes(statements=$statements, routes=$routes, module=$module)
      }
    }
  },
  program(statements=$statements) where {
    ...
    not {
      or {
        and {
          ...
          service_v2_contract_composition_projects(surface=$surface, binding=$leaf)
        },
        and {
