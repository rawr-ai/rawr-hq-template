...
import { assertAllowedHyperresearchOperation } from "../src/service/shared/adapters/hyperresearch-cli";
...
async function makeRunFixture() {
  ...
  await fs.writeFile(path.join(stepsRoot, "01-canonical-query.md"), "# Step 1\nPersist the query.\n", "utf8");
  await fs.writeFile(path.join(stepsRoot, "02-source-capture.md"), "# Step 2\nCapture a source.\n", "utf8");
  await fs.writeFile(path.join(stepsRoot, "03-final-artifact.md"), "# Step 3\nWrite final report.\n", "utf8");
...
describe("hyperresearch-codex synthetic runtime slice", () => {
  it("runs the synthetic three-step control plane with fresh step hashes, CLI calls, and final artifacts", async () => {
    ...
    expect(result.ledger.steps.map((step) => step.status)).toEqual(["complete", "complete", "complete"]);
    ...
    await expect(
      fs.readFile(path.join(fixture.vaultRoot, "research", "temp", "codex-artifacts", "final-report.md"), "utf8"),
  ...
  it("resumes from the durable ledger without rerunning completed steps", async () => {
    ...
    const firstClient = createClient(createClientOptions({
      repoRoot: fixture.root,
      cli: firstCli,
    }));
    ...
    const secondClient = createClient(createClientOptions({
      repoRoot: fixture.root,
      cli: secondCli,
    }));
