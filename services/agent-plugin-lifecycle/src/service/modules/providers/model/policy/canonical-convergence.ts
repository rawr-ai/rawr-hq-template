...
export function planCanonicalConvergence(
  ...
  const target =
    observation.kind === "observed" ? observation.inventory.target : observation.target;
  if (target.provider !== projection.provider) {
    return blocked("INCOMPATIBLE_PROVIDER", target, projection, [
      issue(
        "PROJECTION_MISMATCH",
        "target.provider",
  ...
  if (!capability.compatible) {
    return blocked("INCOMPATIBLE_PROVIDER", target, projection, [
      capabilityIssue(projection, capability),
    ]);
  }
  ...
  for (const desired of projection.members) {
    const related = managed.filter(
      (live) => live.pluginId === desired.pluginId || live.nativeIdentity === desired.nativeIdentity
    );
    if (related.length > 1) {
      return blocked("BLOCKED_COLLISION", target, projection, [
        issue(
  ...
  for (const exposure of inventory.standaloneExposures) {
    if (
      ...
      continue;
    const desired = projection.members.find(
      (member) =>
...
export function verifyCanonicalSelectedInventory(
  ...
  return first === undefined ? success(inventory) : failure([first, ...issues.slice(1)]);
}
...
export function verifyCanonicalFinalInventory(
  ...
  const selectedIdentities = new Set(
    projection.members.map((member) => `${member.pluginId}\0${member.nativeIdentity}`)
  );
  const extraManaged = inventory.members.find(
    (member) => !selectedIdentities.has(`${member.pluginId}\0${member.nativeIdentity}`)
  );
  ...
  const configuredResidue = inventory.standaloneExposures.find(
    (exposure) => exposure.providerSourceIdentity === projection.marketplace.identity
  );
...
export function verifyCanonicalConfiguredExposureRetired(
  ...
  const remains = inventory.standaloneExposures.some(
    (exposure) =>
...
function liveCollisionIssues(
  ...
  for (const [index, left] of inventory.members.entries()) {
    const duplicate = inventory.members
      .slice(index + 1)
      .find(
        (right) => right.pluginId === left.pluginId || right.nativeIdentity === left.nativeIdentity
      );
...
function strictlyOwned(
  ...
  return (
    live.providerSourceIdentity === projection.marketplace.identity &&
...
function sameVisibility(live: NativeMemberObservation, desired: ProviderProjectionMember): boolean {
  return (
    sameNames(live.visibleSkills, desired.visible.skills) &&
...
function freezeNonEmpty<T>(items: NonEmptyReadonlyArray<T>): NonEmptyReadonlyArray<T> {
