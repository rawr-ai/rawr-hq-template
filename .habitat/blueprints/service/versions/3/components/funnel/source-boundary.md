...
predicate service_v3_source_boundary_is_production_source() {
  $filename <: r"(?:^|.*/)src/(?:client|service/.*)\.ts$"
}

// Recognizes a relative edge that leaves one module and enters a sibling.
predicate service_v3_source_boundary_is_sibling_module_source($source) {
  or {
    and {
...
predicate service_v3_source_boundary_is_database_source($source) {
  or {
    $source <: r"^[\"']\./(?:service/)?db(?:/[^\"']*)?[\"']$",
    $source <: r"^[\"'](?:\.\./)+(?:service/)?db(?:/[^\"']*)?[\"']$"
  }
...
predicate service_v3_source_boundary_is_forbidden_database_source($source) {
  service_v3_source_boundary_is_database_source(source=$source),
...
predicate service_v3_source_boundary_is_proof_source($source) {
  $source <: r"^[\"'](?:\.\./)+test(?:/[^\"']*)?[\"']$"
}

// Recognizes concrete Node, Bun, and Effect platform module sources.
predicate service_v3_source_boundary_is_platform_source($source) {
...
predicate service_v3_source_boundary_is_forbidden_source($source) {
  or {
    service_v3_source_boundary_is_sibling_module_source(source=$source),
    service_v3_source_boundary_is_forbidden_database_source(source=$source),
    service_v3_source_boundary_is_proof_source(source=$source),
    service_v3_source_boundary_is_platform_source(source=$source)
  }
}
...
or {
  ...
  service_v3_source_boundary_is_production_source(),
  service_v3_source_boundary_is_forbidden_source(source=$source)
}
```
