...
interface TargetAssessmentBase {
...
interface AvailableTargetAssessment extends TargetAssessmentBase {
  readonly kind: "Available";
  readonly session: NativeAgentProviderSession;
  readonly capabilities: NativeProviderCapabilities;
  readonly inventory: NativeProviderInventory;
}

interface UnavailableTargetAssessment extends TargetAssessmentBase {
...
type TargetAssessment = AvailableTargetAssessment | UnavailableTargetAssessment;
...
/**
 * Inspects provider targets sequentially through the app-supplied ready catalog.
 *
 * @param content - Selected provider content whose native state is inspected.
 * @param targets - Canonically ordered provider targets.
 * @param nativeProviders - Closed ready resource catalog supplied by the app.
 * @param options - Reconciliation policy for omitted managed plugins.
 * @param mutationIntent - Whether inspection must admit every planned mutation.
 */
export function inspectProviderTargets(
  content: SelectedContent,
  targets: readonly ProviderTarget[],
  nativeProviders: NativeAgentProviderResources,
  options: ReconcileOptions,
  mutationIntent: boolean
): Effect.Effect<readonly TargetAssessment[]> {
  return Effect.forEach(
    targets,
    (target) => inspectProviderTarget(content, target, nativeProviders, options, mutationIntent),
    { concurrency: 1 }
...
/**
 * Applies admitted target changes in order and stops after failed or uncertain mutation.
 */
export function reconcileProviderTargets(
  content: SelectedContent,
  assessments: readonly TargetAssessment[],
  options: ReconcileOptions
): Effect.Effect<readonly ProviderMutationTargetResult[]> {
  return Effect.gen(function* () {
    const results: ProviderMutationTargetResult[] = [];
    for (let index = 0; index < assessments.length; index += 1) {
      const assessment = assessments[index]!;
      if (assessment.kind === "Unavailable" || !assessment.needsMutation) {
        results.push(convergedMutationTargetResult(assessment));
        continue;
      }
      const result = yield* reconcileProviderTarget(content, assessment, options);
      results.push(result);
...
function inspectProviderTarget(
  content: SelectedContent,
  target: ProviderTarget,
  nativeProviders: NativeAgentProviderResources,
  options: ReconcileOptions,
  mutationIntent: boolean
): Effect.Effect<TargetAssessment> {
  return Effect.gen(function* () {
    const acquisition = yield* Effect.result(acquireTargetSession(nativeProviders, target));
    if (Result.isFailure(acquisition)) {
      return unavailableAssessment(
        target,
        "Native provider acquisition failed",
        acquisition.failure
      );
    }
    ...
    if (Result.isFailure(observed)) {
      return unavailableAssessment(target, "Native provider inspection failed", observed.failure);
    }
...
function acquireTargetSession(
  nativeProviders: NativeAgentProviderResources,
  target: ProviderTarget
): Effect.Effect<NativeAgentProviderSession, NativeAgentProviderFailure> {
  switch (target.provider) {
    case "codex":
      return nativeProviders.codex.acquire({ home: target.home });
    case "claude":
      return nativeProviders.claude.acquire({ home: target.home });
  }
}
...
function assessInventory(
  ...
  options: ReconcileOptions,
  mutationIntent: boolean
): Effect.Effect<AvailableTargetAssessment> {
  return Effect.gen(function* () {
    ...
    for (const member of content.members) {
      ...
      const verified = yield* verifyMemberFiles(session, selector, member.pluginId, files, facts);
...
function reconcileProviderTarget(
  content: SelectedContent,
  assessment: AvailableTargetAssessment,
  options: ReconcileOptions
  ...
  return Effect.gen(function* () {
    ...
    if (forceRefresh) {
      if (assessment.marketplacePresent) {
        const liveBeforeRemovalAttempt = yield* Effect.result(session.inventory());
        if (Result.isFailure(liveBeforeRemovalAttempt)) {
          return failedTarget(assessment.target, operations, Object.freeze([]), [
            providerIssue(
              "NativeObservationFailed",
              `Marketplace could not be reobserved before removal: ${nativeFailureDetail(liveBeforeRemovalAttempt.failure)}`
            ),
          ]);
        }
        const liveBeforeRemoval = liveBeforeRemovalAttempt.success;
        ...
        const step = yield* mutateAndObserve(
          session,
          operation,
        ...
        const terminal = yield* mutationTerminal(
          content,
          assessment,
          options,
          operations,
    ...
    for (const memberAssessment of assessment.members) {
      ...
      if (live?.enabled === false && session.provider === "codex") {
        return yield* failedAfterMutation(content, assessment, options, operations, [
          ...issues,
    ...
    if (Result.isFailure(finalInventoryAttempt)) {
      return failedTarget(assessment.target, operations, Object.freeze([]), [
        ...
        providerIssue(
          "NativeObservationFailed",
          `Final provider state could not be observed: ${nativeFailureDetail(finalInventoryAttempt.failure)}`
        ),
      ]);
    }
    const finalInventory = finalInventoryAttempt.success;
...
function mutateAndObserve(
  session: NativeAgentProviderSession,
  operation: ConfirmedNativeOperation,
  mutate: () => Effect.Effect<unknown, NativeAgentProviderFailure>,
  postcondition: (inventory: NativeProviderInventory) => boolean
): Effect.Effect<MutationStep> {
  return Effect.gen(function* () {
    const mutation = yield* Effect.result(mutate());
    if (Result.isFailure(mutation)) {
      const phase = commandPhase(mutation.failure);
      const observed = yield* Effect.result(session.inventory());
      if (Result.isSuccess(observed)) {
        const inventory = observed.success;
        ...
        return {
          kind: "Failed",
          issue: providerIssue("NativeCommandFailed", nativeFailureDetail(mutation.failure)),
        };
      }
      ...
      return {
        ...
        issue: providerIssue(
          "NativeObservationFailed",
          `Provider state could not be observed after a command started: ${nativeFailureDetail(observed.failure)}`
        ),
    ...
    return {
      ...
      issue: providerIssue(
        "NativeObservationFailed",
        `Provider state could not be observed after a command returned: ${nativeFailureDetail(observed.failure)}`
      ),
  ...
  return Effect.gen(function* () {
    if (step.kind === "Failed") {
      return failedTarget(
        assessment.target,
        operations,
        yield* observeTerminalFacts(content, assessment, options),
        [...issues, step.issue]
      );
    }
    if (step.kind === "Uncertain") {
      return Object.freeze({
        ...
        facts: yield* observeTerminalFacts(content, assessment, options),
...
function failedAfterMutation(
  ...
  return observeTerminalFacts(content, assessment, options).pipe(
    Effect.map((facts) => failedTarget(assessment.target, operations, facts, issues))
  );
}

function observeTerminalFacts(
  content: SelectedContent,
  assessment: AvailableTargetAssessment,
  options: ReconcileOptions
  ...
  return Effect.result(session.inventory()).pipe(
    Effect.flatMap((inventory) =>
      Result.isFailure(inventory)
        ...
        : assessInventory(
            ...
            options,
            false
          ).pipe(Effect.map((observed) => observed.facts))
    )
  );
}
...
function verifyMemberFiles(
  ...
  Readonly<{ matches: boolean; failed: boolean; issues: readonly ProviderIssue[] }>
> {
  return Effect.gen(function* () {
    const issues: ProviderIssue[] = [];
    let failed = false;
    ...
    if (Result.isFailure(observation)) {
      return Object.freeze({
        ...
        issues: Object.freeze([
          providerIssue(
            "NativeObservationFailed",
            `Could not read the selected files for ${selector}: ${nativeFailureDetail(observation.failure)}`,
            pluginId
          ),
        ]),
      });
...
function commandPhase(error: NativeAgentProviderFailure): NativeProviderCommandPhase {
  return error.commandPhase;
}

function nativeFailureDetail(error: NativeAgentProviderFailure): string {
  return error.detail;
}
