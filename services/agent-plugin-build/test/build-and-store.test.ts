...
import { createReleaseArtifactRef } from "@rawr/agent-plugin-release";
...
import {
  ...
  type ArtifactStore,
} from "../src/artifact-store/filesystem-store";
import {
...
describe("build application and append-only artifact store", () => {
  ...
  it("keeps a multi-member set invisible when a later member fails, then converges deterministically", async () => {
    ...
    const applications = createAgentPluginBuildApplications({ source, artifacts });
    const mode = { kind: "complete-set" } as const;
  ...
  it("rejects an open skill ownership plan before any complete-set artifact operation", async () => {
    ...
    const result = await createAgentPluginBuildApplications({ source: await realSource(), artifacts }).build({
  ...
  it("reclassifies a rejected member path when a concurrent builder commits the exact set", async () => {
    ...
    const racingApplication = createAgentPluginBuildApplications({
      source: setup.source,
  ...
  it.each(["mismatch", "read-failure"] as const)(
    ...
    async (outcome) => {
      ...
      const result = await createAgentPluginBuildApplications({ source, artifacts }).build({
        contentWorkspace: repository.policy,
        mode: { kind: "complete-set" },
      });
      expect(result).toMatchObject({
  ...
  it("preserves and rejects a late conflicting digest-path winner", async () => {
    ...
    const applications = createAgentPluginBuildApplications({ source: await realSource(), artifacts });
    const result = await applications.build({
      contentWorkspace: repository.policy,
      mode: { kind: "targeted", pluginId: repository.pluginId },
    });
  ...
  it("keeps protected-path knowledge in a test-only source trap and never opens Git or artifacts", async () => {
    ...
    const applications = createAgentPluginBuildApplications({ source: trapped, artifacts });
    const result = await applications.build({
  ...
  async function buildSetup() {
    ...
    return {
      ...
      source,
      artifacts,
      applications: createAgentPluginBuildApplications({ source, artifacts }),
    };
  }
