...
- [x] 0.1 Audit the installed custom controller and distinguish the CLI
  application, distribution format, version selector, lifecycle reconciler, and
  provider adapters.
- [x] 0.2 Confirm canonical Template and Personal `main`, clean isolated
...
- [x] 1.6a Make current Nx task ownership and prerequisites explicit. Keep the
  ...
  provider contract and each concrete provider disjoint test ownership, while
...
- [x] 5.2c Move the releases contract off the root `ocBase` feedback edge.
  ...
  owner-local schema boundary, service telemetry behavior, lint, typecheck, and
  the staged declarative-contract law. This checkpoint does not claim the
  releases module shell or the complete service corpus is green.
- [x] 5.2d Move the governance contract off the root `ocBase` feedback edge.
  Construct both governance operations directly from Effect-oRPC `eoc`, carry
  their existing domain, audience, idempotence, audit, and owner metadata
  explicitly, and keep TypeBox as the request/result authority. Prove the
  owner-local schema and public-client behavior, lint, typecheck, and the staged
  declarative-contract law. This checkpoint does not claim the governance
  module shell or the complete service corpus is green.
- [x] 5.2e Move the packaging contract off the root `ocBase` feedback edge.
  Construct the packaging operation directly from Effect-oRPC `eoc`, carry its
  existing domain, audience, idempotence, basic audit, and owner metadata
  explicitly, and keep TypeBox as the request/result authority. Prove the
...
- [x] 5.2f Move the vendors contract off the root `ocBase` feedback edge.
  ...
  module shell or the complete service corpus is green.
- [x] 5.2g Move the providers contract off the root `ocBase` feedback edge.
  Construct provider test, status, and sync directly from Effect-oRPC `eoc`,
  preserving basic audit for read-only status and full audit for test and sync
  alongside their domain, audience, idempotence, and owner metadata. Keep
  TypeBox as the request/result authority and prove owner-local provider
...
- [x] 5.7e21g Install the current Magic service and database authority before
  ...
  or runner.
- [ ] 5.7e21h Before treating the existing production `model/entities` roots as
  admitted source authority or adding another production entity, extend the
  shared TypeBox contract and platform-neutral declaration laws to service- and
  module-level entity roots. Preserve the common Magic/Template law rather than
  adding a RAWR-only fork. Prove canonical TypeBox schema and generated-type
  ownership, platform and persistence neutrality, and the one-way
  DTO/contract/store-to-entity dependency direction before the kind can expand.
...
- [x] 5.7e21z1 Migrate Vendors as the third production consumer of the
  restored router law. Move the `status` and `update` operation leaves
  byte-for-byte to `router/status.router.ts` and `router/update.router.ts`;
  move the former router barrel to module-root `router.ts`; and change only its
  two relative leaf imports. Preserve the module branch and curated context,
  TypeBox contracts, inline Effect handlers, public keys, status observation,
  held and diverged classification, materialization clock,
  capture/revalidation/apply/restore/settlement order, results, failures,
  defects, interruption, and the root service's
  `./modules/vendors/router` import. Delete the predecessor paths without
  aliases. Add no policy, resource, provider, middleware, contract, test,
  script, or live-state change.
- [x] 5.7e21z2 Migrate Providers as the fourth production consumer of the
  restored router law. Move the `status`, `sync`, and `test` operation leaves
  byte-for-byte to `router/status.router.ts`, `router/sync.router.ts`, and
  `router/test.router.ts`; move the former router barrel to module-root
  `router.ts`; and change only its three relative leaf imports. Preserve the
  module branch and curated context, TypeBox contracts and schemas, inline
  Effect handlers, public operation keys, selected-content derivation and
  revalidation, source-interface and marketplace validation, native
  observation and mutation order, operation-local sessions, targeted and
...
- [x] 5.7e44 Split schema publication from native oRPC contract authority.
  ...
  DTOs, and error-data leaves. Reject executable refinements/codecs, unsafe
...
- Status and verification: [[README]].
