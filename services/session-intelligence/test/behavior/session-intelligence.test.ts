...
} from "../support/service/fixture-data";
import {
  createClientOptions,
...
describe("@habitat-ai/rawr-session-intelligence", () => {
  ...
  it("searches session content with and without the host-provided cache", async () => {
    ...
    expect(indexRuntime.defaultIndexPathCalls).toBe(1);
    ...
    expect(indexRuntime.defaultIndexPathCalls).toBe(2);
    ...
    expect(indexRuntime.defaultIndexPathCalls).toBe(3);
    expect(indexRuntime.getCalls).toBe(2);
    expect(indexRuntime.setCalls).toBe(1);
  });

  it("uses one lazily resolved index path throughout a cached search", async () => {
    const sourceRuntime = createFixtureSourceRuntime();
    const indexRuntime = new MemorySessionIndexRuntime([
      "/tmp/session-index-a.sqlite",
      "/tmp/session-index-b.sqlite",
    ]);
    const client = createClient(createClientOptions({ sourceRuntime, indexRuntime }));
    ...
    await client.search.content(
      ...
      createInvocation("trace-content-single-index")
    );

    expect(indexRuntime.defaultIndexPathCalls).toBe(1);
    expect(new Set(indexRuntime.observedIndexPaths)).toEqual(
      new Set(["/tmp/session-index-a.sqlite"])
    );
  });
