...
/** Discovers Codex sessions through the service-owned root index. */
export async function discoverCodexSessionsFromIndex(
  runtime: SessionSourceRuntime,
  store: CodexDiscoveryStore,
  max: number
): Promise<DiscoveredSessionFile[]> {
  const indexed = await discoverCodexFromIndex(runtime, store, max);
  return indexed.map(codexFileToDiscovered);
}
