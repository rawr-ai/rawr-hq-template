...
export function failure(code: ExportFailureCode, phase: string, message: string, path?: string): ExportFailure {
...
export function toFilesystemError(error: unknown): ExportFilesystemError {
  return error instanceof ExportFilesystemError
    ? error
    : operationFailure("MutationFailed", "filesystem", errorMessage(error));
}
