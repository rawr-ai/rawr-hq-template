...
export function createControllerReleaseEnvelope(manifest: ControllerPayloadManifest): ControllerReleaseEnvelope {
...
export function verifyControllerReleaseEnvelope(
  ...
  if (input.schemaVersion !== CONTROLLER_RELEASE_ENVELOPE_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", "envelope.schemaVersion", "Unsupported controller release envelope version", {
  ...
  if (claimedDigest.ok && manifest.ok) {
    ...
    if (computedDigest !== claimedDigest.value) {
      issues.push(issue("CONTROLLER_DIGEST_MISMATCH", "envelope.controllerDigest", "Claimed controller digest does not match the canonical payload manifest", {
  ...
  checkExpectedDigest(expectations.controllerDigest, "expectations.controllerDigest", "CONTROLLER_DIGEST_MISMATCH", claimedDigest, issues);
  checkExpectedDigest(
  ...
  if (!claimedDigest.ok || !manifest.ok) {
    return failure([issue("EXPECTED_OBJECT", "envelope", "Envelope validation did not produce a complete value")]);
  }
...
export function canonicalStringifyControllerReleaseEnvelope(envelope: ControllerReleaseEnvelope): string {
...
export function canonicalSerializeControllerReleaseEnvelope(envelope: ControllerReleaseEnvelope): Uint8Array {
...
export function decodeControllerReleaseEnvelope(
  ...
  if (!(bytes instanceof Uint8Array)) {
    return failure([issue("EXPECTED_BYTES", "envelope", "Controller release envelope must be a Uint8Array")]);
  }
  if (bytes.byteLength > MAX_CONTROLLER_RELEASE_ENVELOPE_BYTES) {
    return failure([
      issue("ENVELOPE_TOO_LARGE", "envelope", "Controller release envelope exceeds its decode bound", {
  ...
  try {
    ...
    return failure([issue("INVALID_UTF8", "envelope", "Controller release envelope is not valid UTF-8")]);
  }
  let parsed: unknown;
  try {
    parsed = JSON.parse(text);
  } catch {
    return failure([issue("INVALID_JSON", "envelope", "Controller release envelope is not valid JSON")]);
  }
  ...
  if (!equalBytes(bytes, canonical)) {
    return failure([
      issue("NON_CANONICAL_ENVELOPE", "envelope", "Envelope bytes are not the unique canonical representation"),
    ]);
  }
  return verified;
}
...
function checkExpectedDigest(
  ...
  if (claimed.ok && expected.value !== claimed.value) {
    issues.push(issue(mismatchCode, path, "Expected controller identity differs from the release envelope", {
...
function isExactEnvelope(value: unknown, issues: ControllerIssue[]): value is Record<string, unknown> {
  ...
  for (const key of Object.keys(value)) {
    if (!expected.has(key)) issues.push(issue("UNKNOWN_FIELD", `envelope.${key}`, "Field is not part of the closed schema"));
  }
  for (const key of keys) {
    if (!Object.hasOwn(value, key)) issues.push(issue("UNKNOWN_FIELD", `envelope.${key}`, "Required field is missing"));
  }
  return true;
}
