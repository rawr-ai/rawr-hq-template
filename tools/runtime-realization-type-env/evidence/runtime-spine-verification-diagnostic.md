...
4. Explicit lab declarations and cold route factories can flow through derivation, compilation, adapter lowering payloads, bootgraph/catalog records, runtime access/cache checks, adapter delegation, and deployment handoff boundaries.
...
| First server/async harness mounts | Consume contained adapter payloads in real server and async harness mounting paths without promoting durable async or full boundary policy prematurely. |
| Boundary policy matrix | Decide timeout, retry, interruption, telemetry, redaction, and error/exit mapping before migration-ready claims. |
