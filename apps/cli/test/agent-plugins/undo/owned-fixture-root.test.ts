...
describe("capsule owned fixture cleanup", () => {
  it("rejects a symlink substitution and preserves its target", async () => {
    ...
    const target = await mkdtemp(join(dirname(fixture.path), ".rawr-capsule-target-"));
    ...
    try {
      ...
      await expect(fixture.cleanup()).rejects.toThrow(/refusing recursive cleanup|changed immediately/u);
