...
predicate resource_v3_effect_error_authority_is_production_source() {
  $filename <: r".*\.ts$",
...
predicate resource_v3_effect_error_authority_is_error_subclass($name) {
...
predicate resource_v3_effect_error_authority_contains_untyped_error($failure) {
  or {
    $failure <: contains `Error`,
    $failure <: contains $name where {
      resource_v3_effect_error_authority_is_error_subclass(name=$name)
    }
  }
}
...
or {
  `Effect.Effect<$success, $failure>` where {
    resource_v3_effect_error_authority_is_production_source(),
    resource_v3_effect_error_authority_contains_untyped_error(
      failure=$failure
    )
  },
