...
const trackedFiles = execFileSync(
  "git",
  ["ls-files", "plugins/agents/**", "plugins/server/api/**"],
  {
    cwd: process.cwd(),
    encoding: "utf8",
  }
...
const trackedAgentRoots = [
  ...new Set(
    trackedFiles
      .filter((file) => file.startsWith("plugins/agents/"))
...
const trackedServerApiRoots = [
  ...new Set(
    trackedFiles
      .filter((file) => file.startsWith("plugins/server/api/"))
...
assertExactSet(
  trackedAgentRoots,
  ["plugins/agents/hq"],
  "tracked Phase 1 marketplace compatibility lane"
);
assertExactSet(
