import type { VerifiedArtifactSnapshotV1 } from "@rawr/agent-plugin-release";

import { equalBytes } from "../domain/canonical";
import { marketplaceState, sameMarketplaceState } from "../domain/marketplace";
import {
  createMechanicalProviderEvidence,
  type MechanicalEvidenceSource,
  type ProviderVerificationFact,
} from "../domain/evidence";
import type { EvaluationProfile } from "../domain/mode";
import type { ProviderEvent, ProviderOperationOutcome, TargetOperationOutcome } from "../domain/outcome";
import {
...
import { visibleFingerprint, type ManagedMemberClaim } from "../domain/receipt";
import type { ProviderTarget } from "../domain/target";
import type { ProviderCapsuleSession, ProviderCapsuleWriter } from "../ports/capsule";
...
export async function createProjectionPlans(input: ProjectionPlanInput): Promise<readonly ProviderTargetPlan[]> {
  ...
  for (const target of input.targets) {
    ...
    const [capabilityResult, inventoryResult, receiptResult, identityResult] = await Promise.all([
      input.dependencies.provider.inspectCapabilities(target),
      input.dependencies.provider.readInventory(target),
...
export async function inspectTargetsAsBlocked(
  ...
  dependencies: PlanReadDependencies,
  baseIssues: readonly ProviderDeploymentIssue[],
): Promise<readonly ProviderTargetPlan[]> {
  const plans: ProviderTargetPlan[] = [];
  for (const target of targets) {
    const reads = await Promise.all([
      dependencies.provider.inspectCapabilities(target),
      dependencies.provider.readInventory(target),
      dependencies.receipts.read(target),
      dependencies.identities.read(target),
    ]);
    plans.push(blocked(target, null, [...baseIssues, ...collectReadIssues(reads)]));
  }
  return Object.freeze(plans);
}
...
async function executePlansInternal(
  ...
  const candidate = Object.freeze({ protocol: "agent-provider-deployment@v1" as const, plans: Object.freeze(eligible) });
  const preflight = await dependencies.capsule.preflight(candidate);
  if (!preflight.ok) {
  ...
  const begun = await dependencies.capsule.begin(candidate);
  if (!begun.ok) {
...
async function executeMutatingPlan(
  plan: ProviderTargetPlan,
  dependencies: ApplyDependencies,
  session: ProviderCapsuleSession,
  ...
  for (const step of plan.steps) {
    ...
    if (step.kind === "verify-retired") {
      ...
      const inventory = await dependencies.provider.readInventory(plan.target);
...
async function verifyManagedTarget(
  ...
  const inventory = await provider.readInventory(target);
