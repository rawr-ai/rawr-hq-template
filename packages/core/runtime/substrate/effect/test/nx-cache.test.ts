...
const FIXTURE_PREFIX = "runtime-substrate-effect-nx-cache-";
...
test("restores an unchanged build and invalidates on a relevant substrate input", async () => {
  const fixtureRoot = await createFixture();
  const ownerRoot = path.join(fixtureRoot, substratePath);
...
async function runBuild(fixtureRoot: string): Promise<CommandResult> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [nx, "run", "runtime-substrate-effect:build", "--outputStyle=static"],
      { cwd: fixtureRoot, env }
    );
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const timer = setTimeout(() => {
      timeoutError = new Error("runtime-substrate-effect:build exceeded its cache-proof timeout.");
      child.kill("SIGKILL");
    }, 45_000);
...
function cacheHit(result: CommandResult): boolean {
  return /nx run runtime-substrate-effect:build\s+\[local cache\]/iu.test(
    `${result.stdout}\n${result.stderr}`
  );
}
...
async function removeFixture(root: string): Promise<void> {
  ...
  if (
    ...
    throw new Error(`Refusing to remove unexpected runtime-substrate-effect Nx fixture: ${root}`);
  }
