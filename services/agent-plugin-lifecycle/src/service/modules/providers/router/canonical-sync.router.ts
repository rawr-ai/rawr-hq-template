...
import type { AcceptedProviderProjectionBinding, CanonicalChannelResolution } from "../ports/channel";
...
export const canonicalSync = module.canonicalSync.handler(
  async ({ context, input }) => providerOperationResult(
    executeCanonicalSync(input, context.providers),
  ),
);

async function executeCanonicalSync(
  input: unknown,
  ports: CanonicalSyncDependencies,
