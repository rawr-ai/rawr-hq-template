...
export function preToolUseHookOutput(decision: HookDecision): {
  stdout: string;
  stderr: string;
  exitCode: number;
} {
...
export function stopHookOutput(decision: HookDecision): {
  stdout: string;
  stderr: string;
  exitCode: number;
} {
...
function isLikelySourceBypass(command: string): boolean {
  ...
  return (
    /\b(curl|wget)\b/i.test(normalized) ||
...
function isExplicitHyperresearchCaptureCommand(command: string): boolean {
  return (
    /(?:^|\s)(?:HYPERRESEARCH_CODEX_CAPTURE|RAWR_HYPERRESEARCH_CAPTURE)=1(?:\s|$)/.test(command) ||
...
export function decidePreToolUse(input: {
  ...
  const eventName =
    stringField(input.payload, "hook_event_name") ?? stringField(input.payload, "event");
  ...
  if (!command)
    return { action: "error", reason: "PreToolUse Bash payload missing tool_input.command" };
...
function resolveLedgerPath(payload: HookPayload, options?: HookRuntimeOptions): string | null {
  ...
  const configuredPath =
    stringField(payload, "ledger_path") ??
    stringField(payload, "ledgerPath") ??
    (config
      ? (stringField(config, "hyperresearchCodexLedgerPath") ?? stringField(config, "ledgerPath"))
      : null) ??
    (hyperresearch
      ? (stringField(hyperresearch, "ledger_path") ?? stringField(hyperresearch, "ledgerPath"))
      : null);
...
function ledgerValidationMessage(ledger: Record<string, unknown>): string | null {
  ...
  const reviewDispositions = Array.isArray(ledger.reviewDispositions)
  ...
  const violations =
    patchGuard && Array.isArray(patchGuard.violations) ? patchGuard.violations : [];
  ...
  const validationPassed =
    ledger.validationPassed === true ||
    validation?.passed === true ||
    lastValidation?.passed === true ||
    finalValidation?.passed === true;
  if (!validationPassed)
    return "Hyperresearch ledger is complete but missing a passed validation marker";
...
export function decideStop(input: {
  ...
  if (!fs.existsSync(ledgerPath)) {
    return {
      action: "block",
      reason: `Hyperresearch Stop guard could not find ledger: ${ledgerPath}`,
    };
  }
  ...
  try {
    ...
    return {
      action: "block",
      reason: `Hyperresearch Stop guard could not parse ledger: ${ledgerPath}`,
    };
  }

  const record = asRecord(ledger);
  if (!record)
    return { action: "block", reason: `Hyperresearch ledger is not a JSON object: ${ledgerPath}` };
