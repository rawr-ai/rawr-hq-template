...
export function createAgentPluginReleaseSet(
  ...
  if (!isExactRecord(input, ["releaseInput", "releases"], "releaseSet", issues)) {
    return failure([issues[0] ?? issue("EXPECTED_OBJECT", "releaseSet", "Release-set construction input must be an object")]);
  }
  const releaseInput = verifyEmbeddedReleaseInput(input.releaseInput, issues);
  const rawReleases = parseBoundedArray(input.releases, "releaseSet.releases", MAX_RELEASE_MEMBERS, issues);
  ...
  releases.sort((left, right) => compareCanonicalText(
  ...
  if (first === undefined) {
    issues.push(issue("MISSING_EXPECTED_MEMBER", "releaseSet.releases", "A complete release set must contain its declared members", {
  ...
  if (releaseInput === undefined || first === undefined) {
    return failure([issue("EXPECTED_OBJECT", "releaseSet", "Release-set validation did not produce a complete value")]);
  }
  ...
  const body: AgentPluginReleaseSetBody = Object.freeze({
    ...
    members: Object.freeze(releases.map((release) => Object.freeze({
  ...
  if (byteLength > MAX_AGENT_PLUGIN_RELEASE_SET_ENVELOPE_BYTES) {
    return failure([issue("ENVELOPE_TOO_LARGE", "releaseSet", "Release-set envelope exceeds its protocol bound", {
...
export function verifyAgentPluginReleaseSet(
  ...
  if (!isExactRecord(input, ["body", "releaseSetDigest", "schemaVersion"], "releaseSet", issues)) {
    return failure([issues[0] ?? issue("EXPECTED_OBJECT", "releaseSet", "Release-set envelope must be an object")]);
  }
  if (input.schemaVersion !== AGENT_PLUGIN_RELEASE_SET_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", "releaseSet.schemaVersion", "Unsupported release-set envelope version", {
  ...
  const claimedDigest = collect(parseReleaseSetDigest(input.releaseSetDigest, "releaseSet.releaseSetDigest"), issues);
  ...
  if (body !== undefined && claimedDigest !== undefined) {
    ...
    if (computed !== claimedDigest) {
      issues.push(issue("RELEASE_SET_DIGEST_MISMATCH", "releaseSet.releaseSetDigest", "Claimed set digest differs from the complete set body", {
        expected: computed,
...
export function decodeAgentPluginReleaseSet(
  ...
  const decoded = decodeCanonicalJson(bytes, "releaseSet", MAX_AGENT_PLUGIN_RELEASE_SET_ENVELOPE_BYTES);
  if (!decoded.ok) return decoded;
  const verified = verifyAgentPluginReleaseSet(decoded.value);
  if (!verified.ok) return verified;
  if (!(bytes instanceof Uint8Array) || !equalBytes(bytes, canonicalSerializeAgentPluginReleaseSet(verified.value))) {
    return failure([issue("NON_CANONICAL_ENVELOPE", "releaseSet", "Release-set bytes are not the unique canonical representation")]);
  }
  return verified;
}

export function canonicalSerializeAgentPluginReleaseSetBody(body: AgentPluginReleaseSetBody): Uint8Array {
  return canonicalJsonLine(agentPluginReleaseSetBodyValue(body));
}

export function canonicalSerializeAgentPluginReleaseSet(releaseSet: AgentPluginReleaseSet): Uint8Array {
  return canonicalJsonLine(agentPluginReleaseSetValue(releaseSet));
}

export function agentPluginReleaseSetBodyValue(body: AgentPluginReleaseSetBody): CanonicalJsonValue {
  return {
...
function parseReleaseSetBody(
  ...
  if (input.schemaVersion !== AGENT_PLUGIN_RELEASE_SET_SCHEMA_VERSION) {
    issues.push(issue("INVALID_SCHEMA_VERSION", `${path}.schemaVersion`, "Unsupported release-set schema version", {
      expected: AGENT_PLUGIN_RELEASE_SET_SCHEMA_VERSION,
      actual: typeof input.schemaVersion === "number" ? input.schemaVersion : String(input.schemaVersion),
    }));
  }
  ...
  const authority = collect(parseContentAuthority(input.contentAuthority, `${path}.contentAuthority`), issues);
  ...
  const witness = parseCompletenessWitness(input.completenessWitness, `${path}.completenessWitness`, issues);
  const ownershipIndex = parseDistributionOwnershipIndex(input.ownershipIndex, `${path}.ownershipIndex`, issues);
  const members = parseSetMembers(input.members, `${path}.members`, issues);

  if (witness !== undefined && inputDigest !== undefined && witness.releaseInputDigest !== inputDigest) {
    issues.push(issue("RELEASE_INPUT_IDENTITY_MISMATCH", `${path}.completenessWitness.releaseInputDigest`, "Completeness witness belongs to another release input", {
      expected: inputDigest,
      actual: witness.releaseInputDigest,
    }));
  }
  if (witness !== undefined && ownershipIndex !== undefined && !sameCanonicalValue(
    ownershipIndexValue(witness.ownershipIndex),
    ownershipIndexValue(ownershipIndex),
  )) {
    issues.push(issue("OWNERSHIP_INDEX_MISMATCH", `${path}.ownershipIndex`, "Set ownership index differs from its completeness witness"));
  }
  if (witness !== undefined && members !== undefined) validateWitnessMemberIds(witness, members, path, issues);
  if (
...
function parseSetMembers(
  ...
  values.forEach((candidate, index) => {
    ...
    if (!isExactRecord(candidate, ["artifactDigest", "pluginId", "releaseDigest"], memberPath, issues)) return;
    const pluginId = collect(parsePluginId(candidate.pluginId, `${memberPath}.pluginId`), issues);
    const rd = collect(parseReleaseDigest(candidate.releaseDigest, `${memberPath}.releaseDigest`), issues);
    const ad = collect(parseArtifactDigest(candidate.artifactDigest, `${memberPath}.artifactDigest`), issues);
  ...
  for (let index = 1; index < members.length; index += 1) {
    if (members[index - 1]!.pluginId === members[index]!.pluginId) {
      issues.push(issue("DUPLICATE_PLUGIN_ID", path, `Duplicate set member: ${members[index]!.pluginId}`));
    }
  }
  if (members.length === 0) {
...
function verifyEmbeddedReleaseInput(input: unknown, issues: ReleaseIssue[]): AgentPluginReleaseInput | undefined {
  if (typeof input !== "object" || input === null || Array.isArray(input)) {
    issues.push(issue("EXPECTED_OBJECT", "releaseSet.releaseInput", "Release input must be a verified value"));
    return undefined;
  }
...
function validateExpectedMembership(
  ...
  const expected = new Map(input.completenessWitness.expectedMembers.map((member) => [member.pluginId, member.payloadDigest]));
  const actual = new Map(releases.map((release) => [release.artifactBody.releaseBody.pluginId, release]));
  for (const [pluginId, payloadDigest] of [...expected].sort(([left], [right]) => compareCanonicalText(left, right))) {
    const release = actual.get(pluginId);
    if (release === undefined) {
      issues.push(issue("MISSING_EXPECTED_MEMBER", `releaseSet.members.${pluginId}`, "Expected member is absent", { actual: pluginId }));
    } else if (release.artifactBody.releaseBody.payloadDigest !== payloadDigest) {
      issues.push(issue("PAYLOAD_DIGEST_MISMATCH", `releaseSet.members.${pluginId}.payloadDigest`, "Member payload differs from the completeness witness", {
  ...
  for (const pluginId of [...actual.keys()].sort(compareCanonicalText)) {
    if (!expected.has(pluginId)) {
      issues.push(issue("EXTRA_MEMBER", `releaseSet.members.${pluginId}`, "Release is not declared by the completeness witness", { actual: pluginId }));
    }
  }
}
...
function validateReleaseIdentity(
  ...
  if (body.contentAuthority !== input.body.contentAuthority) {
    issues.push(issue("SOURCE_IDENTITY_MISMATCH", `releaseSet.members.${body.pluginId}.contentAuthority`, "Release has another content authority", {
      expected: input.body.contentAuthority,
      actual: body.contentAuthority,
    }));
  }
  if (body.releaseInputDigest !== input.releaseInputDigest) {
    issues.push(issue("RELEASE_INPUT_IDENTITY_MISMATCH", `releaseSet.members.${body.pluginId}.releaseInputDigest`, "Release belongs to another release input", {
  ...
  if (declaration === undefined) {
    issues.push(issue("MEMBER_NOT_DECLARED", `releaseSet.members.${body.pluginId}`, "Release is not declared by the verified release input", {
    ...
    if (
      ...
      issues.push(issue("RELEASE_INPUT_IDENTITY_MISMATCH", `releaseSet.members.${body.pluginId}.aliases`, "Release aliases differ from the verified release input"));
    }
    if (!sameCanonicalValue(
      payloadManifestValue(declaration.payload.manifest),
      payloadManifestValue(body.payloadManifest),
    )) {
      issues.push(issue("PAYLOAD_MANIFEST_MISMATCH", `releaseSet.members.${body.pluginId}.payloadManifest`, "Release manifest differs from the verified release input"));
    }
    for (const field of ["vendor", "curation"] as const) {
      if (!sameCanonicalValue(
        ...
        issues.push(issue("RELEASE_INPUT_IDENTITY_MISMATCH", `releaseSet.members.${body.pluginId}.${field}`, `Release ${field} bindings differ from the verified release input`));
      }
    }
  }
  if (firstBody !== undefined) {
    ...
    for (const field of fields) {
      if (body[field] !== firstBody[field]) {
        issues.push(issue("SOURCE_IDENTITY_MISMATCH", `releaseSet.members.${body.pluginId}.${field}`, "Release belongs to another source snapshot", {
...
function validateWitnessMemberIds(
  ...
  for (const pluginId of [...expected].sort(compareCanonicalText)) {
    if (!actual.has(pluginId)) {
      issues.push(issue("MISSING_EXPECTED_MEMBER", `${path}.members.${pluginId}`, "Set omits a completeness-witness member", { actual: pluginId }));
    }
  }
  for (const pluginId of [...actual].sort(compareCanonicalText)) {
    if (!expected.has(pluginId)) {
      issues.push(issue("EXTRA_MEMBER", `${path}.members.${pluginId}`, "Set contains a member absent from its completeness witness", { actual: pluginId }));
    }
  }
}
...
function validateReleaseSetMembers(
  ...
  for (const [pluginId, member] of body.members.map((entry) => [entry.pluginId, entry] as const)) {
    ...
    const sourceFields = ["contentAuthority", "sourceRepository", "sourceCommit", "sourceTree", "releaseInputDigest"] as const;
...
function reportDuplicateReleaseMembers(releases: readonly AgentPluginRelease[], issues: ReleaseIssue[]): void {
  ...
  for (const [pluginId, count] of [...counts].sort(([left], [right]) => compareCanonicalText(left, right))) {
    if (count > 1) {
      issues.push(issue("DUPLICATE_PLUGIN_ID", "releaseSet.releases", `Duplicate release member: ${pluginId}`));
    }
  }
}
