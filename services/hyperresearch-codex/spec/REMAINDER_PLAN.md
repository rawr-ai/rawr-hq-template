...
The Codex-RAWR packet-provenance light proof, short multi-source runtime proof, and repaired full-tier Inngest V8 proof are green; see `evidence.md`. The focused child-agent lifecycle diagnostic has now failed the resume case: same-process children pass, but pre-resume child handles become `not_found` after `codex-rawr exec resume`. App-server smoke reproduced the same cold-resume child-handle failure with structured collab events. Active Hyperresearch Codex parity is not clean/green/done until `HR-CODEX-035` is resolved or the claim is explicitly re-scoped. The next concrete track is Codex/RAWR runtime repair or an explicit-child-resume app-server variant; it is not a Hyperresearch service redesign. Hooks, parked MCP work, production Inngest readiness, and unrelated global downstream plugin drift remain separate tracks unless explicitly promoted.
